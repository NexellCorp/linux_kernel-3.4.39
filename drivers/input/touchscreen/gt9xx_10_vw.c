...
//	static const u16 touch_key_array[] = GTP_KEY_TAB;
	#define GTP_MAX_KEY_NUM	 8
	static  u16 touch_key_state[GTP_MAX_KEY_NUM] = {0};
#endif
...
s32 gtp_set_version(struct i2c_client *client,u8 version)
{
    s32 ret = 0;
    u8 buf[3]={GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff,0x4f};
...
Output:
	None.
*******************************************************/
static void gtp_key_up(struct goodix_ts_data* ts)
{
	u8 i;
...
Output:
	None.
*******************************************************/
static void gtp_key_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	u8 keyid;
//	printk("key: x=%d,y=%d\n",x,y);
	if(x > pkey_position[0].x-10  &&  x < pkey_position[0].x+2)
	{
		if(y>pkey_position[0].y-3 && y<pkey_position[0].y+3) keyid=0;
		else if(y>pkey_position[1].y-3 && y<pkey_position[1].y+3) keyid=1;
		else if(y>pkey_position[2].y-3 && y<pkey_position[2].y+3) keyid=2;
		else if(y>pkey_position[3].y-3 && y<pkey_position[3].y+3) keyid=3;
		else if(y>pkey_position[4].y-3 && y<pkey_position[4].y+3) keyid=4;
		else if(y>pkey_position[5].y-3 && y<pkey_position[5].y+3) keyid=5;
		else if(y>pkey_position[6].y-3 && y<pkey_position[6].y+3) keyid=6;
		else if(y>pkey_position[7].y-3 && y<pkey_position[7].y+3) keyid=7;
		else keyid=0xff;
		if(keyid!=0xff)
		{
			//printk("key %d down\n",pkeyid[keyid]);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (ts->enter_update)
    {
        printk(KERN_INFO "enter update , return\r\n");
        return;
    ...
    if((finger & 0x80) == 0)
    {
        //printk(KERN_INFO "(finger & 0x80) == 0 , goto exit_work_func \r\n");
        goto exit_work_func;
    }
    ...
    {
        //printk(KERN_INFO "touch_num > GTP_MAX_TOUCH , goto exit_work_func \r\n");
        goto exit_work_func;
    }
    if (touch_num > 1)
    {
    ...
    if (pre_touch || touch_num)
    {
        ...
        for (i = 0; i < GTP_MAX_TOUCH; i++)
        {
            if (touch_index & (0x01<<i))
            {
				...
				if(input_x>GTP_MAX_WIDTH)
				{
					gtp_key_down(ts, id, input_x, input_y, input_w);
					key_down = 1;
				}
				else
				{
...
*******************************************************/
static s32 gtp_init_panel(struct goodix_ts_data *ts,u8 resetid)
{
    s32 ret = -1;
...
#if GTP_CUSTOM_CFG
//    config[RESOLUTION_LOC]     = (u8)GTP_MAX_WIDTH_ALL;
//    config[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH_ALL>>8);
...
//    config[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);
    ...
    check_sum = 0;
    if(resetid) config[GTP_ADDR_LENGTH]=0x00;
    ...
    ts->abs_x_max = GTP_MAX_WIDTH;//(config[RESOLUTION_LOC + 1] << 8) + config[RESOLUTION_LOC];
    ts->abs_y_max = GTP_MAX_HEIGHT; //(config[RESOLUTION_LOC + 3] << 8) + config[RESOLUTION_LOC + 2];
...
static s8 gtp_findcfg(struct i2c_client *client)
{
    ...
    if(ret<0)
    {
    	GTP_ERROR("GTP get cfgid failed  ");
	    return -1;
	}
	cfgid = buf[2];
	...
	for(i=0;i<CFGNUM;i++)
	{
		if(cfgid==tpconfig[i].TPID) break;
	}
	...
	cfgslt = i;
	pkeyid=&tpconfig[cfgslt].keyid[0];
	pkey_position = &tpconfig[cfgslt].key_position[0];

//	printk("cfgslt = %d (%d,%d)\n",cfgslt,pkey_position->x,pkey_position->y);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    {
    	if(tpconfig[cfgslt].keyid[index]>0)
        input_set_capability(ts->input_dev,EV_KEY,tpconfig[cfgslt].keyid[index]);
    }
#endif
...
//    else
//    {
//        GTP_INFO("I2C communication Success!");
//    }
...
//    }
	...
	ret = gtp_init_panel(ts,0);
    if (ret < 0)
    {
        GTP_ERROR("GTP init panel failed(reset id).");
    }
