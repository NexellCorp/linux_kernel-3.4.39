/*
 * NEXELL NXP USB HOST EHCI Controller
 *
...
struct nxp_ehci_hcd {
	...
	struct clk *clk;
	struct usb_phy *phy;
#ifdef CONFIG_USB_EHCI_NXP_RESUME_WORK
...
static const struct hc_driver nxp_ehci_hc_driver = {
	...
	.product_desc		= "NXP EHCI Host Controller",
...
static int __devinit nxp_ehci_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
#if defined( CONFIG_USB_HSIC_NXP)
...
{
#ifndef CONFIG_USB_EHCI_NXP_RESUME_WORK
	struct nxp_ehci_hcd *nxp_ehci = dev_get_drvdata(dev);
	struct usb_hcd *hcd = nxp_ehci->hcd;
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	struct platform_device *pdev = to_platform_device(dev);
...
MODULE_ALIAS("platform:nxp-ehci-synop");
