...
unsigned short HNG_HAPI_UNITBL[] = 
{
	...
	0xD78C, 0xD790, 0xD798, 0xD799, 0xD79B, 0xD79D, 0x0000,    
};
...
void INC_UNICODE2ASCII(INC_INT8* pOutput, INC_UINT16* pUnicode, INC_UINT16 unSize)
{
/*
	INC_INT16              nIndex = 0;
	INC_UINT16   shData = 0;        
	char *           szTemp = 0;
	unsigned short * pshData = HNG_HAPI_UNITBL;        
	int              nLen = strlen(psAsciiBuf);

	memset(psAsciiBuf, 0x00, nLen);
	szTemp = psAsciiBuf;

	while((*psUnicodeBuf != 0) || (*(psUnicodeBuf+1) != 0))
	{
		if((psUnicodeBuf[1] == 0x31) && ((0x31<=psUnicodeBuf[0]) && (psUnicodeBuf[0] <=0x4e))) // ㄱ,ㄴ,ㄷ...
		{        
			*szTemp = 0xa4;                        
			szTemp++;
			*szTemp = *psUnicodeBuf + 0x70;
			psUnicodeBuf += 2;                        
			szTemp++;
		}                
		else if((psUnicodeBuf[0] < 0x80) && (psUnicodeBuf[1]==0x00)) // 영어
		{
			*szTemp = *psUnicodeBuf;
			szTemp++;
			psUnicodeBuf += 2;
		...
		{
			nIndex=0;
			pshData = HNG_HAPI_UNITBL;
			memcpy(&shData, psUnicodeBuf, 2);
			while(*pshData != 0)
			{
				if(shData == *pshData)
				{
			...
			*szTemp = (nIndex/94) + 0xB0;
			szTemp++;
			*szTemp = (nIndex%94) + 0xA1;
			psUnicodeBuf += 2;
			szTemp++;
		}
	}
...
void INC_TYPE5_EXTENSION2(ST_FIB_INFO* pFibInfo)
{
	...
	if(pType->ITEM.bitD2 == 1)
	{
		...
		if(nLoop == nTotalSeqNo)
		{
			...
			if(pEWSOutputMsg->ucForm) pEWSOutputMsg->ucResionCnt++;
			...
			if(nStrLen == (nTotalCnt - nIndex))
				...
				INC_UNICODE2ASCII(pEWSOutputMsg->acEWSMessage, pEWSOutputMsg->awUNICODE, nStrLen);
