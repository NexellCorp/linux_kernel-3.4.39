...
INC_UINT16 gaSyncValue[21];

INC_UINT32 g_SetDelay = 200;
INC_UINT32 g_ScanIFDelay = 1500;
INC_UINT32 g_ScanRFDelay = 1500;
...
INC_UINT8 INC_CHIP_STATUS(INC_UINT8 ucI2CID)
{
	...
	INC_MSG_PRINTF(1,"[INC] CHIP ID Read : (0x%04x)\n", uiChipID);
	if(uiChipID != INC_T3A00_CHIP_ID){
		INC_MSG_PRINTF(1,"[INC] CHIP ID Read Error : (0x%04x)\n", uiChipID);
		return INC_ERROR;
	}
	return INC_SUCCESS;
}
...
INC_UINT8 INC_PLL_SET(INC_UINT8 ucI2CID) 
{
	...
	case INPUT_CLOCK_19200KHZ:
		...
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);	// CLK_SEL OFF (T3A00)
		//asj 20131025 for Vco reference change
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x03C0);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x08A3);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x3D70);	
		INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0xA360);	

	break;
	default:
		INC_MSG_PRINTF(1,"[INC]Not Support ucPLL clock=(%d)\n", m_ucPLL_Mode);
		return INC_ERROR;	
	}
	...
	if(uiDpll != 0x0001){
		INC_MSG_PRINTF(1,"[INC] PLL Set Error : 0x%04x\n",uiDpll);
	...
	if(uiDpll != 0x01E2)
	{
		INC_MSG_PRINTF(1,"[INC] DPLL Set Error uiDpll =(0x%04x)\n", uiDpll);
		pInfo->nBbpStatus = ERROR_PLL;
		return INC_ERROR;
	}
...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
	...
	{
		INC_MSG_PRINTF(1,"[INC] T3A00 Chip ID Read Fail.\n");
		return INC_ERROR;
	}
	if(INC_PLL_SET(ucI2CID) != INC_SUCCESS) 		
	{
		INC_MSG_PRINTF(1,"[INC] T3A00 PLL INIT Fail.\n");
		return INC_ERROR;
	}
...
INC_UINT8 INC_READY(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
	...
	if( ulFreq == 196736 ) {
		if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x04C0)	{
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0000);

			//20131021 for vco reference change
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x01, 0x04C0);  
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x02, 0x0923);  
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x03, 0x7C00);  
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x04, 0x0060); 
			byIsChange = 1;
			}
		}
		else if(m_ucPLL_Mode == INPUT_CLOCK_19200KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x4C0)	{
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x00, 0x0000);
			//20131021 for vco reference change
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x01, 0x04C0);
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x02, 0x0BB3);  
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x03, 0x8000);  
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+ 0x04, 0x0060);  
		...
		{
			INC_MSG_PRINTF(1,"[INC]INC_READY ERROR\n");
			return INC_ERROR;
		}
		...
		if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) {
			...
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x03C0)	{
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x00, 0x0000);

			//20131018 for Vco reference change
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x01, 0x03C0);	
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x02, 0x08A3);	
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x3D70);	
			INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0xA360);	
...
INC_UINT8 INC_FICDECODER_CRC_COUNT(INC_UINT8 ucI2CID, ST_SIMPLE_FIC bSimpleFIC)
{
	INC_UINT16		wFicLen, uPeriodFrame, uFIBCnt, uiRefSleep = 20;
	INC_UINT16		nLoop, nFicCnt, nIndex;
	INC_UINT8		abyBuff[MAX_FIC_SIZE];
	ST_BBPINFO*		pInfo;
	...
	for(nLoop = 0; nLoop < nFicCnt; nLoop++)
	{
		...
		wFicLen = INC_CMD_READ(ucI2CID, APB_VTB_BASE+ 0x09) + 1;
		...
		{
			ST_FIB_INFO stFIB;
			INC_SET_UPDATEFIC(&stFIB, &abyBuff[nIndex*32]);
			if(stFIB.uiIsCRC == 0)
				return INC_ERROR;
...
INC_UINT32 INC_SYNCDETECTOR_CHECK(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
	if(INC_STOP(ucI2CID) != INC_SUCCESS)return INC_ERROR;
	if(INC_READY(ucI2CID, ulFreq) != INC_SUCCESS) return INC_ERROR;
	if(INC_SYNCDETECTOR(ucI2CID, ulFreq, 0) != INC_SUCCESS) return INC_ERROR;
	if(INC_FICDECODER_CRC_COUNT(ucI2CID, (ST_SIMPLE_FIC)0) != INC_SUCCESS) return INC_ERROR;
	return INC_SUCCESS;
}
...
INC_UINT8 INC_ENSEMBLE_SCAN(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
	...
	if(INC_SYNCDETECTOR(ucI2CID, ulFreq, 1) != INC_SUCCESS)
	{
		...
		//INC_MSG_PRINTF(1,"[ED0] = %x  [528]= %x \n", INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0xD0),
		//INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x28)) ;
...
*/
	return INC_SUCCESS ; //pInfo->ucSyncLock;
}
