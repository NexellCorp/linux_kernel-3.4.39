...
#if !defined(CONFIG_DVFS_NR_RUNNING_POLICY) && !defined(CONFIG_DVFS_NR_RUNNING_QUICK_POLICY)
#error "not configed nr running policy for cpu dynamic up/down"
#endif
...
static long hotplug_trigger_out = -1UL;
static long hotplug_trigger_in  = -1UL;
...
#ifdef CONFIG_DVFS_NR_RUNNING_POLICY
...
#ifdef CONFIG_DVFS_NR_RUNNING_QUICK_POLICY
static void dvfs_nr_based_hotplug(void *data, unsigned int event)
{
	struct cpu_hotplug_dvfs *p_data = (struct cpu_hotplug_dvfs *)data;
	unsigned int online = 0;
...
static void *hotplug_policy_init(void)
{
	struct cpufreq_frequency_table	*freq_table;
	unsigned int freq_min = -1UL, freq_max = 0, freq;
	int i, ret = 0;
	...
	if (-1UL == hotplug_trigger_out && -1UL == hotplug_trigger_in) {
		freq_table = cpufreq_frequency_get_table(0);
		if (IS_ERR(freq_table)) {
			pr_err("%s: cpufreq_table for CPU:%d not found\n", __func__, 0);
			return freq_table;
		}
		...
		cpu_p.freq_trg_out = (freq_max - freq_min)/2 + freq_min;
		cpu_p.freq_trg_in  = freq_min;
	} else {
		cpu_p.freq_trg_out = hotplug_trigger_out;
		cpu_p.freq_trg_in = hotplug_trigger_in;
	}
	...
	ret = cpufreq_register_notifier(&dvfs_hotplug, CPUFREQ_TRANSITION_NOTIFIER);
	if (ret)
		return ERR_PTR(-EINVAL);
	...
	pr_info("cpu: policy %s tunnable out %u, in %u (%u~%u)\n" ,
		h_cpu.policy.name, cpu_p.freq_trg_out, cpu_p.freq_trg_in, freq_max, freq_min);
	return ((void *)&cpu_p);
}
...
void nxp_dvfs_based_hotplug_trigger(long trigger_out, long trigger_in)
{
	 hotplug_trigger_out = trigger_out;
	 hotplug_trigger_in = trigger_in;
}
EXPORT_SYMBOL(nxp_dvfs_based_hotplug_trigger);
