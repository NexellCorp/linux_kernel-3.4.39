...
unsigned int nxp_cpufreq_voltage(unsigned long freqhz, int force)
{
	struct cpufreq_dvfs_data *dvfs = get_cpufreq_data();
        ...
        unsigned long mode, void *unused)
{
    ...
    case PM_SUSPEND_PREPARE:
    	...
    	nxp_cpufreq_voltage(max_freq, 1);
    	break;
...
static void nxp_cpufreq_update(struct cpufreq_dvfs_data *dvfs)
{
	...
	if (freqs.new > freqs.old)
		nxp_cpufreq_voltage(freqs.new*1000, 0);
...
static int __cpuinit nxp_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	/*
	 * NXP multi-core processors has 2 cores
