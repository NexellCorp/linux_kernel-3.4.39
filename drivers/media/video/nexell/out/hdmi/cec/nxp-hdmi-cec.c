...
#include "../hdmi-priv.h"
#elif defined(CONFIG_NEXELL_DISPLAY_HDMI)
#include "arch/arm/mach-nxp4330/soc/hdmi-priv.h"
#else
#error "you must config CONFIG_NXP_OUT_HDMI or CONFIG_NEXELL_DISPLAY_HDMI"
#endif
 ...
 * externs
 */
extern int register_hdmi_irq_callback(int int_num, int (*callback)(void *), void *data);
extern int unregister_hdmi_irq_callback(int int_num);
...
static int nxp_hdmi_cec_open(struct inode *inode, struct file *file)
{
    ...
    // irq
    if (register_hdmi_irq_callback(HDMI_INTC_EN_CEC, _irq_callback, me)) {
...
static int nxp_hdmi_cec_release(struct inode *inode, struct file *file)
{
    ...
    _cec_disable_irq();
    unregister_hdmi_irq_callback(HDMI_INTC_EN_CEC);
    me->open_count--;
    return 0;
}
