	...
	(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	struct DMB_OPEN_INFO_T *pDMB = (struct DMB_OPEN_INFO_T *)filp->private_data;
...
//	print_log(1, "dmb_ioctl\n");
	...
	case IOCTL_DMB_TUNER_GET_RSSI:
		if (dmb_isr_sig == 1) {
			...
			res = bbm_com_tuner_get_rssi(hInit, (u32 *)(&info.buff[0]));
			err |= copy_to_user((void *)arg, (void *)&info, size);
		}
		break;
...
//			print_log(hInit,"IOCTL_DMB_GET_BER conflict\n");
          ...
          break;
	default:
		print_log(hInit, "dmb ioctl error!\n");
		res = BBM_NOK;
		break;
	}
 ...
 *	Return 		: none.
 */
    nxp_soc_gpio_set_int_mode(CFG_IO_DMB_IRQ, 2);
    nxp_soc_gpio_set_int_enable(CFG_IO_DMB_IRQ, 0);

	res = request_irq(PB_PIO_IRQ(CFG_IO_DMB_IRQ),  dmb_irq, IRQF_TRIGGER_FALLING , FC8080_NAME, NULL);
	...
	if (res) {
		...
		nxp_soc_gpio_set_int_enable(CFG_IO_DMB_IRQ, 1);
		nxp_soc_gpio_clr_int_pend(CFG_IO_DMB_IRQ);
    }
