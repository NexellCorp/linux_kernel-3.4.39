...
void platform_hw_setting(void)
{
  ...
  nxp_soc_gpio_set_io_dir(CFG_GPIO_DMB_POWER_1V2, 1);  //output mode
  
  
  nxp_soc_gpio_set_io_pull_enb(CFG_GPIO_DMB_POWER, 0);     //pull disable
  nxp_soc_gpio_set_io_pull_enb(CFG_GPIO_DMB_POWER_1V2, 0); //pull disable
...
void platform_hw_init(u32 power_pin, u32 reset_pin)
{
   ...
   ms_wait(1);
   nxp_soc_gpio_set_out_value(CFG_GPIO_DMB_POWER_1V2, CTRUE);
   ms_wait(1);
...
void platform_hw_deinit(u32 power_pin)
{
   ...
   nxp_soc_gpio_set_out_value(CFG_GPIO_DMB_POWER_1V2, CFALSE);
   ms_wait(1);
   nxp_soc_gpio_set_out_value(CFG_GPIO_DMB_POWER, CFALSE);
