...
extern irqreturn_t mtv_isr_tdmb_primary(int irq, void *param);

#if 1
extern int mtv_isr_thread_tdmb_primary(void *data);
...
static int mtv_create_thread(void)
{
	...
	if (mtv_cb_ptr->isr_thread_cb == NULL) {
		...
		mtv_cb_ptr->isr_thread_cb
			= kthread_run(mtv_isr_thread_tdmb_primary, NULL, "mtv_isr_thread_tdmb_primary");
...
static int mtv_power_on(E_DMB_TV_MODE_TYPE tv_mode, unsigned long arg)
{
	...
	mtv_cb_ptr->isr_workqueue = create_singlethread_workqueue(NXTV_DEV_NAME);
...
static int mtv_init_device(void)
{
...
#if defined(NXTV_IF_SPI) || defined(NXTV_FIC_I2C_INTR_ENABLED)
	mtv_cb_ptr->irq = gpio_to_irq(NXTV_IRQ_INT); ///!!!! 
	ret = request_irq(mtv_cb_ptr->irq, mtv_isr_tdmb_primary,
					IRQ_TYPE_EDGE_FALLING, "mtv_isr_tdmb_primary", NULL);
	if (ret != 0) {
	...
	wake_lock_init(&mtv_cb_ptr->wake_lock, WAKE_LOCK_SUSPEND, NXTV_DEV_NAME);
...
static int __init mtv_tdmb_primary_module_init(void)
{
   	int ret;
...
static void __exit mtv_tdmb_primary_module_exit(void)
{
	mtv_deinit_device();
...
module_init(mtv_tdmb_primary_module_init);
module_exit(mtv_tdmb_primary_module_exit);
MODULE_DESCRIPTION("TDMB primary chip driver");
MODULE_LICENSE("GPL");
