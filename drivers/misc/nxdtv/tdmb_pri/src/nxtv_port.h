...
############################################################################*/
  ...
  #if defined(CONFIG_DTV_TDMB_1CHIP_AV_TPEG)
	#define NXTV_MAX_NUM_DAB_DATA_SVC	1
  #else
	#define NXTV_MAX_NUM_DAB_DATA_SVC	0
  #endif
...
############################################################################*/
	...
	*================================================================*/
	...
	#define	NXTV_REG_GET(reg)            			(U8)mtv_spi_read((U8)(reg))
	#define	NXTV_REG_BURST_GET(reg, buf, size) 		mtv_spi_read_burst((U8)(reg), buf, (size))
	#define	NXTV_REG_SET(reg, val)       			mtv_spi_write((U8)(reg), (U8)(val))       
	#define	NXTV_REG_MASK_SET(reg, mask, val)\
		do {					\
			U8 tmp;				\
			tmp = (NXTV_REG_GET(reg)|(U8)(mask)) & (U8)((~(mask))|(val));\
...
############################################################################*/
	...
	#if defined(__KERNEL__)	
		extern struct mutex nxtv_guard;
		#define NXTV_GUARD_INIT		mutex_init(&nxtv_guard)
		#define NXTV_GUARD_LOCK		mutex_lock(&nxtv_guard)
		#define NXTV_GUARD_FREE		mutex_unlock(&nxtv_guard)
		#define NXTV_GUARD_DEINIT 	((void)0)
		
    #elif defined(WINCE)        
	        extern CRITICAL_SECTION		nxtv_guard;
	        #define NXTV_GUARD_INIT		InitializeCriticalSection(&nxtv_guard)
	        #define NXTV_GUARD_LOCK		EnterCriticalSection(&nxtv_guard)
	        #define NXTV_GUARD_FREE		LeaveCriticalSection(&nxtv_guard)
	        #define NXTV_GUARD_DEINIT	DeleteCriticalSection(&nxtv_guard)
	#else
		// temp: TODO
