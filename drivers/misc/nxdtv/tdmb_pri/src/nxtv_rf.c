 ...
 * RETURN VALUE : none.
 *============================================================================*/
void nxtvRF_ConfigurePowerType(E_NXTV_TV_MODE_TYPE eTvMode)
{
...
INT nxtvRF_ConfigureAdcClock(E_NXTV_TV_MODE_TYPE eTvMode, E_NXTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	U8 REGE9,RD15;
	INT i;
	...
	{
	        NXTV_DBGMSG1("[nxtvRF_ConfigureAdcClock] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
	...
	for(i=0; i<10;i++)
	{
		...
		{
			NXTV_DBGMSG0("[nxtvRF_ConfigureAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	if(i == 10)
	{
		NXTV_DBGMSG0("[nxtvRF_ConfigureAdcClock] ADC clock unlocked!Check the power supply.\n");
	...
	//NXTV_DBGMSG1("[nxtvRF_ConfigureAdcClock] ADC clk Type: %d\n", g_eRtvAdcClkFreqType[RaonTvChipIdx]);

	return NXTV_SUCCESS;
}


INT nxtvRF_ChangeAdcClock(E_NXTV_TV_MODE_TYPE eTvMode, E_NXTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{	
	U8 REGE9, RD15;
#ifdef NXTV_PWR_DCDC	
	U8 RD2F;
	...
	if(pbAdcClkSynTbl[0] == 0xFF)
    {
		NXTV_DBGMSG1("[nxtvRF_ChangeAdcClock] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
		return NXTV_UNSUPPORT_ADC_CLK;
    }
	...
	for(i=0; i<10;i++)
	{
		...
		{
			NXTV_DBGMSG0("[nxtvRF_ChangeAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	if(i == 10)
	{
		NXTV_DBGMSG0("[nxtvRF_ChangeAdcClock] ADC clock unlocked!Check the power supply.\n");
		return NXTV_ADC_CLK_UNLOCKED;
	}
	...
	{
		NXTV_DBGMSG0("[nxtvRF_ChangeAdcClock] DCDC CLOCK SYNTH  UnLock..\n");
	}
#endif
...
//	NXTV_DBGMSG1("[nxtvRF_ChangeAdcClock] ADC clk Type: %d\n", g_eRtvAdcClkFreqType[RaonTvChipIdx]);

	return NXTV_SUCCESS;	
}


INT nxtvRF_SetFrequency(E_NXTV_TV_MODE_TYPE eTvMode, UINT nChNum, U32 dwChFreqKHz)
{
#if defined(NXTV_TDMB_ENABLE) || defined(NXTV_DAB_ENABLE)
	int nIdx;
#endif
...
#ifdef NXTV_ISDBT_ENABLE
		case NXTV_TV_MODE_1SEG :
			if((g_eRtvCountryBandType==NXTV_COUNTRY_BAND_BRAZIL) || (g_eRtvCountryBandType==NXTV_COUNTRY_BAND_ARGENTINA))		
				nChNum -= 1;
	...
	if((nRet=nxtvRF_ChangeAdcClock(eTvMode, eAdcClkFreqType)) != NXTV_SUCCESS)
		goto RF_SET_FREQ_EXIT;
	...
	switch( eTvMode )
	{
		...
		case NXTV_TV_MODE_1SEG :			
			...
			do{
				...
				{ 
					NXTV_REG_SET(0x20, nChNum);	
					NXTV_DELAY_MS(2);	//2ms Delay 
				//	NXTV_DBGMSG3("[nxtvRF_SetFrequency] 1SEG PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
		...
		case NXTV_TV_MODE_FM :		
			...
			do{
				...
				{ 
					NXTV_REG_SET(0x20, 0x00);	
					NXTV_DELAY_MS(2);	//2ms Delay 
				//	NXTV_DBGMSG3("[nxtvRF_SetFrequency] FM PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
		...
		case NXTV_TV_MODE_TDMB :	
			...
			do{
				...
				//NXTV_DBGMSG3("[nxtvRF_SetFrequency] TDMB PLL PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				...
				{ 
					NXTV_REG_SET(0x20, 0x00);	
					NXTV_DELAY_MS(2);	//2ms Delay 
					NXTV_DBGMSG3("[nxtvRF_SetFrequency] TDMB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
		...
		case NXTV_TV_MODE_TDMB:
			...
			do{
				...
				{ 
					NXTV_REG_SET(0x20, 0x00);	
					NXTV_DELAY_MS(2);	//2ms Delay 
				//	NXTV_DBGMSG3("[nxtvRF_SetFrequency] DAB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
	...
	if(PLL_Verify_cnt == 0)
	{
		NXTV_DBGMSG0("[nxtvRF_SetFrequency] PLL unlocked!\n");
...
INT nxtvRF_Initilize(E_NXTV_TV_MODE_TYPE eTvMode)
{
...
#ifdef NXTV_PWR_DCDC
	{
		...
		{
			NXTV_DBGMSG0("[nxtvRF_Initilize] Clock Unlock \n");
		}
	}
#endif
//////////////////////////////CODE PATCH FOR OTP////////////////////////////////////
