...
volatile E_NXTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType;
BOOL g_fRtvStreamEnabled;

#if defined(NXTV_TDMB_ENABLE) || defined(NXTV_ISDBT_ENABLE)
	E_NXTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType;
#endif
...
void nxtv_ConfigureHostIF(void)
{
...
INT nxtv_InitSystem(E_NXTV_TV_MODE_TYPE eTvMode, E_NXTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	INT nRet;
	int i;
	...
	for(i=1; i<=100; i++)
	{
		...
		NXTV_DBGMSG1("[nxtv_InitSystem] Power On wait: %d\n", i);

		NXTV_DELAY_MS(5);
	}

	NXTV_DBGMSG1("nxtv_InitSystem: Power On Check error: %d\n", i);
	return NXTV_POWER_ON_CHECK_ERROR;

NXTV_POWER_ON_SUCCESS:
	
	nxtvRF_ConfigurePowerType(eTvMode);

	if((nRet=nxtvRF_ConfigureAdcClock(eTvMode, eAdcClkFreqType)) != NXTV_SUCCESS)
		return nRet;
