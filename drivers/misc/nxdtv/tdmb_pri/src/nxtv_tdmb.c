...
static void tdmb_InitOFDM(void)
{
	...
	if(g_eRtvCountryBandType == NXTV_COUNTRY_BAND_KOREA)
	{
...
void nxtvTDMB_StandbyMode(int on)
{
	NXTV_GUARD_LOCK;
...
UINT nxtvTDMB_GetLockStatus(void)
{
	U8 lock_stat;
	UINT lock_st = 0;
   	...
   	{
   		NXTV_DBGMSG0("[nxtvTDMB_GetLockStatus] NXTV Freqency change state! \n");	
		return 0x0;	 
	}
...
U32 nxtvTDMB_GetPER(void)
{
    U8 rdata0, rdata1, rs_sync;

	if(g_fRtvChannelChange) 
	{
		NXTV_DBGMSG0("[nxtvTDMB_GetPER] NXTV Freqency change state! \n");
		return 0;	 
	}	
...
S32 nxtvTDMB_GetRSSI(void) 
{
	...
	if(g_fRtvChannelChange)
	{
		NXTV_DBGMSG0("[nxtvTDMB_GetRSSI] NXTV Freqency change state! \n");
		return 0;
	}	
...
U32 nxtvTDMB_GetCNR(void)
{
	...
	if(g_fRtvChannelChange) 
	{
		NXTV_DBGMSG0("[nxtvTDMB_GetCNR] NXTV Freqency change state! \n");
		return 0;	 
	}	
...
U32 nxtvTDMB_GetCER(void)
{
	...
	if(g_fRtvChannelChange) {
		NXTV_DBGMSG0("[nxtvTDMB_GetCER] NXTV Freqency change state! \n");
		return 2000;	 
	}	
	...
	if(cer_period_cnt != 0) {
		...
		//NXTV_DBGMSG2("[nxtvTDMB_GetCER] cer_cnt: %u, cer_period_cnt: %u\n", cer_cnt, cer_period_cnt);
...
U32 nxtvTDMB_GetBER(void)
{
	...
	if(g_fRtvChannelChange) 
	{
		NXTV_DBGMSG0("[nxtvTDMB_GetBER] NXTV Freqency change state! \n");
...
UINT nxtvTDMB_GetAntennaLevel(U32 dwCER)
{
	UINT nCurLevel = 0;
...
U32 nxtvTDMB_GetPreviousFrequency(void)
{
...
//	g_fRtvFicOpened, g_nRtvFicOpenedStatePath);
	...
	NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL);
	#endif
...
void nxtvTDMB_CloseAllSubChannels(void)
{
	NXTV_GUARD_LOCK;
...
INT nxtvTDMB_CloseSubChannel(UINT nSubChID)
{
...
static void tdmb_OpenSubChannel(UINT nSubChID, E_NXTV_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	NXTV_REG_SET(INT_E_UCLRL, MSC1_INTR_BITS); // MSC1 Interrupts status clear.
  	...
  	NXTV_REG_MAP_SEL(HOST_PAGE);
  	g_bRtvIntrMaskRegL &= ~(MSC1_INTR_BITS);
  	NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL); /* Enable MSC1 interrupts. */ 
	#endif
	...
	if((eServiceType == NXTV_SERVICE_VIDEO) || (eServiceType == NXTV_SERVICE_AUDIO))
	{
		...
		{	/* First enabled. */
			...
			NXTV_REG_MAP_SEL(HOST_PAGE);	
			g_bRtvIntrMaskRegL &= ~(MSC1_INTR_BITS);
			NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL); /* Enable MSC1 interrupts and restore FIC . */		
		#endif
		}
	...
	else /* Data */
	{
		...
		if((g_nRtvUsedHwSubChIdxBits & TDMB_MSC0_SUBCH_USE_MASK) == 0)
		{
			...
			NXTV_REG_MAP_SEL(HOST_PAGE);	
			g_bRtvIntrMaskRegL &= ~(MSC0_INTR_BITS);
			NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL);	 // restore FIC	
	#endif
		}
...
INT nxtvTDMB_OpenSubChannel(U32 dwChFreqKHz, UINT nSubChID, 
			E_NXTV_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	if(g_dwTdmbPrevChFreqKHz == dwChFreqKHz)
	{
		...
		if(g_aRegSubChIdBits[DIV32(nSubChID)] & (1<<MOD32(nSubChID)))
		{
			...
			NXTV_DBGMSG1("[nxtvTDMB_OpenSubChannel] Already opened sub channed ID(%d)\n", nSubChID);
	...
	else
	{
		...
		nRet = nxtvRF_SetFrequency(NXTV_TV_MODE_TDMB, 0, dwChFreqKHz);
...
INT nxtvTDMB_OpenSubChannelExt(U32 dwChFreqKHz, UINT nSubChID, 
			E_NXTV_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	INT nRet = NXTV_SUCCESS;
	...
	nRet = nxtvTDMB_ScanFrequency(dwChFreqKHz);
...
INT nxtvTDMB_ReadFIC(U8 *pbBuf)
{
	...
	{
		NXTV_DBGMSG0("[nxtvTDMB_ReadFIC] NOT OPEN FIC\n");
...
//		    printk("FIC_E_INT occured!\n");
	...
	NXTV_DBGMSG0("[nxtvTDMB_ReadFIC] FIC read timeout\n");
...
#if defined(NXTV_IF_SERIAL_TSIF) || defined(NXTV_IF_SPI_SLAVE)
	//NXTV_DBGMSG1("[nxtvTDMB_CloseFIC] Closing with FIC_state_path(%d)\n", g_nRtvFicOpenedStatePath);
...
INT nxtvTDMB_OpenFIC(void)
{
	if (g_fRtvFicOpened == TRUE) {
		NXTV_DBGMSG0("[nxtvTDMB_OpenFIC] Must close FIC prior to opening FIC!\n");
	...
	//NXTV_DBGMSG1("[nxtvTDMB_OpenFIC] Opened with FIC_state_path(%d)\n", g_nRtvFicOpenedStatePath);
#else
 ...
 */
INT nxtvTDMB_ScanFrequency(U32 dwChFreqKHz)
{
	...
	scan_flag = nxtvRF_SetFrequency(NXTV_TV_MODE_TDMB, 0, dwChFreqKHz);
	...
	while(1)
	{
		...
		{
			NXTV_DBGMSG0("[nxtvTDMB_ScanFrequency] Scan Timeout! \n");
			scan_flag = NXTV_CHANNEL_NOT_DETECTED;
			break;
		}
...
TDMB_SCAN_EXIT:
	...
	//NXTV_DBGMSG2("[nxtvTDMB_ScanFrequency] RF Freq = %d PreGain = %d ",dwChFreqKHz, PreGain);
...
void nxtvTDMB_DisableTiiInterrupt(void)
{
	NXTV_GUARD_LOCK;
...
void nxtvTDMB_EnableTiiInterrupt(void)
{
	NXTV_GUARD_LOCK;
...
void nxtvTDMB_GetTii(NXTV_TDMB_TII_INFO *ptTiiInfo)
{
	NXTV_GUARD_LOCK;
...
INT nxtvTDMB_Initialize(E_NXTV_COUNTRY_BAND_TYPE eRtvCountryBandType)
{
	INT nRet;
	...
	nRet = nxtv_InitSystem(NXTV_TV_MODE_TDMB, NXTV_ADC_CLK_FREQ_8_MHz);
	if(nRet != NXTV_SUCCESS)
		return nRet;

	/* Must after nxtv_InitSystem() to save ADC clock. */
	tdmb_InitDemod();
	
	nRet = nxtvRF_Initilize(NXTV_TV_MODE_TDMB);
	if(nRet != NXTV_SUCCESS)
		return nRet;
...
#ifdef NXTV_CIF_MODE_ENABLED
	NXTV_DBGMSG0("[nxtvTDMB_Initialize] CIF enabled\n");
#else
	NXTV_DBGMSG0("[nxtvTDMB_Initialize] SINGLE CHANNEL enabled\n");
#endif
