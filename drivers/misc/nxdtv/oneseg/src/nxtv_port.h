...
*===========================================================================*/
///TEST
//#define NXTV_CHIP_PKG_WLCSP	// MTV220/318
#define NXTV_CHIP_PKG_QFN		// MTV818
//#define NXTV_CHIP_PKG_LGA	// MTV250/350
...
############################################################################*/
	...
	#define NXTV_MAX_NUM_DAB_AV_SVC	1
...
############################################################################*/
	...
	* Defines the register I/O macros.
	*================================================================*/
	unsigned char mtv_spi_read_1seg(unsigned char reg);
	void mtv_spi_read_burst_1seg(unsigned char reg, unsigned char *buf, int size);
	void mtv_spi_write_1seg(unsigned char reg, unsigned char val);

	#define	NXTV_REG_GET(reg)            			(U8)mtv_spi_read_1seg((U8)(reg))
	#define	NXTV_REG_BURST_GET(reg, buf, size) 		mtv_spi_read_burst_1seg((U8)(reg), buf, (size))
	#define	NXTV_REG_SET(reg, val)       			mtv_spi_write_1seg((U8)(reg), (U8)(val))       
	#define	NXTV_REG_MASK_SET(reg, mask, val)\
		do {					\
			U8 tmp;				\
			tmp = (NXTV_REG_GET(reg)|(U8)(mask)) & (U8)((~(mask))|(val));\
...
############################################################################*/
	...
	#if defined(__KERNEL__)	
		extern struct mutex nxtv_guard_1seg;
		#define NXTV_GUARD_INIT		mutex_init(&nxtv_guard_1seg)
		#define NXTV_GUARD_LOCK		mutex_lock(&nxtv_guard_1seg)
		#define NXTV_GUARD_FREE		mutex_unlock(&nxtv_guard_1seg)
		#define NXTV_GUARD_DEINIT 	((void)0)
		
    #elif defined(WINCE)        
	        extern CRITICAL_SECTION		nxtv_guard_1seg;
	        #define NXTV_GUARD_INIT		InitializeCriticalSection(&nxtv_guard_1seg)
	        #define NXTV_GUARD_LOCK		EnterCriticalSection(&nxtv_guard_1seg)
	        #define NXTV_GUARD_FREE		LeaveCriticalSection(&nxtv_guard_1seg)
	        #define NXTV_GUARD_DEINIT	DeleteCriticalSection(&nxtv_guard_1seg)
	#else
		// temp: TODO
...
void nxtvOEM_ConfigureInterrupt_1SEG(void);
void nxtvOEM_PowerOn_1SEG(int on);
