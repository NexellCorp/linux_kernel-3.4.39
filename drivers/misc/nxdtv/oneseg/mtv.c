...
struct mtv_cb *mtv_1seg_cb_ptr;

#if defined(NXTV_IF_SPI) || defined(NXTV_FIC_I2C_INTR_ENABLED)
extern irqreturn_t mtv_isr_1seg(int irq, void *param);

#if 1
extern int mtv_isr_thread_1seg(void *data);
...
unsigned int get_tsp_queue_count_1seg(void)
{
...
static void mtv_reset_tsp_queue(void)
{
	...
	mtv_1seg_cb_ptr->read_stop = TRUE;

	/* Acquire the mutex. */
	mutex_lock(&mtv_1seg_cb_ptr->read_lock);

	if (mtv_tsp_pool.cnt != MAX_NUM_TS_PKT_BUF) {
		/* Free a previous TSP use in read(). */
		if (mtv_1seg_cb_ptr->prev_tsp) {
			mtv_free_tsp_1seg(mtv_1seg_cb_ptr->prev_tsp);
			mtv_1seg_cb_ptr->prev_tsp = NULL; 
		}

		while ((tsp=mtv_get_tsp_1seg()) != NULL) 
			mtv_free_tsp_1seg(tsp);
	...
	mutex_unlock(&mtv_1seg_cb_ptr->read_lock);

#ifdef DEBUG_MTV_IF_MEMORY
	DMBMSG("Max alloc #tsp: %u\n", mtv_1seg_cb_ptr->max_alloc_tsp_cnt);
#endif
}

/* Peek a ts packet from ts data queue. */
MTV_TS_PKT_INFO * mtv_peek_tsp_1seg(void)
{
...
MTV_TS_PKT_INFO * mtv_get_tsp_1seg(void)
{
	MTV_TS_PKT_INFO *tsp = NULL;
	struct list_head *head_ptr = &mtv_tsp_queue.head;
...
void mtv_put_tsp_1seg(MTV_TS_PKT_INFO *tsp)
{
	spin_lock(&mtv_tsp_queue.lock);
...
void mtv_free_tsp_1seg(MTV_TS_PKT_INFO *tsp)
{	
	if (tsp == NULL)
		return;
...
MTV_TS_PKT_INFO *mtv_alloc_tsp_1seg(void)
{	
	MTV_TS_PKT_INFO *tsp = NULL;
...
#ifdef DEBUG_MTV_IF_MEMORY
		mtv_1seg_cb_ptr->max_alloc_tsp_cnt 
			= MAX(mtv_1seg_cb_ptr->max_alloc_tsp_cnt,
...
static int mtv_create_tsp_pool(void)
{
	...
	mtv_tsp_pool.cnt = 0;
#ifdef DEBUG_MTV_IF_MEMORY
	mtv_1seg_cb_ptr->max_alloc_tsp_cnt = 0;
#endif
...
static void delete_msc_decoding_buffer(void)
{
	...
	for (i = 0; i < NXTV_MAX_NUM_DAB_DATA_SVC; i++) {
		if (mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i]) {
			vfree(mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i]);
			mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i] = NULL;
		}
	}
}
...
static int create_msc_decoding_buffer(void)
{
	...
	for (i = 0; i < NXTV_MAX_NUM_DAB_DATA_SVC; i++) {
		mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i] = vmalloc(MAX_MULTI_MSC_BUF_SIZE);
		if (mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i] == NULL) {
...
{		
	...
	if (mtv_1seg_cb_ptr->f_flags & O_NONBLOCK) { /* Non-blocking mode. */
		if (mtv_tsp_queue.cnt == 0)
			return -EAGAIN;
	}
	else { /* The file is opened in blocking mode. */
		if (wait_event_interruptible(mtv_1seg_cb_ptr->read_wq,
			mtv_tsp_queue.cnt || (mtv_1seg_cb_ptr->read_stop == TRUE))) {
	...
	for (i = 0; i < NXTV_MAX_NUM_DAB_DATA_SVC; i++)
		...
		#else
		cifdec.msc_buf_ptr[i] = mtv_1seg_cb_ptr->dec_msc_kbuf_ptr[i]; /* kernel buffer */
		#endif
	#endif
#endif
	...
	while (1) {
		if (mtv_1seg_cb_ptr->read_stop == TRUE) {
			ret = -EINTR; /* Force stopped. */
			goto unlock_read;
		}

		/* Check if the size of MSC input buffer to be copied. */
		if (atomic_read(&mtv_1seg_cb_ptr->num_opened_subch)) { /* subch opened */
			if ((tsp = mtv_peek_tsp_1seg()) != NULL) {
		#ifdef NXTV_SPI_MSC1_ENABLED
		...
		if (tsp->msc1_size != 0) { /* Force 0 index to Video/Audio.*/
			if (subch_id_tbl[0] == 0xFF) /* Default Invalid. */
				subch_id_tbl[0] = mtv_1seg_cb_ptr->av_subch_id;

			/* Check if subch ID was changed in the same out buf index ? */
			if (subch_id_tbl[0] == mtv_1seg_cb_ptr->av_subch_id) {
				...
				DMBMSG("Changed ID: from old(%u) to new(%u)\n",
						subch_size_tbl[0], mtv_1seg_cb_ptr->av_subch_id);
		}
#endif
		...
		if (tsp->msc0_size != 0) {
			...
			if (subch_id_tbl[MSC0_BASE_IDX] == 0xFF) /* Default Invalid. */
				subch_id_tbl[MSC0_BASE_IDX] = mtv_1seg_cb_ptr->data_subch_id;
			
			/* Check if subch ID was changed in the same out buf index ? */
			if (subch_id_tbl[MSC0_BASE_IDX] == mtv_1seg_cb_ptr->data_subch_id) {
				...
				DMBMSG("Changed ID: from old(%u) to new(%u)\n",
					subch_size_tbl[MSC0_BASE_IDX], mtv_1seg_cb_ptr->data_subch_id);
	#else
		#ifdef NXTV_BUILD_CIFDEC_WITH_DRIVER
...
unlock_read:
	mtv_free_tsp_1seg(tsp); /* for "goto unlock_read;" statement. */
...
exit_read:
	...
	mtv_1seg_cb_ptr->max_remaining_tsp_cnt
		= MAX(mtv_1seg_cb_ptr->max_remaining_tsp_cnt, mtv_tsp_queue.cnt);
#endif
...
static inline ssize_t read_single_service(char *buf, size_t cnt)
{
	...
	if (mtv_1seg_cb_ptr->f_flags & O_NONBLOCK) { /* non-blocking mode. */
		unsigned int have_tsp_bytes
			= mtv_tsp_queue.cnt * mtv_1seg_cb_ptr->msc1_thres_size;
	...
	do {
		...
		if (mtv_1seg_cb_ptr->prev_tsp == NULL) {
			/* Dequeue a tsp from tsp_queue. */
			if ((tsp = mtv_get_tsp_1seg()) == NULL)
				goto unlock_read; /* Stop. */

			mtv_1seg_cb_ptr->prev_tsp = tsp;
			msc_buf_ptr = &tsp->msc1_buf[1];
		}
		else
			tsp = mtv_1seg_cb_ptr->prev_tsp;
		...
		if (copy_to_user(buf, msc_buf_ptr, copy_bytes)) {
			...
			if (tsp->msc1_size == 0) {
				mtv_free_tsp_1seg(tsp);
				mtv_1seg_cb_ptr->prev_tsp = NULL; /* All used. */
			}
		}
...
static ssize_t mtv_read(struct file *filp, char *buf, size_t count, loff_t *pos)
{
	...
	if (atomic_cmpxchg(&mtv_1seg_cb_ptr->read_flag, 0, 1)) {
	...
	atomic_set(&mtv_1seg_cb_ptr->read_flag, 0);
...
static int mtv_create_thread(void)
{
	...
	if (mtv_1seg_cb_ptr->isr_thread_cb == NULL) {
		init_waitqueue_head(&mtv_1seg_cb_ptr->isr_wq);
 
		mtv_1seg_cb_ptr->isr_thread_cb
			= kthread_run(mtv_isr_thread_1seg, NULL, "mtv_isr_thread_1seg");

		if (IS_ERR(mtv_1seg_cb_ptr->isr_thread_cb)) {
			WARN(1, KERN_ERR "[mtv_create_thread] Thread create error\n");
			ret = PTR_ERR(mtv_1seg_cb_ptr->isr_thread_cb);
			mtv_1seg_cb_ptr->isr_thread_cb = NULL;
		}
	}
...
#if 1
	if (mtv_1seg_cb_ptr->isr_thread_cb != NULL) {
		kthread_stop(mtv_1seg_cb_ptr->isr_thread_cb);
		mtv_1seg_cb_ptr->isr_thread_cb = NULL;
	}
#else
#endif
...
#if defined(NXTV_IF_SPI) || defined(NXTV_FIC_I2C_INTR_ENABLED)
	disable_irq(mtv_1seg_cb_ptr->irq);

	#if 1
	mtv_delete_thread();
	#else
	cancel_work_sync(&mtv_1seg_cb_ptr->tdmb_isr_work);
	flush_workqueue(mtv_1seg_cb_ptr->isr_workqueue);
	destroy_workqueue(mtv_1seg_cb_ptr->isr_workqueue);
	mtv_1seg_cb_ptr->isr_workqueue = NULL;
	#endif
#endif
...
static int mtv_power_on(E_DMB_TV_MODE_TYPE tv_mode, unsigned long arg)
{
	...
	#if 0
	INIT_WORK(&mtv_1seg_cb_ptr->tdmb_isr_work, mtv_isr_handler);
	mtv_1seg_cb_ptr->isr_workqueue = create_singlethread_workqueue(NXTV_DEV_NAME);
	if (mtv_1seg_cb_ptr->isr_workqueue == NULL) {
		DMBERR("couldn't create workqueue\n");
		return -ENOMEM;
	}
	#else
	mtv_1seg_cb_ptr->isr_cnt = 0;
	if ((ret=mtv_create_thread()) != 0)
		return ret;
	#endif

	enable_irq(mtv_1seg_cb_ptr->irq); /* After DMB init */
#endif

	mtv_1seg_cb_ptr->tv_mode = tv_mode;
...
#endif
{
	...
	mutex_lock(&mtv_1seg_cb_ptr->ioctl_lock);
	...
	mutex_unlock(&mtv_1seg_cb_ptr->ioctl_lock);
...
{
	return fasync_helper(fd, filp, mode, &mtv_1seg_cb_ptr->fasync);
}
#endif
...
static int mtv_close(struct inode *inode, struct file *filp)
{
	...
	mtv_power_off();
	mutex_unlock(&mtv_1seg_cb_ptr->ioctl_lock);
	mutex_destroy(&mtv_1seg_cb_ptr->ioctl_lock);

#if defined(NXTV_IF_SPI)
	mtv_delete_tsp_pool();
	mutex_destroy(&mtv_1seg_cb_ptr->read_lock);
#endif

#if defined(NXTV_FIC_I2C_INTR_ENABLED)
	fasync_helper(-1, filp, 0, &mtv_1seg_cb_ptr->fasync);
#endif

	wake_unlock(&mtv_1seg_cb_ptr->wake_lock);

	atomic_set(&mtv_1seg_cb_ptr->open_flag, 0);
...
static int mtv_open(struct inode *inode, struct file *filp)
{
...
#if defined(CONFIG_ISDBT_1SEG_MTV818)
	DMBMSG("ISDBT1SEG MTVOPEN\n");
#endif
	/* Check if the device is already opened ? */
	if (atomic_cmpxchg(&mtv_1seg_cb_ptr->open_flag, 0, 1)) {
	...
	/* Allow thread to block on read() if the blocking mode. */
	mtv_1seg_cb_ptr->read_stop = FALSE;
	mutex_init(&mtv_1seg_cb_ptr->read_lock);

	if ((filp->f_flags & O_NONBLOCK) == 0) { /* Blocking mode */
		init_waitqueue_head(&mtv_1seg_cb_ptr->read_wq);
	...
	mtv_1seg_cb_ptr->f_flags = filp->f_flags;
	mtv_1seg_cb_ptr->is_power_on = FALSE;
	mutex_init(&mtv_1seg_cb_ptr->ioctl_lock);

	wake_lock(&mtv_1seg_cb_ptr->wake_lock);
...
{
	wake_lock_destroy(&mtv_1seg_cb_ptr->wake_lock);

#if defined(NXTV_IF_SPI) ||defined(NXTV_FIC_I2C_INTR_ENABLED)
	free_irq(gpio_to_irq(NXTV_IRQ_INT), NULL);
#endif
	...
	spi_unregister_driver(&mtv_spi_driver_1seg);
#endif

	if (mtv_1seg_cb_ptr) {
		kfree(mtv_1seg_cb_ptr);
		mtv_1seg_cb_ptr = NULL;
	}
...
static int mtv_init_device(void)
{
	...
	mtv_1seg_cb_ptr = kzalloc(sizeof(struct mtv_cb), GFP_KERNEL);
	if (!mtv_1seg_cb_ptr) {
		DMBERR("mtv_1seg_cb_ptr allocate error!\n");
		return -ENOMEM;
	}


	//DMBMSG("mtv_1seg_cb_ptr(0x%p), mtv_1seg_cb_ptr[%d](0x%p)\n", mtv_1seg_cb_ptr, i, mtv_1seg_cb_ptr[i]);
...
#elif defined(NXTV_IF_SPI)
	if ((ret=spi_register_driver(&mtv_spi_driver_1seg)) < 0) {
...
#if defined(NXTV_IF_SPI) || defined(NXTV_FIC_I2C_INTR_ENABLED)
	mtv_1seg_cb_ptr->irq = gpio_to_irq(NXTV_IRQ_INT); ///!!!! 
	ret = request_irq(mtv_1seg_cb_ptr->irq, mtv_isr_1seg,
					IRQ_TYPE_EDGE_FALLING, "mtv_isr_1seg", NULL);
	if (ret != 0) {
		DMBERR("Failed to install irq (%d)\n", ret);
		goto err;
	}
	disable_irq(mtv_1seg_cb_ptr->irq); /* Must disabled */
#endif

	atomic_set(&mtv_1seg_cb_ptr->open_flag, 0);
	atomic_set(&mtv_1seg_cb_ptr->read_flag, 0);

	wake_lock_init(&mtv_1seg_cb_ptr->wake_lock, WAKE_LOCK_SUSPEND, NXTV_DEV_NAME);
...
static int __init mtv_1seg_module_init(void)
{
   	int ret;
...
static void __exit mtv_1seg_module_exit(void)
{
	mtv_deinit_device();
...
module_init(mtv_1seg_module_init);
module_exit(mtv_1seg_module_exit);
MODULE_DESCRIPTION("MTV 1seg driver");
MODULE_LICENSE("GPL");
