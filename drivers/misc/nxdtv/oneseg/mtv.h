...
#include "./src/nxtv.h" /* Place before mtv_ioctl.h ! */
#include "mtv_ioctl.h"
...
#define DMBERR(fmt, args...) \
	do { printk(KERN_ERR "ISDBT_1seg: %s(): " fmt, __func__, ## args); } while (0)

#ifdef DMB_DEBUG
	#define DMBMSG(fmt, args...) \
		printk(KERN_INFO "ISDBT_1seg: %s(): " fmt, __func__, ## args)
#else 
...
struct mtv_cb
{
	...
	and mtv_free_tsp_1seg(mtv_1seg_cb_ptr->prev_tsp) on ioctl:mtv_reset_tsp_queue(). */
	...
	&& defined(NXTV_BUILD_CIFDEC_WITH_DRIVER)
	/* Pointer to DATA MSC buffer to be decoded. */
	unsigned char *dec_msc_kbuf_ptr[NXTV_MAX_NUM_DAB_DATA_SVC];
	#endif
#endif
...
extern struct mtv_cb *mtv_1seg_cb_ptr;
#if defined(NXTV_IF_TSIF) || defined(NXTV_IF_SPI_SLAVE)
	extern struct i2c_driver mtv_i2c_driver;
#elif defined(NXTV_IF_SPI)
	extern struct spi_driver mtv_spi_driver_1seg;
#endif
...
void mtv_put_tsp_1seg(MTV_TS_PKT_INFO *pkt);
void mtv_free_tsp_1seg(MTV_TS_PKT_INFO *pkt);
