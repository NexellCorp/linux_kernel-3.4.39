...
static INLINE void reset_msc_if_debug(void)
{
	...
	mtv_sec_cb_ptr->msc0_ts_intr_cnt = 0;
	mtv_sec_cb_ptr->msc0_ovf_intr_cnt = 0;

	#ifdef NXTV_CIF_MODE_ENABLED
	mtv_sec_cb_ptr->msc0_cife_cnt = 0;
	#endif
#endif	

#ifdef NXTV_SPI_MSC1_ENABLED
	mtv_sec_cb_ptr->msc1_ts_intr_cnt = 0;
	mtv_sec_cb_ptr->msc1_ovf_intr_cnt = 0;
#endif

	mtv_sec_cb_ptr->max_remaining_tsp_cnt = 0;
}
...
static INLINE void show_msc_if_statistics(void)
{
	...
	DMBMSG("MSC1[ovf: %ld/%ld], MSC0[ovf: %ld/%ld], # remaining tsp: %u\n",
		mtv_sec_cb_ptr->msc1_ovf_intr_cnt, mtv_sec_cb_ptr->msc1_ts_intr_cnt,
		mtv_sec_cb_ptr->msc0_ovf_intr_cnt, mtv_sec_cb_ptr->msc0_ts_intr_cnt,
		mtv_sec_cb_ptr->max_remaining_tsp_cnt);
	#else
	DMBMSG("MSC1[ovf: %ld/%ld], MSC0[ovf: %ld/%ld, cife: %ld], # remaining tsp: %u\n",
		mtv_sec_cb_ptr->msc1_ovf_intr_cnt, mtv_sec_cb_ptr->msc1_ts_intr_cnt,
		mtv_sec_cb_ptr->msc0_ovf_intr_cnt, mtv_sec_cb_ptr->msc0_ts_intr_cnt,
		mtv_sec_cb_ptr->msc0_cife_cnt, mtv_sec_cb_ptr->max_remaining_tsp_cnt);
	#endif
...
static INLINE void tdmb_dab_reset_tsp_queue(void)
{
	...
	if (atomic_read(&mtv_sec_cb_ptr->num_opened_subch) == 0) {
		if(mtv_sec_cb_ptr->fic_opened == FALSE) /* FIC was closed. */
			mtv_reset_tsp_queue(); /* All reset. */
		else /* Clear MSC contents only */
			mtv_clear_contents_only_in_tsp_queue(1);
	}
	else { /* MSC was opened yet */
		if (mtv_sec_cb_ptr->fic_opened == FALSE) /* Clear FIC contents only */
...
static INLINE void dab_close_all_subchannels(void)
{
	...
	atomic_set(&mtv_sec_cb_ptr->num_opened_subch, 0);
#if defined(NXTV_IF_SPI)
...
static INLINE int dab_close_subchannel(unsigned long arg)
{
	...
	if (ret == NXTV_SUCCESS) {
		if (atomic_read(&mtv_sec_cb_ptr->num_opened_subch) != 0)
			atomic_dec(&mtv_sec_cb_ptr->num_opened_subch);
	}
	else
		DMBERR("failed: %d\n", ret);
...
static INLINE int dab_open_subchannel(unsigned long arg)
{
	...
	case NXTV_SERVICE_VIDEO:
		mtv_sec_cb_ptr->av_subch_id = subch_id;
	...
	#ifndef NXTV_CIF_MODE_ENABLED
		mtv_sec_cb_ptr->data_subch_id = subch_id;
	#endif	
	...
	mtv_sec_cb_ptr->msc1_thres_size = thres_size;
#else
	if (svc_type & (NXTV_SERVICE_VIDEO | NXTV_SERVICE_AUDIO))
		mtv_sec_cb_ptr->msc1_thres_size = thres_size;
	else
		mtv_sec_cb_ptr->msc0_thres_size = thres_size;
#endif

	ret = nxtvDAB_OpenSubChannel(ch_freq_khz, subch_id, svc_type, thres_size);
	if (ret == NXTV_SUCCESS) {
		if (atomic_read(&mtv_sec_cb_ptr->num_opened_subch) == 0) { /* The first open */
		#if defined(NXTV_IF_SPI)
		...
		atomic_inc(&mtv_sec_cb_ptr->num_opened_subch);
	}
	else {
...
static INLINE void dab_close_fic(void)
{
	...
	mtv_sec_cb_ptr->fic_opened = FALSE;
	mtv_sec_cb_ptr->fic_size = 0; /* Reset */

#ifdef NXTV_FIC_SPI_INTR_ENABLED
	mtv_sec_cb_ptr->wakeup_tspq_thres_cnt = MTV_SPI_WAKEUP_TSP_Q_CNT;
	tdmb_dab_reset_tsp_queue();
#endif
}
...
static INLINE int dab_open_fic(unsigned long arg)
{
	...
	ret = nxtvDAB_OpenFIC();
	if (ret == NXTV_SUCCESS) {
		mtv_sec_cb_ptr->fic_opened = TRUE;
		...
		*/
		...
		/* Allow thread to read TSP data on read() */
		mtv_sec_cb_ptr->read_stop = FALSE;
		mtv_sec_cb_ptr->wakeup_tspq_thres_cnt = 1; /* For FIC parsing in time. */	
#endif
...
static INLINE int dab_init(unsigned long arg)
{
	...
	mtv_sec_cb_ptr->tv_mode = DMB_TV_MODE_DAB;
	atomic_set(&mtv_sec_cb_ptr->num_opened_subch, 0); /* NOT opened state */

	mtv_sec_cb_ptr->fic_opened = FALSE;
	mtv_sec_cb_ptr->fic_size = 0;
...
static INLINE int tdmb_get_lock_status(unsigned long arg)
{
	...
	lock_mask = nxtvTDMB_GetLockStatus_SEC();
...
static INLINE int tdmb_get_signal_info(unsigned long arg)
{
	...
	sig.lock_mask = nxtvTDMB_GetLockStatus_SEC();	
	sig.ber = nxtvTDMB_GetBER_SEC(); 	 
	sig.cnr = nxtvTDMB_GetCNR_SEC(); 
	sig.per = nxtvTDMB_GetPER_SEC(); 
	sig.rssi = nxtvTDMB_GetRSSI_SEC();
	sig.cer = nxtvTDMB_GetCER_SEC();
	sig.ant_level = nxtvTDMB_GetAntennaLevel_SEC(sig.cer);
...
{
	nxtvTDMB_CloseAllSubChannels_SEC();
...
static INLINE int tdmb_close_subchannel(unsigned long arg)
{
	...
	ret = nxtvTDMB_CloseSubChannel_SEC(subch_id);
	if (ret == NXTV_SUCCESS) {
		if (atomic_read(&mtv_sec_cb_ptr->num_opened_subch) != 0)
			atomic_dec(&mtv_sec_cb_ptr->num_opened_subch);
	}
...
static INLINE int tdmb_open_subchannel(unsigned long arg)
{
	...
	ret = nxtvTDMB_OpenSubChannel_SEC(ch_freq_khz, subch_id, svc_type, thres_size);
	if (ret == NXTV_SUCCESS) {
		if (atomic_read(&mtv_sec_cb_ptr->num_opened_subch) == 0) {/* The first open */
	#if defined(NXTV_IF_SPI)
			/* Allow thread to read TSP data on read() */
...
static INLINE int tdmb_scan_freq(unsigned long arg)
{
	...
	ret = nxtvTDMB_ScanFrequency_SEC(ch_freq_khz);
	if (ret == NXTV_SUCCESS)
		return 0;
	else {
		if(ret == NXTV_CHANNEL_NOT_DETECTED)
...
static INLINE int tdmb_read_fic(unsigned long arg)
{
	...
	ret = nxtvTDMB_ReadFIC_SEC(fic_buf);
	if (ret > 0)	{
#if defined(NXTV_IF_SPI)
		if (copy_to_user((void __user*)argp->buf, &fic_buf[1], ret))
#else
...
static INLINE int tdmb_init(unsigned long arg)
{
	...
	mtv_sec_cb_ptr->tv_mode = DMB_TV_MODE_TDMB;
	atomic_set(&mtv_sec_cb_ptr->num_opened_subch, 0); /* NOT opened state */
	mtv_sec_cb_ptr->fic_opened = FALSE;
	...
	ret = nxtvTDMB_Initialize_SEC(NXTV_COUNTRY_BAND_KOREA);
	if (ret == NXTV_SUCCESS)
		return 0;
	else {
		DMBERR("Tuner initialization failed: %d\n", ret);
...
static INLINE int isdbt_init(unsigned long arg)
{
	...
	mtv_sec_cb_ptr->tv_mode = DMB_TV_MODE_1SEG;
	mtv_sec_cb_ptr->msc1_thres_size = MTV_TS_THRESHOLD_SIZE;
#if defined(NXTV_IF_SPI)
	mtv_sec_cb_ptr->wakeup_tspq_thres_cnt = MTV_SPI_WAKEUP_TSP_Q_CNT;
#endif
...
static INLINE int fm_init(unsigned long arg)
{
	...
	mtv_sec_cb_ptr->tv_mode = DMB_TV_MODE_FM;
	mtv_sec_cb_ptr->msc1_thres_size = MTV_TS_THRESHOLD_SIZE;
#if defined(NXTV_IF_SPI)
	mtv_sec_cb_ptr->wakeup_tspq_thres_cnt = MTV_SPI_WAKEUP_TSP_Q_CNT;
#endif
