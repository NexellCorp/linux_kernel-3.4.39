...
#define DMBERR(fmt, args...) \
	do { printk(KERN_ERR "TDMB_sec: %s(): " fmt, __func__, ## args); } while (0)

#ifdef DMB_DEBUG
	#define DMBMSG(fmt, args...) \
		printk(KERN_INFO "TDMB_sec: %s(): " fmt, __func__, ## args)
#else 
...
struct mtv_cb
{
	...
	and mtv_free_tsp_sec(mtv_cb_ptr->prev_tsp) on ioctl:mtv_reset_tsp_queue(). */
...
extern struct mtv_cb *mtv_sec_cb_ptr;
#if defined(NXTV_IF_TSIF) || defined(NXTV_IF_SPI_SLAVE)
	extern struct i2c_driver mtv_i2c_driver;
#elif defined(NXTV_IF_SPI)
	extern struct spi_driver mtv_spi_driver_1;
#endif
...
void mtv_put_tsp_sec(MTV_TS_PKT_INFO *pkt);
void mtv_free_tsp_sec(MTV_TS_PKT_INFO *pkt);
