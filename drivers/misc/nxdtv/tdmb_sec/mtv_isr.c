...
//		DMBMSG("FIC_E_INT occured!\n");
		...
		kill_fasync(&mtv_sec_cb_ptr->fasync, SIGIO, POLL_IN);
...
static INLINE void read_fic(MTV_TS_PKT_INFO *tsp)
{
	...
	unsigned int size = mtv_sec_cb_ptr->fic_size;
	...
	{	/* This case can be occured at weak signal area. */
		...
		mtv_sec_cb_ptr->fic_size = size; /* Update FIC size. */
	}
#endif
...
{
	unsigned int size = mtv_sec_cb_ptr->msc0_thres_size;
...
#ifdef DEBUG_MTV_IF_MEMORY
	mtv_sec_cb_ptr->msc0_ts_intr_cnt++;
#endif
...
static INLINE MTV_TS_PKT_INFO *proc_msc0(MTV_TS_PKT_INFO *tsp, U8 istatus)
{
	if (istatus & MSC0_INTR_BITS) {
		...
		#ifdef DEBUG_MTV_IF_MEMORY
			mtv_sec_cb_ptr->msc0_ovf_intr_cnt++;
		#endif	
		}
		else {
...
{
	unsigned int size = mtv_sec_cb_ptr->msc1_thres_size;

//printk("isr: mtv_sec_cb_ptr->msc1_thres_size(%u)\n", mtv_sec_cb_ptr->msc1_thres_size);
...
#ifdef DEBUG_MTV_IF_MEMORY
	mtv_sec_cb_ptr->msc1_ts_intr_cnt++;
#endif
...
static INLINE MTV_TS_PKT_INFO *proc_msc1(MTV_TS_PKT_INFO *tsp, U8 istatus)
{
	if (istatus & MSC1_INTR_BITS) {
		if (istatus & (MSC1_E_UNDER_FLOW|MSC1_E_OVER_FLOW)) {
			nxtv_ClearAndSetupMemory_MSC1(mtv_sec_cb_ptr->tv_mode);
...
#ifdef DEBUG_MTV_IF_MEMORY
			mtv_sec_cb_ptr->msc1_ovf_intr_cnt++;
#endif
		}
		else {
			/* Allocate a TS packet from TSP pool. */
...
__isr_thread_cnt++;
	...
	if (tsp != NULL) {
		...
		if ((mtv_sec_cb_ptr->f_flags & O_NONBLOCK) == 0) {
			if (!mtv_sec_cb_ptr->first_interrupt) {
				if ((get_tsp_queue_count_sec() >= mtv_sec_cb_ptr->wakeup_tspq_thres_cnt))
					wake_up_interruptible(&mtv_sec_cb_ptr->read_wq);
			} else {
				mtv_sec_cb_ptr->first_interrupt = false;
				wake_up_interruptible(&mtv_sec_cb_ptr->read_wq);
			}
		}			
	}
...
int mtv_isr_thread_tdmb_secondary(void *data)
{
	DMBMSG("Start 0 ...\n");
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(mtv_sec_cb_ptr->isr_wq,
				kthread_should_stop() || mtv_sec_cb_ptr->isr_cnt);
...
irqreturn_t mtv_isr_tdmb_secondary(int irq, void *param)
{
	if(mtv_sec_cb_ptr->is_power_on == TRUE) {
	__isr_cnt++;
		mtv_sec_cb_ptr->isr_cnt++;
		wake_up_interruptible(&mtv_sec_cb_ptr->isr_wq);
	}
