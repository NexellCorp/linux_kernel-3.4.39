...
extern volatile E_NXTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType_SEC;
extern E_NXTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType_SEC;
...
extern     BOOL g_fDabEnableReconfigureIntr;
	...
	extern BOOL g_fRtvFicOpened_SEC;
...
extern volatile BOOL g_fRtvChannelChange_SEC;
extern BOOL g_fRtvStreamEnabled_SEC;

extern UINT g_nRtvMscThresholdSize_SEC;	
extern U8 g_bRtvIntrMaskRegL_SEC;
#ifdef NXTV_DAB_ENABLE
...
{	
	...
	if(g_fRtvStreamEnabled_SEC == FALSE)
		return;

#if defined(NXTV_IF_SPI) || defined(NXTV_IF_EBI2)
	NXTV_REG_MAP_SEL(HOST_PAGE);
	NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL_SEC|MSC0_INTR_BITS|MSC1_INTR_BITS);
...
{
#ifndef NXTV_IF_MPEG2_PARALLEL_TSIF
	if(g_fRtvStreamEnabled_SEC == TRUE)
		return;
...
#endif
	nxtv_SetupThreshold_MSC1(g_nRtvMscThresholdSize_SEC);
...
static INLINE void nxtv_DisableFicInterrupt(void)
{
	...
	NXTV_REG_MAP_SEL(HOST_PAGE);
	g_bRtvIntrMaskRegL_SEC |= FIC_E_INT;
	NXTV_REG_SET(0x62, g_bRtvIntrMaskRegL_SEC);     
#endif
}
 ...
 *============================================================================*/ 
void nxtv_ConfigureHostIF_SEC(void);
INT  nxtv_InitSystem_SEC(E_NXTV_TV_MODE_TYPE eTvMode, E_NXTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType);
