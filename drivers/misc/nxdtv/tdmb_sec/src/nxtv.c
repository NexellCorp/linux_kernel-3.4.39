...
volatile BOOL g_fRtvChannelChange_SEC;


volatile E_NXTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType_SEC;
BOOL g_fRtvStreamEnabled_SEC;

#if defined(NXTV_TDMB_ENABLE) || defined(NXTV_ISDBT_ENABLE)
	E_NXTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType_SEC;
#endif

#if defined(NXTV_TDMB_ENABLE) || defined(NXTV_DAB_ENABLE)
	BOOL g_fRtvFicOpened_SEC;
...
UINT g_nRtvMscThresholdSize_SEC; 
U8 g_bRtvIntrMaskRegL_SEC;
#ifdef NXTV_DAB_ENABLE
...
INT nxtv_InitSystem_SEC(E_NXTV_TV_MODE_TYPE eTvMode, E_NXTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	...
	for(i=1; i<=100; i++)
	{
		...
		NXTV_DBGMSG1("[nxtv_InitSystem_SEC] Power On wait: %d\n", i);

		NXTV_DELAY_MS(5);
	}

	NXTV_DBGMSG1("nxtv_InitSystem_SEC: Power On Check error: %d\n", i);
	return NXTV_POWER_ON_CHECK_ERROR;

NXTV_POWER_ON_SUCCESS:
	
	nxtvRF_ConfigurePowerType_SEC(eTvMode);

	if((nRet=nxtvRF_ConfigureAdcClock_SEC(eTvMode, eAdcClkFreqType)) != NXTV_SUCCESS)
		return nRet;
