...
static UINT __isr_cnt[RAONTV_MAX_NUM_DEMOD_CHIP];
static UINT __isr_thread_cnt[RAONTV_MAX_NUM_DEMOD_CHIP];
	...
	if (tsp != NULL) {
		...
		if ((mtv_cb_ptr->f_flags & O_NONBLOCK) == 0) {
			if (!mtv_cb_ptr->first_interrupt) {
				if ((get_tsp_queue_count(demod_no) >= mtv_cb_ptr->wakeup_tspq_thres_cnt))
...
int mtv_isr_thread_0(void *data)
{
...
irqreturn_t mtv_isr_0(int irq, void *param)
{
	struct mtv_cb * mtv_cb_ptr = mtv_cb_ptrs[0];
...
int mtv_isr_thread_1(void *data)
{
...
irqreturn_t mtv_isr_1(int irq, void *param)
{
	struct mtv_cb * mtv_cb_ptr = mtv_cb_ptrs[1];
