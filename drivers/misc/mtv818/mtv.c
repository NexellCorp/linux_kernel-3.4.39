...
struct mtv_cb *mtv_cb_ptrs[RAONTV_MAX_NUM_DEMOD_CHIP];

#if defined(RTV_IF_SPI) || defined(RTV_FIC_I2C_INTR_ENABLED)
extern irqreturn_t mtv_isr_0(int irq, void *param);
#if (RAONTV_MAX_NUM_DEMOD_CHIP == 2)
extern irqreturn_t mtv_isr_1(int irq, void *param);
#endif

#if 1
extern int mtv_isr_thread_0(void *data);
#if (RAONTV_MAX_NUM_DEMOD_CHIP == 2)
extern int mtv_isr_thread_1(void *data);
#endif
...
static MTV_TSP_QUEUE_INFO mtv_tsp_pool[RAONTV_MAX_NUM_DEMOD_CHIP];
static MTV_TSP_QUEUE_INFO mtv_tsp_queue[RAONTV_MAX_NUM_DEMOD_CHIP];

unsigned int get_tsp_queue_count(int demod_no)
{
	return mtv_tsp_queue[demod_no].cnt;
}
...
static void mtv_reset_tsp_queue(struct mtv_cb *mtv_cb_ptr)
{
	...
	if (mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt != MAX_NUM_TS_PKT_BUF) {
		...
		if (mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt != MAX_NUM_TS_PKT_BUF)
			WARN(1, KERN_ERR "[mtv_reset_tsp_queue] Abnormal cnt! (%u/%u)\n",
						mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt, MAX_NUM_TS_PKT_BUF);
	}
...
MTV_TS_PKT_INFO * mtv_peek_tsp(int demod_no)
{
	...
	struct list_head *head_ptr = &mtv_tsp_queue[demod_no].head;

	spin_lock(&mtv_tsp_queue[demod_no].lock);
	
	if (!list_empty(head_ptr))
		tsp = list_first_entry(head_ptr, MTV_TS_PKT_INFO, link);

	spin_unlock(&mtv_tsp_queue[demod_no].lock);
...
void mtv_put_tsp(int demod_no, MTV_TS_PKT_INFO *tsp)
{
	...
	list_add_tail(&tsp->link, &mtv_tsp_queue[demod_no].head);
	mtv_tsp_queue[demod_no].cnt++;
...
{	
	...
	spin_lock(&mtv_tsp_pool[demod_no].lock);
	...
	list_add_tail(&tsp->link, &mtv_tsp_pool[demod_no].head);
	mtv_tsp_pool[demod_no].cnt++;

	spin_unlock(&mtv_tsp_pool[demod_no].lock);
}
...
{	
	MTV_TS_PKT_INFO *tsp = NULL;
	struct list_head *head_ptr = &mtv_tsp_pool[mtv_cb_ptr->demod_no].head;

	spin_lock(&mtv_tsp_pool[mtv_cb_ptr->demod_no].lock);

	if (!list_empty(head_ptr)) {
		tsp = list_first_entry(head_ptr, MTV_TS_PKT_INFO, link);
		list_del(&tsp->link);
		mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt--;

#ifdef DEBUG_MTV_IF_MEMORY
		mtv_cb_ptr->max_alloc_tsp_cnt 
			= MAX(mtv_cb_ptr->max_alloc_tsp_cnt,
				MAX_NUM_TS_PKT_BUF - mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt);
#endif	
	}

	spin_unlock(&mtv_tsp_pool[mtv_cb_ptr->demod_no].lock);
...
static int mtv_create_tsp_pool(struct mtv_cb *mtv_cb_ptr)
{
	...
	/* TSP queue */
	mtv_cb_ptr->prev_tsp = NULL;
	spin_lock_init(&mtv_tsp_queue[mtv_cb_ptr->demod_no].lock);
	INIT_LIST_HEAD(&mtv_tsp_queue[mtv_cb_ptr->demod_no].head);
	mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt = 0;
	
	spin_lock_init(&mtv_tsp_pool[mtv_cb_ptr->demod_no].lock);
	INIT_LIST_HEAD(&mtv_tsp_pool[mtv_cb_ptr->demod_no].head);

	mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt = 0;
#ifdef DEBUG_MTV_IF_MEMORY
	...
	for (i = 0; i < MAX_NUM_TS_PKT_BUF; i++) {
		...
		list_add_tail(&tsp->link, &mtv_tsp_pool[mtv_cb_ptr->demod_no].head);
		mtv_tsp_pool[mtv_cb_ptr->demod_no].cnt++;
	}
...
void mtv_clear_contents_only_in_tsp_queue(struct mtv_cb *mtv_cb_ptr, int fic_msc_type)
{
	...
	spin_lock(&mtv_tsp_queue[mtv_cb_ptr->demod_no].lock); /* for IRQ */

	list_for_each (pos, &mtv_tsp_queue[mtv_cb_ptr->demod_no].head) { /* Lookup tsq Q */
	...
	spin_unlock(&mtv_tsp_queue[mtv_cb_ptr->demod_no].lock);
...
{		
	...
	if (mtv_cb_ptr->f_flags & O_NONBLOCK) { /* Non-blocking mode. */
		if (mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt == 0)
			return -EAGAIN;
	}
	else { /* The file is opened in blocking mode. */
		if (wait_event_interruptible(mtv_cb_ptr->read_wq,
			mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt || (mtv_cb_ptr->read_stop == TRUE))) {
...
unlock_read:
	...
	if ((mtv_cb_ptr->f_flags & O_NONBLOCK) == 0) {
		if (mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt) /* for next time */
...
exit_read:
	...
	mtv_cb_ptr->max_remaining_tsp_cnt
		= MAX(mtv_cb_ptr->max_remaining_tsp_cnt, mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt);
#endif
...
static inline ssize_t read_single_service(struct mtv_cb * mtv_cb_ptr, char *buf, size_t cnt)
{
	...
	if (mtv_cb_ptr->f_flags & O_NONBLOCK) { /* non-blocking mode. */
		unsigned int have_tsp_bytes
			= mtv_tsp_queue[mtv_cb_ptr->demod_no].cnt * mtv_cb_ptr->msc1_thres_size;
...
static ssize_t mtv_read(struct file *filp, char *buf, size_t count, loff_t *pos)
{
	...
	mtv_cb_ptr = mtv_cb_ptrs[demod_no];
		...
		init_waitqueue_head(&mtv_cb_ptr->isr_wq);
 		...
 		if (demod_no == 0)
			mtv_cb_ptr->isr_thread_cb
				= kthread_run(mtv_isr_thread_0, NULL, "mtv_isr_thread_0");

	#if (RAONTV_MAX_NUM_DEMOD_CHIP == 2)
		if (demod_no == 1)
			mtv_cb_ptr->isr_thread_cb
				= kthread_run(mtv_isr_thread_1, NULL, "mtv_isr_thread_1");
	#endif
...
static int mtv_close(struct inode *inode, struct file *filp)
{
	...
	for (demod_no = 0; demod_no < RAONTV_MAX_NUM_DEMOD_CHIP; demod_no++) {
		mtv_cb_ptr = mtv_cb_ptrs[demod_no];
...
static void mtv_deinit_device(void)
{
	...
	for (i = 0; i < RAONTV_MAX_NUM_DEMOD_CHIP; i++) {
		mtv_cb_ptr = mtv_cb_ptrs[i];
...
static int mtv_init_device(void)
{
	...
	for (i = 0; i < RAONTV_MAX_NUM_DEMOD_CHIP; i++) {
		...
		mtv_cb_ptr->demod_no = i;

		mtv_cb_ptrs[i] = mtv_cb_ptr;

		//DMBMSG("mtv_cb_ptr(0x%p), mtv_cb_ptrs[%d](0x%p)\n", mtv_cb_ptr, i, mtv_cb_ptrs[i]);
		...
		case 1:
			if ((ret=spi_register_driver(&mtv_spi_driver_1)) < 0) {
				DMBERR("RAONTV SPI1 driver register failed\n");
				goto err;
			}
			break;
	#endif
...
#if defined(RTV_IF_SPI) || defined(RTV_FIC_I2C_INTR_ENABLED)
		if (i == 0) {
			mtv_cb_ptr->irq = gpio_to_irq(RAONTV_IRQ_INT); ///!!!! 
			ret = request_irq(mtv_cb_ptr->irq, mtv_isr_0,
							IRQ_TYPE_EDGE_FALLING, "mtv_isr_0", NULL);
			if (ret != 0) {
		...
		if (i == 1) {
			...
			ret = request_irq(mtv_cb_ptr->irq, mtv_isr_1,
							IRQ_TYPE_EDGE_FALLING, "mtv_isr_1", NULL);
			if (ret != 0) {
				DMBERR("Failed to install irq (%d)\n", ret);
				goto err;
			}		
