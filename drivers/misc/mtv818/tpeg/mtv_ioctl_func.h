...
{
	if (tpeg_get_tsp_queue_count(mtv_cb_ptr->demod_no) == 0)
...
static INLINE int tdmb_get_lock_status(struct mtv_cb *mtv_cb_ptr, unsigned long arg)
{
	...
	lock_mask = rtvTDMB_GetLockStatus_tpeg(mtv_cb_ptr->demod_no);
...
static INLINE int tdmb_get_signal_info(struct mtv_cb *mtv_cb_ptr, unsigned long arg)
{
	...
	sig.lock_mask = rtvTDMB_GetLockStatus_tpeg(mtv_cb_ptr->demod_no);	
	sig.ber = rtvTDMB_GetBER_tpeg(mtv_cb_ptr->demod_no); 	 
	sig.cnr = rtvTDMB_GetCNR_tpeg(mtv_cb_ptr->demod_no); 
	sig.per = rtvTDMB_GetPER_tpeg(mtv_cb_ptr->demod_no); 
	sig.rssi = rtvTDMB_GetRSSI_tpeg(mtv_cb_ptr->demod_no);
	sig.cer = rtvTDMB_GetCER_tpeg(mtv_cb_ptr->demod_no);
	sig.ant_level = rtvTDMB_GetAntennaLevel_tpeg(mtv_cb_ptr->demod_no, sig.cer);
...
{
	rtvTDMB_CloseAllSubChannels_tpeg(mtv_cb_ptr->demod_no);
...
static INLINE int tdmb_close_subchannel(struct mtv_cb * mtv_cb_ptr, unsigned int subch_id)
{
	...
	ret = rtvTDMB_CloseSubChannel_tpeg(mtv_cb_ptr->demod_no, subch_id);
	if (ret == RTV_SUCCESS) {
		if (atomic_read(&mtv_cb_ptr->num_opened_subch) != 0)
			atomic_dec(&mtv_cb_ptr->num_opened_subch);
	}
...
static INLINE int tdmb_open_subchannel(struct mtv_cb * mtv_cb_ptr, unsigned long arg)
{
	...
	ret = rtvTDMB_OpenSubChannel_tpeg(mtv_cb_ptr->demod_no, ch_freq_khz, subch_id, svc_type, thres_size);
	if (ret == RTV_SUCCESS) {
		if (atomic_read(&mtv_cb_ptr->num_opened_subch) == 0) {/* The first open */
	#if defined(RTV_IF_SPI)
			/* Allow thread to read TSP data on read() */
...
static INLINE int tdmb_scan_freq(struct mtv_cb * mtv_cb_ptr, unsigned long arg)
{
	...
	ret = rtvTDMB_ScanFrequency_tpeg(mtv_cb_ptr->demod_no, ch_freq_khz);
	if (ret == RTV_SUCCESS)
		return 0;
	else {
		if(ret == RTV_CHANNEL_NOT_DETECTED)
...
static INLINE int tdmb_read_fic(struct mtv_cb * mtv_cb_ptr, unsigned long arg)
{
	...
	ret = rtvTDMB_ReadFIC_tpeg(mtv_cb_ptr->demod_no, fic_buf);
	if (ret > 0)	{
#if defined(RTV_IF_SPI)
		if (copy_to_user((void __user*)argp->buf, &fic_buf[1], ret))
#else
...
{
	rtvTDMB_CloseFIC_tpeg(mtv_cb_ptr->demod_no);
...
static INLINE int tdmb_open_fic(struct mtv_cb *mtv_cb_ptr, unsigned long arg)
{
	...
	ret = rtvTDMB_OpenFIC_tpeg(mtv_cb_ptr->demod_no);
	if (ret == RTV_SUCCESS) {
		mtv_cb_ptr->fic_opened = TRUE;
...
static INLINE int tdmb_init(struct mtv_cb * mtv_cb_ptr, unsigned long arg)
{
	...
	ret = rtvTDMB_Initialize_tpeg(mtv_cb_ptr->demod_no, RTV_COUNTRY_BAND_KOREA);
	if (ret == RTV_SUCCESS)
		return 0;
	else {
		DMBERR("Tuner initialization failed: %d\n", ret);
