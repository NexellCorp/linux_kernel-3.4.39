 ...
 * RETURN VALUE : none.
 *============================================================================*/
void rtvRF_ConfigurePowerType_tpeg(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	RTV_REG_SET(demod_no, 0x54, REG54);
	RTV_REG_SET(demod_no, 0x52, REG52);
	RTV_REG_SET(demod_no, 0x30, REG30);
	RTV_REG_SET(demod_no, 0x2F, REG2F);	
}
...
INT rtvRF_ConfigureAdcClock_tpeg(int demod_no, E_RTV_TV_MODE_TYPE eTvMode, E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	U8 REGE9,RD15;
	INT i;
	...
	{
	        RTV_DBGMSG1("[rtvRF_ConfigureAdcClock_tpeg] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
	...
	RTV_REG_MAP_SEL(demod_no, RF_PAGE);
	RTV_REG_SET(demod_no, 0xE8, (REGE8 | pbAdcClkSynTbl[0]));
	RTV_REG_SET(demod_no, 0xE9, (REGE9 | pbAdcClkSynTbl[1]));				
	RTV_REG_SET(demod_no, 0xEA, (REGEA | pbAdcClkSynTbl[2]));				
	RTV_REG_SET(demod_no, 0xEB, (REGEB | pbAdcClkSynTbl[3]));				
	RTV_REG_SET(demod_no, 0xEC, (REGEC | pbAdcClkSynTbl[4]));				
	RTV_REG_SET(demod_no, 0xED, (REGED | pbAdcClkSynTbl[5]));				
	RTV_REG_SET(demod_no, 0xEE, (REGEE | pbAdcClkSynTbl[6]));	
	
	for(i=0; i<10;i++)
	{
		RD15 = RTV_REG_GET(demod_no, 0x15) & 0x01; 
		if(RD15) 
		{
			 break;
		}
		...
		{
			RTV_DBGMSG0("[rtvRF_ConfigureAdcClock_tpeg] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	if(i == 10)
	{
		RTV_DBGMSG0("[rtvRF_ConfigureAdcClock_tpeg] ADC clock unlocked!Check the power supply.\n");
	...
	if(eTvMode == RTV_TV_MODE_1SEG)
	{
		...
		RTV_REG_SET(demod_no, 0x1A, 0x8B);
		RTV_REG_SET(demod_no, 0x18, 0xC0);
		RTV_REG_SET(demod_no, 0x19, 0x03);
		RTV_REG_SET(demod_no, 0x07, 0xF0); 
	}
#endif

	
	// Save the ADC clock type.
	g_eRtvAdcClkFreqType_tpeg[demod_no] = eAdcClkFreqType;

	//RTV_DBGMSG1("[rtvRF_ConfigureAdcClock_tpeg] ADC clk Type: %d\n", g_eRtvAdcClkFreqType_tpeg[RaonTvChipIdx]);

	return RTV_SUCCESS;
}


INT rtvRF_ChangeAdcClock_tpeg(int demod_no, E_RTV_TV_MODE_TYPE eTvMode, E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{	
	U8 REGE9, RD15;
#ifdef RTV_PWR_DCDC	
	U8 RD2F;
	...
	if(eAdcClkFreqType == g_eRtvAdcClkFreqType_tpeg[demod_no])
		return RTV_SUCCESS;
		
	if(pbAdcClkSynTbl[0] == 0xFF)
    {
		RTV_DBGMSG1("[rtvRF_ChangeAdcClock_tpeg] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
		return RTV_UNSUPPORT_ADC_CLK;
    }
...
#ifdef RTV_PWR_DCDC
	RD2F = RTV_REG_GET(demod_no, 0x2F);     
	RTV_REG_SET(demod_no, 0x2F, (RD2F & 0xF7));
#endif	
	...
	for(i=0; i<10;i++)
	{
		...
		{
			RTV_DBGMSG0("[rtvRF_ChangeAdcClock_tpeg] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	if(i == 10)
	{
		RTV_DBGMSG0("[rtvRF_ChangeAdcClock_tpeg] ADC clock unlocked!Check the power supply.\n");
		return RTV_ADC_CLK_UNLOCKED;
	}
		...
		case RTV_ADC_CLK_FREQ_8_MHz:     
            switch( eTvMode )
            {
				...
				case RTV_TV_MODE_DAB_L :
					...
					RTV_REG_SET(demod_no, 0x40,0x8F); //PNCO
					RTV_REG_SET(demod_no, 0x41,0xC2); //PNCO
					RTV_REG_SET(demod_no, 0x42,0xF5); //PNCO
					RTV_REG_SET(demod_no, 0x43,0x00); //PNCO
                    break;
#endif	
                default: break;	
	...
	case RTV_TV_MODE_DAB_L :
					...
					RTV_REG_SET(demod_no, 0x40,0x00); //PNCO
					RTV_REG_SET(demod_no, 0x41,0x00); //PNCO
					RTV_REG_SET(demod_no, 0x42,0xF0); //PNCO
					RTV_REG_SET(demod_no, 0x43,0x00); //PNCO
#endif
                default: break;
            } /* End of switch(eTvMode) */
        ...
        case RTV_ADC_CLK_FREQ_9_MHz:	
            switch( eTvMode )
            {
		      ...
		      RTV_REG_SET(demod_no, 0x54, 0x58); 
					...
					RTV_REG_SET(demod_no, 0x40,0x0D); //PNCO
					RTV_REG_SET(demod_no, 0x41,0x74); //PNCO
					RTV_REG_SET(demod_no, 0x42,0xDA); //PNCO
					RTV_REG_SET(demod_no, 0x43,0x00); //PNCO
                    break;
#endif
                default: break;
		...
		case RTV_ADC_CLK_FREQ_9_6_MHz:	
            switch( eTvMode )
            {
		      ...
		      RTV_REG_SET(demod_no, 0x54, 0x58); 
					...
					RTV_REG_SET(demod_no, 0x40,0xCC); //PNCO
					RTV_REG_SET(demod_no, 0x41,0xCC); //PNCO
					RTV_REG_SET(demod_no, 0x42,0xCC); //PNCO
					RTV_REG_SET(demod_no, 0x43,0x00); //PNCO
                    break;
#endif			
                default: break;
	...
	if(RD15) 
	{
		 RTV_REG_SET(demod_no, 0x2F, RD2F);
	}
	else
	{
		RTV_DBGMSG0("[rtvRF_ChangeAdcClock_tpeg] DCDC CLOCK SYNTH  UnLock..\n");
	}
#endif
...
//	RTV_DBGMSG1("[rtvRF_ChangeAdcClock_tpeg] ADC clk Type: %d\n", g_eRtvAdcClkFreqType_tpeg[RaonTvChipIdx]);

	return RTV_SUCCESS;	
}


INT rtvRF_SetFrequency_tpeg(int demod_no, E_RTV_TV_MODE_TYPE eTvMode, UINT nChNum, U32 dwChFreqKHz)
{
#if defined(RTV_TDMB_ENABLE) || defined(RTV_DAB_ENABLE)
	int nIdx;
#endif
	...
	g_fRtvChannelChange_tpeg[demod_no] = TRUE;
	...
	switch( eTvMode )
	{
#ifdef RTV_ISDBT_ENABLE
		case RTV_TV_MODE_1SEG :
			if((g_eRtvCountryBandType_tpeg[demod_no]==RTV_COUNTRY_BAND_BRAZIL) || (g_eRtvCountryBandType_tpeg[demod_no]==RTV_COUNTRY_BAND_ARGENTINA))		
				nChNum -= 1;
		...
		case RTV_TV_MODE_FM :							
			...
			eAdcClkFreqType = g_eRtvAdcClkFreqType_tpeg; /* Only init adc clock type. */
			break;
#endif
		...
		case RTV_TV_MODE_TDMB : 	
			...
			//if(g_eRtvCountryBandType_tpeg == RTV_COUNTRY_BAND_KOREA)
			{
				nIdx = (dwChFreqKHz - TDMB_CH_FREQ_START__KOREA) / TDMB_CH_FREQ_STEP__KOREA;
	...
	if((nRet=rtvRF_ChangeAdcClock_tpeg(demod_no, eTvMode, eAdcClkFreqType)) != RTV_SUCCESS)
		goto RF_SET_FREQ_EXIT;
		...
		case RTV_TV_MODE_1SEG :			
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, nChNum);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	RTV_DBGMSG3("[rtvRF_SetFrequency_tpeg] 1SEG PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
		...
		case RTV_TV_MODE_FM :		
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	RTV_DBGMSG3("[rtvRF_SetFrequency_tpeg] FM PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
...
#ifdef RTV_TDMB_ENABLE
		case RTV_TV_MODE_TDMB :	
            RTV_REG_SET(demod_no, 0x23, (dwPllNF>>22)&0xFF);
			RTV_REG_SET(demod_no, 0x24, (dwPllNF>>14)&0xFF);
			RTV_REG_SET(demod_no, 0x25, (dwPllNF>>6)&0xFF);
			RTV_REG_SET(demod_no, 0x26, (((dwPllNF&0x0000003F)<<2)) | 0x00);	
			RTV_DELAY_MS(1);  //1ms Delay	
			RTV_REG_SET(demod_no, 0x20, 0x00);
                        
            RTV_DELAY_MS(2);	//2ms Delay 	 
			wr_dwPllNF_Verify_val = dwPllNF>>1;

			do{
				RD15 = RTV_REG_GET(demod_no, 0x15);
				rd_dwPllNF_Verify_val = (RTV_REG_GET(demod_no, 0x12) << 22) + (RTV_REG_GET(demod_no, 0x13) << 14) +(RTV_REG_GET(demod_no, 0x14) << 6) +(RD15 >> 2);

				//RTV_DBGMSG3("[rtvRF_SetFrequency_tpeg] TDMB PLL PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				...
				{ 
					RTV_REG_SET(demod_no, 0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
					RTV_DBGMSG3("[rtvRF_SetFrequency_tpeg] TDMB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);

			RTV_REG_MAP_SEL(demod_no, 0x06); // ofdm
		...
		case RTV_TV_MODE_TDMB:
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	RTV_DBGMSG3("[rtvRF_SetFrequency_tpeg] DAB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
	...
	if(PLL_Verify_cnt == 0)
	{
		RTV_DBGMSG0("[rtvRF_SetFrequency_tpeg] PLL unlocked!\n");
...
INT rtvRF_Initilize_tpeg(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	do
	{
		RTV_REG_SET(demod_no, ptInitTbl->bReg, ptInitTbl->bVal);
		ptInitTbl++;						
	} while( --nNumTblEntry );
	
	// Auto LNA
	RTV_REG_SET(demod_no, ptLNA[0].bReg, ptLNA[0].bVal);
	RTV_REG_SET(demod_no, ptLNA[1].bReg, ptLNA[1].bVal);
	...
	if(eTvMode == RTV_TV_MODE_1SEG)
	{
		do
		{		
			RTV_REG_MASK_SET(demod_no, ptAutoCh->bReg, ptAutoCh->bMask, ptAutoCh->bVal);
			ptAutoCh++;	
		} while(--nNumAutoInit );

		RTV_REG_SET(demod_no, 0x23,(dwAutoPllNF>>22)&0xFF);
		RTV_REG_SET(demod_no, 0x24,(dwAutoPllNF>>14)&0xFF);
		RTV_REG_SET(demod_no, 0x25,(dwAutoPllNF>>6)&0xFF);

		RTV_REG_SET(demod_no, 0x26,((dwAutoPllNF&0x0000003F) << 2) | eTvMode);

		RTV_REG_SET(demod_no, 0x62, (dwAutoPllNFSTEP >> 16) & 0xFF);
		RTV_REG_SET(demod_no, 0x63, (dwAutoPllNFSTEP >> 8 ) & 0xFF);
		RTV_REG_SET(demod_no, 0x64, (dwAutoPllNFSTEP & 0xFF));

#ifdef RAONTV_CHIP_PKG_WLCSP
       if(RTV_REG_GET(demod_no, 0x01) == 0x09)
       	{
       		RTV_REG_SET(demod_no, 0x21,0xF1);
       	}
#endif 
	}
...
#ifdef RTV_PWR_DCDC
	{
		U8 RD15 = RTV_REG_GET(demod_no, 0x15) & 0x01; 
		U8 RD2F = RTV_REG_GET(demod_no, 0x2F);       
		if(RD15)
		{
			RD2F |=0x08;
			RTV_REG_SET(demod_no, 0x2F, RD2F);
		...
		{
			RTV_DBGMSG0("[rtvRF_Initilize_tpeg] Clock Unlock \n");
		}
	}
#endif
//////////////////////////////CODE PATCH FOR OTP////////////////////////////////////
    if(((U16)(RTV_REG_GET(demod_no, 0x10) << 8) | (U16)(RTV_REG_GET(demod_no, 0x11))) != 0xFFFF)
    {
...
#ifdef RTV_TDMB_ENABLE
			case RTV_TV_MODE_TDMB :
				RTV_REG_SET(demod_no, 0x2C,0x48);
				RTV_REG_SET(demod_no, 0x47,0xC0);
				break;
#endif
		...
		RTV_REG_SET(demod_no, 0x2A,0x05);
		RTV_REG_SET(demod_no, 0x2D,0x8c);
		RTV_REG_SET(demod_no, 0x61,0x25);
    }
	else
    {
		switch( eTvMode )
...
#ifdef RTV_TDMB_ENABLE
			case RTV_TV_MODE_TDMB :
				RTV_REG_SET(demod_no, 0x2C,0xC8);
				RTV_REG_SET(demod_no, 0x47,0xB0);
				break;
#endif
		...
		RTV_REG_SET(demod_no, 0x2A,0x07);
		RTV_REG_SET(demod_no, 0x2D,0xec);
		RTV_REG_SET(demod_no, 0x61,0x2a);
    }
////////////////////////////////////////////////////////////////////////////////////	
