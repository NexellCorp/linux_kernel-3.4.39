...
#define RAONTV_MAX_NUM_DEMOD_CHIP	1
...
############################################################################*/
	...
	*================================================================*/
	...
	#define	RTV_REG_GET(demod_no, reg)            			(U8)mtv_spi_read_tpeg(demod_no, (U8)(reg))
	#define	RTV_REG_BURST_GET(demod_no, reg, buf, size) 	mtv_spi_read_burst_tpeg(demod_no, (U8)(reg), buf, (size))
	#define	RTV_REG_SET(demod_no, reg, val)       			mtv_spi_write_tpeg(demod_no, (U8)(reg), (U8)(val))       
...
############################################################################*/
	...
	#if defined(__KERNEL__)	
		extern struct mutex raontv_guard_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
		#define RTV_GUARD_INIT(demod_no)		mutex_init(&raontv_guard_tpeg[demod_no])
		#define RTV_GUARD_LOCK(demod_no)		mutex_lock(&raontv_guard_tpeg[demod_no])
		#define RTV_GUARD_FREE(demod_no)		mutex_unlock(&raontv_guard_tpeg[demod_no])
		#define RTV_GUARD_DEINIT(demod_no)		((void)0)
		
    #elif defined(WINCE)        
	        extern CRITICAL_SECTION		raontv_guard_tpeg[2];
	        #define RTV_GUARD_INIT(demod_no)	InitializeCriticalSection(&raontv_guard_tpeg[demod_no])
	        #define RTV_GUARD_LOCK(demod_no)	EnterCriticalSection(&raontv_guard_tpeg[demod_no])
	        #define RTV_GUARD_FREE(demod_no)	LeaveCriticalSection(&raontv_guard_tpeg[demod_no])
	        #define RTV_GUARD_DEINIT(demod_no)	DeleteCriticalSection(&raontv_guard_tpeg[demod_no])
	#else
		// temp: TODO
...
void rtvOEM_ConfigureInterrupt_tpeg(int demod_no);
void rtvOEM_PowerOn_tpeg(int demod_no, int on);
