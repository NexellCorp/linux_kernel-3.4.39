...
extern volatile E_RTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
extern E_RTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
...
extern     BOOL g_fDabEnableReconfigureIntr;
	...
	extern BOOL g_fRtvFicOpened_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
	#if defined(RTV_IF_SPI) && defined(RTV_CIF_MODE_ENABLED)
	extern U32 g_aOpenedCifSubChBits_MSC0_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP][2];
	#endif

	#if defined(RTV_IF_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE)
	extern E_RTV_FIC_OPENED_PATH_TYPE g_nRtvFicOpenedStatePath[RAONTV_MAX_NUM_DEMOD_CHIP];
	#endif
#endif


/* Use SPI/EBI2 interrupt handler to prevent the changing of register map. */
extern volatile BOOL g_fRtvChannelChange_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
extern BOOL g_fRtvStreamEnabled_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];

extern UINT g_nRtvMscThresholdSize_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];	
extern U8 g_bRtvIntrMaskRegL_tpeg[RAONTV_MAX_NUM_DEMOD_CHIP];
#ifdef RTV_DAB_ENABLE
	extern U8 g_bRtvIntrMaskRegH[RAONTV_MAX_NUM_DEMOD_CHIP];
#endif
...
{	
	...
	if(g_fRtvStreamEnabled_tpeg[demod_no] == FALSE)
		return;

#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL_tpeg[demod_no]|MSC0_INTR_BITS|MSC1_INTR_BITS);
...
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	if(g_fRtvStreamEnabled_tpeg[demod_no] == TRUE)
		return;
    ...
    g_fRtvStreamEnabled_tpeg[demod_no] = TRUE;  
#endif /* #ifndef RTV_IF_MPEG2_PARALLEL_TSIF */
}
...
static INLINE void rtv_StreamEnable(int demod_no)
{
	...
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	g_bRtvIntrMaskRegL_tpeg[demod_no] = 0x8F;
    RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL_tpeg[demod_no]);
#else
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x29, 0x00);
...
#endif
	rtv_SetupThreshold_MSC1(demod_no, g_nRtvMscThresholdSize_tpeg[demod_no]);
...
static INLINE void rtv_DisableFicInterrupt(int demod_no)
{
	...
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	g_bRtvIntrMaskRegL_tpeg[demod_no] |= FIC_E_INT;
...
static INLINE void rtv_EnableFicInterrupt(int demod_no)
{
    ...
    RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
    g_bRtvIntrMaskRegL_tpeg[demod_no] &= ~(FIC_E_INT);
    RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL_tpeg);
#endif
}
...
static INLINE void rtv_DeleteSpiCifSubChannelID_MSC0(int demod_no, unsigned int nSubChID)
{
	...
	if((g_aOpenedCifSubChBits_MSC0_tpeg[demod_no][nArrayIdx] & dwBitVal) != 0)
		g_aOpenedCifSubChBits_MSC0_tpeg[demod_no][nArrayIdx] &= ~dwBitVal; 
}
