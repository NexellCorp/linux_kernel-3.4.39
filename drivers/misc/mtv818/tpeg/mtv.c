...
#include "mtv_ioctl_func.h"
		...
		#include "./src/raontv_cif_dec.h"
	#endif
#endif


struct mtv_cb *mtv_cb_ptrs_tpeg;

#if defined(RTV_IF_SPI) || defined(RTV_FIC_I2C_INTR_ENABLED)
extern irqreturn_t mtv_isr_tpeg(int irq, void *param);


#if 1
extern int mtv_isr_thread_tpeg(void *data);
...
static MTV_TSP_QUEUE_INFO mtv_tsp_pool_tpeg;
static MTV_TSP_QUEUE_INFO mtv_tsp_queue_tpeg;

unsigned int tpeg_get_tsp_queue_count(int demod_no)
{
	return mtv_tsp_queue_tpeg.cnt;
}
...
static void mtv_reset_tsp_queue(struct mtv_cb *mtv_cb_ptr)
{
	...
	if (mtv_tsp_pool_tpeg.cnt != MAX_NUM_TS_PKT_BUF) {
		/* Free a previous TSP use in read(). */
		if (mtv_cb_ptr->prev_tsp) {
			tpeg_mtv_free_tsp(mtv_cb_ptr->demod_no, mtv_cb_ptr->prev_tsp);
			mtv_cb_ptr->prev_tsp = NULL; 
		}

		while ((tsp=tpeg_mtv_get_tsp(mtv_cb_ptr->demod_no)) != NULL) 
			tpeg_mtv_free_tsp(mtv_cb_ptr->demod_no, tsp);

		if (mtv_tsp_pool_tpeg.cnt != MAX_NUM_TS_PKT_BUF)
			WARN(1, KERN_ERR "[mtv_reset_tsp_queue] Abnormal cnt! (%u/%u)\n",
						mtv_tsp_pool_tpeg.cnt, MAX_NUM_TS_PKT_BUF);
	}
...
MTV_TS_PKT_INFO * tpeg_mtv_peek_tsp(int demod_no)
{
	MTV_TS_PKT_INFO *tsp = NULL;
	struct list_head *head_ptr = &mtv_tsp_queue_tpeg.head;

	spin_lock(&mtv_tsp_queue_tpeg.lock);
	
	if (!list_empty(head_ptr))
		tsp = list_first_entry(head_ptr, MTV_TS_PKT_INFO, link);

	spin_unlock(&mtv_tsp_queue_tpeg.lock);

	return tsp;
}

/* Dequeue a ts packet from ts data queue. */
MTV_TS_PKT_INFO * tpeg_mtv_get_tsp(int demod_no)
{
	MTV_TS_PKT_INFO *tsp = NULL;
	struct list_head *head_ptr = &mtv_tsp_queue_tpeg.head;
...
void tpeg_mtv_put_tsp(int demod_no, MTV_TS_PKT_INFO *tsp)
{
	spin_lock(&mtv_tsp_queue_tpeg.lock);

	list_add_tail(&tsp->link, &mtv_tsp_queue_tpeg.head);
	mtv_tsp_queue_tpeg.cnt++;

	spin_unlock(&mtv_tsp_queue_tpeg.lock);
}

void tpeg_mtv_free_tsp(int demod_no, MTV_TS_PKT_INFO *tsp)
{	
	if (tsp == NULL)
		return;

	spin_lock(&mtv_tsp_pool_tpeg.lock);
	...
	list_add_tail(&tsp->link, &mtv_tsp_pool_tpeg.head);
	mtv_tsp_pool_tpeg.cnt++;

	spin_unlock(&mtv_tsp_pool_tpeg.lock);
}


MTV_TS_PKT_INFO *mtv_alloc_tsp_tpeg(struct mtv_cb *mtv_cb_ptr)
{	
	MTV_TS_PKT_INFO *tsp = NULL;
	struct list_head *head_ptr = &mtv_tsp_pool_tpeg.head;
	...
	if (!list_empty(head_ptr)) {
		...
		mtv_cb_ptr->max_alloc_tsp_cnt 
			= MAX(mtv_cb_ptr->max_alloc_tsp_cnt,
				MAX_NUM_TS_PKT_BUF - mtv_tsp_pool_tpeg.cnt);
#endif	
	}
...
static int mtv_create_tsp_pool(struct mtv_cb *mtv_cb_ptr)
{
	...
	/* TSP queue */
	mtv_cb_ptr->prev_tsp = NULL;
	spin_lock_init(&mtv_tsp_queue_tpeg.lock);
	INIT_LIST_HEAD(&mtv_tsp_queue_tpeg.head);
	mtv_tsp_queue_tpeg.cnt = 0;
	
	spin_lock_init(&mtv_tsp_pool_tpeg.lock);
	INIT_LIST_HEAD(&mtv_tsp_pool_tpeg.head);

	mtv_tsp_pool_tpeg.cnt = 0;
#ifdef DEBUG_MTV_IF_MEMORY
...
void mtv_clear_contents_only_in_tsp_queue(struct mtv_cb *mtv_cb_ptr, int fic_msc_type)
{
	...
	spin_lock(&mtv_tsp_queue_tpeg.lock); /* for IRQ */

	list_for_each (pos, &mtv_tsp_queue_tpeg.head) { /* Lookup tsq Q */
...
{		
	...
	while (1) {
		...
		if ((tsp = tpeg_mtv_get_tsp(mtv_cb_ptr->demod_no)) == NULL)
...
unlock_read:
	tpeg_mtv_free_tsp(mtv_cb_ptr->demod_no, tsp); /* for "goto unlock_read;" statement. */

	if ((mtv_cb_ptr->f_flags & O_NONBLOCK) == 0) {
		if (mtv_tsp_queue_tpeg.cnt) /* for next time */
...
exit_read:
	...
	mtv_cb_ptr->max_remaining_tsp_cnt
		= MAX(mtv_cb_ptr->max_remaining_tsp_cnt, mtv_tsp_queue_tpeg.cnt);
#endif
...
static inline ssize_t read_single_service(struct mtv_cb * mtv_cb_ptr, char *buf, size_t cnt)
{
	...
	if (mtv_cb_ptr->f_flags & O_NONBLOCK) { /* non-blocking mode. */
		unsigned int have_tsp_bytes
			= mtv_tsp_queue_tpeg.cnt * mtv_cb_ptr->msc1_thres_size;
...
static int mtv_create_thread(struct mtv_cb * mtv_cb_ptr)
{
	...
	if (mtv_cb_ptr->isr_thread_cb == NULL) {
		...
		mtv_cb_ptr->isr_thread_cb
			= kthread_run(mtv_isr_thread_tpeg, NULL, "mtv_isr_thread_tpeg");
...
static void mtv_power_off(struct mtv_cb * mtv_cb_ptr)
{
	...
	rtvOEM_PowerOn_tpeg(mtv_cb_ptr->demod_no, 0);
...
static int mtv_power_on(struct mtv_cb * mtv_cb_ptr, E_DMB_TV_MODE_TYPE tv_mode, unsigned long arg)
{
	...
	rtvOEM_PowerOn_tpeg(mtv_cb_ptr->demod_no, 1);
...
atomic_t 	tdmb_open_flag_tpeg; /* to open only once */
...
static int mtv_close(struct inode *inode, struct file *filp)
{
	...
	atomic_set(&tdmb_open_flag_tpeg, 0);
...
static int mtv_open(struct inode *inode, struct file *filp)
{
	...
	if (atomic_cmpxchg(&tdmb_open_flag_tpeg, 0, 1)) {
...
static void mtv_deinit_device(void)
{
...
#if defined(RTV_IF_SPI) ||defined(RTV_FIC_I2C_INTR_ENABLED)
	free_irq(gpio_to_irq(RAONTV_IRQ_INT_1), NULL);
#endif
	...
	spi_unregister_driver(&mtv_spi_driver_tpeg);
#endif
...
static int mtv_init_device(void)
{
	...
	mtv_cb_ptrs_tpeg = mtv_cb_ptr;

	//DMBMSG("mtv_cb_ptr(0x%p), mtv_cb_ptrs_tpeg[%d](0x%p)\n", mtv_cb_ptr, i, mtv_cb_ptrs_tpeg[i]);
...
#elif defined(RTV_IF_SPI)
	if ((ret=spi_register_driver(&mtv_spi_driver_tpeg)) < 0) {
		DMBERR("RAONTV SPI0 driver register failed\n");
		goto err;
	}
...
#if defined(RTV_IF_SPI) || defined(RTV_FIC_I2C_INTR_ENABLED)
	mtv_cb_ptr->irq = gpio_to_irq(RAONTV_IRQ_INT_1); ///!!!! 
	ret = request_irq(mtv_cb_ptr->irq, mtv_isr_tpeg,
					IRQ_TYPE_EDGE_FALLING, "mtv_isr_tpeg", NULL);
	if (ret != 0) {
...
static int __init mtv_tpeg_module_init(void)
{
   	int ret;
...
static void __exit mtv_tpeg_module_exit(void)
{
	mtv_deinit_device();
...
module_init(mtv_tpeg_module_init);
module_exit(mtv_tpeg_module_exit);
MODULE_DESCRIPTION("MTV TPEG driver");
MODULE_LICENSE("GPL");
