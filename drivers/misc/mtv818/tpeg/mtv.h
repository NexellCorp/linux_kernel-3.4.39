...
struct mtv_cb
{
	...
	and tpeg_mtv_free_tsp(mtv_cb_ptr->prev_tsp) on ioctl:mtv_reset_tsp_queue(). */
	...
	struct spi_device *spi_ptr_tpeg;	
#endif
};
...
extern struct mtv_cb *mtv_cb_ptrs_tpeg;
#if defined(RTV_IF_TSIF) || defined(RTV_IF_SPI_SLAVE)
	extern struct i2c_driver mtv_i2c_driver;
#elif defined(RTV_IF_SPI)
	extern struct spi_driver mtv_spi_driver_tpeg;
#endif
...
void tpeg_mtv_put_tsp(int demod_no, MTV_TS_PKT_INFO *pkt);
void tpeg_mtv_free_tsp(int demod_no, MTV_TS_PKT_INFO *pkt);
