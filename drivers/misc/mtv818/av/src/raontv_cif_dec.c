...
#define MOD32(x)        ((x) & 31)
static U32 g_aAddedSubChIdBits[RAONTV_MAX_NUM_DEMOD_CHIP][2]; /* Used sub channel ID bits. [0]: 0 ~ 31, [1]: 32 ~ 63 */

static UINT g_nOutDecBufIdxBits; /* Decoded out buffer index bits */
static UINT g_aOutDecBufIdx[RAONTV_MAX_NUM_DEMOD_CHIP][64]; /* Decoded out buffer index for sub ch */

static E_RTV_SERVICE_TYPE g_eaSvcType[RAONTV_MAX_NUM_DEMOD_CHIP][64];

static BOOL g_fCifInited[RAONTV_MAX_NUM_DEMOD_CHIP] = FALSE;
static UINT g_nFicIC[RAONTV_MAX_NUM_DEMOD_CHIP];
