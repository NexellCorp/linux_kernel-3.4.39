...
volatile BOOL g_fRtvChannelChange[RAONTV_MAX_NUM_DEMOD_CHIP];


volatile E_RTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType[RAONTV_MAX_NUM_DEMOD_CHIP];
BOOL g_fRtvStreamEnabled[RAONTV_MAX_NUM_DEMOD_CHIP];

#if defined(RTV_TDMB_ENABLE) || defined(RTV_ISDBT_ENABLE)
	E_RTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType[RAONTV_MAX_NUM_DEMOD_CHIP];
#endif

#if defined(RTV_TDMB_ENABLE) || defined(RTV_DAB_ENABLE)
	BOOL g_fRtvFicOpened[RAONTV_MAX_NUM_DEMOD_CHIP];
	#if defined(RTV_IF_SPI) && defined(RTV_CIF_MODE_ENABLED)
	U32 g_aOpenedCifSubChBits_MSC0[RAONTV_MAX_NUM_DEMOD_CHIP][2]; /* Used sub channel ID bits. [0]: 0 ~ 31, [1]: 32 ~ 63 */
	#endif
...
UINT g_nRtvMscThresholdSize[RAONTV_MAX_NUM_DEMOD_CHIP]; 
U8 g_bRtvIntrMaskRegL[RAONTV_MAX_NUM_DEMOD_CHIP];
#ifdef RTV_DAB_ENABLE
