...
#define RAONTV_MAX_NUM_DEMOD_CHIP	2
...
############################################################################*/
	...
	#if defined(__KERNEL__)	
		extern struct mutex raontv_guard[RAONTV_MAX_NUM_DEMOD_CHIP];
		#define RTV_GUARD_INIT(demod_no)		mutex_init(&raontv_guard[demod_no])
		#define RTV_GUARD_LOCK(demod_no)		mutex_lock(&raontv_guard[demod_no])
		#define RTV_GUARD_FREE(demod_no)		mutex_unlock(&raontv_guard[demod_no])
		#define RTV_GUARD_DEINIT(demod_no)		((void)0)
		
    #elif defined(WINCE)        
	        extern CRITICAL_SECTION		raontv_guard[2];
	        #define RTV_GUARD_INIT(demod_no)	InitializeCriticalSection(&raontv_guard[demod_no])
	        #define RTV_GUARD_LOCK(demod_no)	EnterCriticalSection(&raontv_guard[demod_no])
	        #define RTV_GUARD_FREE(demod_no)	LeaveCriticalSection(&raontv_guard[demod_no])
	        #define RTV_GUARD_DEINIT(demod_no)	DeleteCriticalSection(&raontv_guard[demod_no])
	#else
		// temp: TODO
