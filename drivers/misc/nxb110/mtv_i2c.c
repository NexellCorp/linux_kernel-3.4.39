...
void mtv_i2c_read_burst(unsigned char reg, unsigned char *buf, int size)
{
	...
	struct i2c_msg msg[] = {
		 {.addr = NXB110TV_CHIP_ADDR>>1, .flags = 0, .buf = wbuf, .len = 1},
		 {.addr = NXB110TV_CHIP_ADDR>>1, .flags = I2C_M_RD, .buf = buf, .len = size}
	};
...
unsigned char mtv_i2c_read(unsigned char reg)
{
	...
	struct i2c_msg msg[] = {
	     {.addr = NXB110TV_CHIP_ADDR>>1, .flags = 0, .buf = wbuf, .len = 1},
	     {.addr = NXB110TV_CHIP_ADDR>>1, .flags = I2C_M_RD, .buf = rbuf, .len = 1}
	};
...
void mtv_i2c_write(unsigned char reg, unsigned char val)
{
	...
	struct i2c_msg msg =
		{.addr = NXB110TV_CHIP_ADDR>>1, .flags = 0, .buf = wbuf, .len = 2};
