...
#ifndef __NXB110TV_INTERNAL_H__
#define __NXB110TV_INTERNAL_H__
...
#define RTV_REG_MAP_SEL(demod_no, page)	\
	RTV_REG_SET(demod_no, MAP_SEL_REG, MAP_SEL_VAL(page))
...
extern volatile E_RTV_ADC_CLK_FREQ_TYPE g_eRtvAdcClkFreqType[NXB110TV_MAX_NUM_DEMOD_CHIP];
extern E_RTV_COUNTRY_BAND_TYPE g_eRtvCountryBandType[NXB110TV_MAX_NUM_DEMOD_CHIP];
...
extern     BOOL g_fDabEnableReconfigureIntr;
	...
	extern BOOL g_fRtvFicOpened[NXB110TV_MAX_NUM_DEMOD_CHIP];
	#if defined(RTV_IF_SPI) && defined(RTV_CIF_MODE_ENABLED)
	extern U32 g_aOpenedCifSubChBits_MSC0[NXB110TV_MAX_NUM_DEMOD_CHIP][2];
	#endif

	#if defined(RTV_IF_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE)
	extern E_RTV_FIC_OPENED_PATH_TYPE g_nRtvFicOpenedStatePath[NXB110TV_MAX_NUM_DEMOD_CHIP];
	#endif
#endif


/* Use SPI/EBI2 interrupt handler to prevent the changing of register map. */
extern volatile BOOL g_fRtvChannelChange[NXB110TV_MAX_NUM_DEMOD_CHIP];
extern BOOL g_fRtvStreamEnabled[NXB110TV_MAX_NUM_DEMOD_CHIP];

extern UINT g_nRtvMscThresholdSize[NXB110TV_MAX_NUM_DEMOD_CHIP];	
extern U8 g_bRtvIntrMaskRegL[NXB110TV_MAX_NUM_DEMOD_CHIP];
#ifdef RTV_DAB_ENABLE
	extern U8 g_bRtvIntrMaskRegH[NXB110TV_MAX_NUM_DEMOD_CHIP];
#endif



#define FM_MIN_FREQ_KHz			75950 // See PLL table in nxb110tv_rf_pll_data_fm.h
...
/* Forward prototype. */
static INLINE void rtv_SetupThreshold_MSC1(int demod_no, U16 wThresholdSize);
static INLINE void rtv_SetupMemory_MSC1(int demod_no, E_RTV_TV_MODE_TYPE eTvMode);

static INLINE void rtv_GetTii(int demod_no, RTV_TDMB_TII_INFO *ptTiiInfo)
{
	RTV_REG_MAP_SEL(demod_no, FEC_PAGE);
	RTV_REG_SET(demod_no, 0x5A, 0x00);
	ptTiiInfo->tii_combo = RTV_REG_GET(demod_no, 0x5C);
	ptTiiInfo->tii_pattern = RTV_REG_GET(demod_no, 0x5D);

	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x36, 0x40);
}
...
// Pause straem
static INLINE void rtv_StreamDisable(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{	
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	if(g_fRtvStreamEnabled[demod_no] == FALSE)
		return;

#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL[demod_no]|MSC0_INTR_BITS|MSC1_INTR_BITS);

#elif defined(RTV_IF_SERIAL_TSIF)
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x29, 0x08);
...
// Resume straem
static INLINE void rtv_StreamRestore(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	if(g_fRtvStreamEnabled[demod_no] == TRUE)
		return;

#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, INT_E_UCLRL, MSC0_INTR_BITS|MSC1_INTR_BITS);
...
#else
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x29, 0x00);
#endif

    g_fRtvStreamEnabled[demod_no] = TRUE;  
#endif /* #ifndef RTV_IF_MPEG2_PARALLEL_TSIF */
}


/* Enable the stream path forcely for ISDB-T and FM only! */
static INLINE void rtv_StreamEnable(int demod_no)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	if(g_fRtvStreamEnabled[demod_no] == TRUE)
		return;
...
#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)				
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x35, MSC1_INTR_BITS); // MSC1 Interrupt status clear.	

	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	g_bRtvIntrMaskRegL[demod_no] = 0x8F;
    RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL[demod_no]);
#else
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	RTV_REG_SET(demod_no, 0x29, 0x00);
...
#if defined(RTV_mode_fm) && defined(rds_enabled) // RTV_FM_RDS_ENABLED	

rtv_SetupThreshold_MSC0(512);
rtv_ClearAndSetupMemory_MSC0();

RTV_REG_SET(0x35, 0x01); // MSC0 Interrupt clear.


#endif
	rtv_SetupThreshold_MSC1(demod_no, g_nRtvMscThresholdSize[demod_no]);
  	rtv_SetupMemory_MSC1(demod_no, RTV_TV_MODE_1SEG);
...
static INLINE void rtv_ConfigureTsifFormat(int demod_no)
{
	...
	RTV_REG_SET(demod_no, 0x45, 0x00);    
  #elif defined(RTV_TSIF_FORMAT_2)
	RTV_REG_SET(demod_no, 0x45, 0x02);
  #elif defined(RTV_TSIF_FORMAT_3)
	RTV_REG_SET(demod_no, 0x45, 0x21);
  #elif defined(RTV_TSIF_FORMAT_4)
	RTV_REG_SET(demod_no, 0x45, 0x23);
  #else
	#error "Code not present"
  #endif
	...
	RTV_REG_SET(demod_no, 0x45, 0x00);    
  #elif defined(RTV_TSIF_FORMAT_2)
	RTV_REG_SET(demod_no, 0x45, 0xE9);

  #elif defined(RTV_TSIF_FORMAT_3)
	RTV_REG_SET(demod_no, 0x45, 0xE1);
  #elif defined(RTV_TSIF_FORMAT_4)
	RTV_REG_SET(demod_no, 0x45, 0x40);
	...
	RTV_REG_SET(demod_no, 0x46, 0x84);
#elif defined(RTV_IF_QUALCOMM_TSIF)
	RTV_REG_SET(demod_no, 0x46, 0xA4);
#elif defined(RTV_IF_SPI_SLAVE)
	RTV_REG_SET(demod_no, 0x46, 0x86);
#endif
}


static INLINE void rtv_ResetMemory_MSC0(int demod_no)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x47, 0x00);  // MSC0 memory control register clear.
}

static INLINE void rtv_ResetMemory_MSC1(int demod_no)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x48, 0x00);  // MSC1 memory control register clear.
#endif	
}

// Only for T-DMB and DAB.
static INLINE void rtv_ClearAndSetupMemory_MSC0(int demod_no)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	...
	RTV_REG_SET(demod_no, 0x47, 0x00);	
	RTV_REG_SET(demod_no, 0x47, (msc0_int_usel<<3) | (1/*msc0_en*/<<2) | int_type);
#endif /* #ifndef RTV_IF_MPEG2_PARALLEL_TSIF */	
}


// Only for T-DMB and DAB.
static INLINE void rtv_SetupThreshold_MSC0(int demod_no, U16 wThresholdSize)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x50,((wThresholdSize>>8) & 0x0F));
	RTV_REG_SET(demod_no, 0x51,(wThresholdSize & 0xFF));
}

static INLINE void rtv_SetupMemory_MSC0(int demod_no, U16 wThresholdSize)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);

	rtv_ClearAndSetupMemory_MSC0(demod_no);

	RTV_REG_SET(demod_no, 0x35, 0x02); // MSC0 Interrupt clear.
	
#endif /* #ifndef RTV_IF_MPEG2_PARALLEL_TSIF */	
}

// For SPI interface ISR.
static INLINE void rtv_ClearAndSetupMemory_MSC1(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{
#ifndef RTV_IF_MPEG2_PARALLEL_TSIF
	...
	RTV_REG_SET(demod_no, 0x48, 0x00);	
	RTV_REG_SET(demod_no, 0x48, (0/*msc1_length_off*/<<5) | (0/*msc1_header_on*/<<4) | (msc1_int_usel<<3) | (1/*msc1_en*/<<2) | int_type);
...
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
    RTV_REG_SET(demod_no, 0x56, ((wThresholdSize>>8) & 0x0F));
	RTV_REG_SET(demod_no, 0x57, (wThresholdSize & 0xFF));
}
...
static INLINE void rtv_SetupMemory_MSC1(int demod_no, E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	rtv_ClearAndSetupMemory_MSC1(demod_no, eTvMode);
#endif /* #ifndef RTV_IF_MPEG2_PARALLEL_TSIF */	
}

/* Only this sub channel contains the RS decoder. */
static INLINE void rtv_Set_MSC1_SUBCH0(int demod_no, UINT nSubChID, BOOL fSubChEnable, BOOL fRsEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	
#if !defined(RTV_IF_MPEG2_PARALLEL_TSIF)
	RTV_REG_SET(demod_no, 0x3A, (fSubChEnable << 7) | (fRsEnable << 6) | nSubChID);
#else
	if(fRsEnable ==TRUE)	
	{
		RTV_REG_SET(demod_no, 0xFF, 0x00);
		RTV_REG_SET(demod_no, 0x3A, (fSubChEnable << 7) | (fRsEnable << 6) | nSubChID);

	}
	else 
	{   RTV_REG_SET(demod_no, 0x3A, 0x00);
		RTV_REG_SET(demod_no, 0xFF, (fSubChEnable << 7) | nSubChID);
	}
#endif
}

static INLINE void rtv_Set_MSC1_SUBCH1(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x3B, (fSubChEnable << 7) | nSubChID);
}

static INLINE void rtv_Set_MSC1_SUBCH2(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x3C, (fSubChEnable << 7) | nSubChID);
}

static INLINE void rtv_Set_MSC0_SUBCH3(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x3D, (fSubChEnable << 7) | nSubChID);
}

static INLINE void rtv_Set_MSC0_SUBCH4(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x3E, (fSubChEnable << 7) | nSubChID);
}

static INLINE void rtv_Set_MSC0_SUBCH5(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x3F, (fSubChEnable << 7) | nSubChID);
}

static INLINE void rtv_Set_MSC0_SUBCH6(int demod_no, UINT nSubChID, BOOL fSubChEnable)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x42, (fSubChEnable << 7) | nSubChID);
}
 ...
 *============================================================================*/ 
#if defined(RTV_TDMB_ENABLE) || defined(RTV_DAB_ENABLE)
static INLINE void rtv_SetupMemory_FIC(int demod_no)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x46, 0x14); // auto user clr, get fic  CRC 2byte including[4]
#if defined(RTV_IF_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE)
	RTV_REG_SET(demod_no, 0x46, 0x1E);
	RTV_REG_SET(demod_no, 0x35, 0x01);
#endif
	RTV_REG_SET(demod_no, 0x46, 0x16); // FIC enable
}

static INLINE void rtv_ResetMemory_FIC(int demod_no)
{
	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	RTV_REG_SET(demod_no, 0x46, 0x00); 
}

static INLINE UINT rtv_GetFicSize(int demod_no)
{
	U8 tr_mode;
	UINT aFicSize[4] = {384/*TRMODE 1*/, 96, 128, 192};
	
   	RTV_REG_MAP_SEL(demod_no, DD_PAGE);
	tr_mode = RTV_REG_GET(demod_no, 0x37);
	if( tr_mode & 0x01 )
	{	
...
static INLINE void rtv_DisableFicTsifPath(int demod_no)
{
#if defined(RTV_IF_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE)
	U8 bCon47;

	RTV_REG_MAP_SEL(demod_no, COMM_PAGE);
	bCon47 = RTV_REG_GET(demod_no, 0x47);
	bCon47 &= ~(1<<2); /* FIC disable */ 	
	RTV_REG_SET(demod_no, 0x47, bCon47);
#endif
}

static INLINE void rtv_EnableFicTsifPath(int demod_no)
{
#if defined(RTV_IF_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE)
	U8 bCon47;
...
static INLINE void rtv_DisableFicInterrupt(int demod_no)
{
#if defined(RTV_FIC_SPI_INTR_ENABLED) || defined(RTV_FIC_I2C_INTR_ENABLED)
	RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
	g_bRtvIntrMaskRegL[demod_no] |= FIC_E_INT;
	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL[demod_no]);     
#endif
}

static INLINE void rtv_EnableFicInterrupt(int demod_no)
{
#if defined(RTV_FIC_SPI_INTR_ENABLED) || defined(RTV_FIC_I2C_INTR_ENABLED)
    RTV_REG_MAP_SEL(demod_no, DD_PAGE);
    RTV_REG_SET(demod_no, 0x35, 0x01); // FIC Interrupt status clear.	
    
    /* Enable the FIC interrupt. */
    RTV_REG_MAP_SEL(demod_no, HOST_PAGE);
    g_bRtvIntrMaskRegL[demod_no] &= ~(FIC_E_INT);
    RTV_REG_SET(demod_no, 0x62, g_bRtvIntrMaskRegL);
#endif
}

static INLINE void rtv_CloseFIC(int demod_no, UINT nOpenedSubChNum,
			E_RTV_FIC_OPENED_PATH_TYPE eOpenedPath)
{
	rtv_ResetMemory_FIC(demod_no);

#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	rtv_DisableFicInterrupt(demod_no);
...
static INLINE E_RTV_FIC_OPENED_PATH_TYPE rtv_OpenFIC(int demod_no, UINT nOpenedSubChNum)
{
	E_RTV_FIC_OPENED_PATH_TYPE ePath = FIC_OPENED_PATH_NOT_USE_IN_PLAY;

#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	rtv_EnableFicInterrupt(demod_no);
	...
	|| defined(RTV_FIC__SCAN_I2C__PLAY_TSIF)
		rtv_DisableFicTsifPath(demod_no); /* For I2C path */
		rtv_EnableFicInterrupt(demod_no);
	...
	|| defined(RTV_FIC__SCAN_TSIF__PLAY_TSIF)
		rtv_EnableFicTsifPath(demod_no);
	...
	|| defined(RTV_FIC__SCAN_TSIF__PLAY_TSIF)
		rtv_DisableFicInterrupt(demod_no); /* To prevent I2C interrupt */
		rtv_EnableFicTsifPath(demod_no);
	...
	rtv_SetupMemory_FIC(demod_no); 

	return ePath;
}

#if defined(RTV_IF_SPI) && defined(RTV_CIF_MODE_ENABLED)
static INLINE void rtv_DeleteSpiCifSubChannelID_MSC0(int demod_no, unsigned int nSubChID)
{
	UINT nArrayIdx = nSubChID >> 5; /* Divide by 32 */
	U32 dwBitVal = 1 << (nSubChID & 31); /* Modular and Shift */
	
	if((g_aOpenedCifSubChBits_MSC0[demod_no][nArrayIdx] & dwBitVal) != 0)
		g_aOpenedCifSubChBits_MSC0[demod_no][nArrayIdx] &= ~dwBitVal; 
}

/* This function add a sub channel ID to verfiy the SPI CIF header.*/
static INLINE void rtv_AddSpiCifSubChannelID_MSC0(int demod_no, unsigned int nSubChID)
{
	UINT nArrayIdx = nSubChID >> 5; /* Divide by 32 */
	U32 dwBitVal = 1 << (nSubChID & 31); /* Modular and Shift */
...
static INLINE BOOL rtv_IsOpenedCifSubChannelID_MSC0(int demod_no, unsigned int nSubChID)
{
	UINT nArrayIdx = nSubChID >> 5; /* Divide by 32 */
	U32 dwBitVal = 1 << (nSubChID & 31); /* Modular and Shift */
...
static INLINE unsigned int rtv_VerifySpiCif_MSC0(int demod_no, const unsigned char *buf,
										unsigned int size)
{
	...
	do {
		...
		if(rtv_IsOpenedCifSubChannelID_MSC0(demod_no, subch_id) == FALSE)
				break;
...
/*==============================================================================
 * External functions for NXB110TV driver core.
 *============================================================================*/ 
