...
* 
* TITLE 	  : NEXELL TV CIF Decoder source file for T-DMB and DAB. 
*
* FILENAME    : nxb110tv_cif_dec.c
*
* DESCRIPTION : 
...
#define MOD32(x)        ((x) & 31)
static U32 g_aAddedSubChIdBits[NXB110TV_MAX_NUM_DEMOD_CHIP][2]; /* Used sub channel ID bits. [0]: 0 ~ 31, [1]: 32 ~ 63 */

static UINT g_nOutDecBufIdxBits; /* Decoded out buffer index bits */
static UINT g_aOutDecBufIdx[NXB110TV_MAX_NUM_DEMOD_CHIP][64]; /* Decoded out buffer index for sub ch */

static E_RTV_SERVICE_TYPE g_eaSvcType[NXB110TV_MAX_NUM_DEMOD_CHIP][64];

static BOOL g_fCifInited[NXB110TV_MAX_NUM_DEMOD_CHIP] = FALSE;
static UINT g_nFicIC[NXB110TV_MAX_NUM_DEMOD_CHIP];
...
UINT rtvCIFDEC_GetDecBufIndex(UINT nSubChID)
{
	...
	CIF_MUTEX_LOCK;
	nDecIdx = g_aOutDecBufIdx[nSubChID];
	CIF_MUTEX_UNLOCK;
...
void rtvCIFDEC_DeleteSubChannelID(int demod_no, UINT nSubChID)
{
	UINT nOutDecBufIdx;

	CIF_MUTEX_LOCK(demod_no);

	if ((g_aAddedSubChIdBits[demod_no][DIV32(nSubChID)] & (1<<MOD32(nSubChID))) != 0) {
		// Delete a sub channel ID.
		g_aAddedSubChIdBits[demod_no][DIV32(nSubChID)] &= ~(1 << MOD32(nSubChID)); 

		nOutDecBufIdx = g_aOutDecBufIdx[demod_no][nSubChID];
		g_nOutDecBufIdxBits[demod_no] &= ~(1 << nOutDecBufIdx);
		g_aOutDecBufIdx[demod_no][nSubChID] = RTV_CIFDEC_INVALID_BUF_IDX;
		CIF_KFILE_CLOSE(demod_no, nSubChID);
	}

	CIF_MUTEX_UNLOCK(demod_no);
}
...
void rtvCIFDEC_Init(void)
{
	...
	memset(g_aOutDecBufIdx, RTV_CIFDEC_INVALID_BUF_IDX, 64);
