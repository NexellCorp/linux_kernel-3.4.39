...
static INLINE void read_threshold_msc0(struct mtv_cb * mtv_cb_ptr, MTV_TS_PKT_INFO *tsp)
{
	unsigned int size = mtv_cb_ptr->msc0_thres_size;

	RTV_REG_MAP_SEL(0, MSC0_PAGE);
	RTV_REG_BURST_GET(0, 0x10, tsp->msc0_buf, size+1); 
	tsp->msc0_size = size;
}
...
int mtv_isr_thread_av(void *data)
{
	DMBMSG("Start 0 ...\n");
...
irqreturn_t mtv_isr_av(int irq, void *param)
{
	if(mtv_cb_ptr->is_power_on == TRUE) {
	__isr_cnt++;
		mtv_cb_ptr->isr_cnt++;
