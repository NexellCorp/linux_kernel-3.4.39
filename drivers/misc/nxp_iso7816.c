...
static int uart_getc(void)
{
	...
	while (readl(pdata->uart_reg + UART_FR) & UART_PL01x_FR_RXFE) {
		if (time_after(jiffies, start + 50))
			return -1;
	}
	data = readl(pdata->uart_reg + UART_DR);
	if (data & 0xFFFFFF00) {
		/* Clear the error */
...
static void fifo_reset(void)
{
	unsigned int tmp = 0;
	while (!(readl(pdata->uart_reg + UART_FR) & UART_PL01x_FR_RXFE)){
		tmp = readl(pdata->uart_reg + UART_DR);
	}
	writel(0xFFFFFFFF, pdata->uart_reg + UART_ECR);
}
		...
		unsigned long arg)
{
	...
	case SCR_IO_WRITE:
		...
		fifo_reset();
		if( scr_cmd.wlen >= 32 ) {
			int len = scr_cmd.wlen;
			int remain = scr_cmd.wlen;
			int tr = 0;
			int trlen =32;

			do {
				write_uart(&scr_cmd.buf[tr],trlen);
				fifo_reset();
				remain -= trlen;
				tr += trlen;
				if(trlen > remain)
		...
		ret = read_uart(scr_cmd.buf, scr_cmd.rlen, 2000);
		if (ret < 0) {
