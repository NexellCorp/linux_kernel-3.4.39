...
static void register_queue_func(
				...
				void (*init)(struct common_queue *),
				void (*enqueue)(struct common_queue *,
						struct queue_entry *),
				struct queue_entry *(*dequeue)(
						 struct common_queue *),
				struct queue_entry *(*peek)(
					struct common_queue *, int pos),
				int (*clear)(struct common_queue *),
				void (*lock)(struct common_queue *),
				void (*unlock)(struct common_queue *),
				int (*size)(struct common_queue *)
...
#if defined(CONFIG_ARCH_S5P6818)
static void _vip_hw_set_addr(int module,
	struct nxp_backward_camera_platform_data *param,
	u32 lu_addr, u32 cb_addr, u32 cr_addr);
static void _vip_hw_set_clock(int module,
	struct nxp_backward_camera_platform_data *param, bool on);
static void _vip_hw_set_sensor_param(int module,
	struct nxp_backward_camera_platform_data *param);
#endif
...
static int _video_allocation_memory(struct nxp_backward_camera_context *,
				    struct nxp_video_frame_buf *);
static void _release_display_irq_callback(
			 struct nxp_backward_camera_context *);
...
static int _enqueue_all_buffer_to_empty_queue(
			struct nxp_backward_camera_context *);

#if defined(CONFIG_ARCH_S5P6818)
static void _init_hw_deinter(struct nxp_backward_camera_context *);
static void _set_deinter_interrupt(
				struct nxp_backward_camera_context *, bool);
#endif
		...
		struct nxp_backward_camera_platform_data *param, bool on)
{
	...
	if (on) {
		volatile void *clkgen_base = (volatile void *)IO_ADDRESS(
		NX_CLKGEN_GetPhysicalAddress(NX_VIP_GetClockNumber(module)));
		NX_CLKGEN_SetBaseAddress(NX_VIP_GetClockNumber(module),
					 (void *)clkgen_base);
		NX_CLKGEN_SetClockDivisorEnable(NX_VIP_GetClockNumber(module),
						CTRUE);
		NX_CLKGEN_SetClockBClkMode(NX_VIP_GetClockNumber(module),
					   NX_BCLKMODE_DYNAMIC);
#if defined(CONFIG_ARCH_S5P4418)
		NX_RSTCON_SetnRST(NX_VIP_GetResetNumber(module),
				  RSTCON_nDISABLE);
		NX_RSTCON_SetnRST(NX_VIP_GetResetNumber(module),
				  RSTCON_nENABLE);
#elif defined(CONFIG_ARCH_S5P6818)
...
#endif
		if (param->is_mipi) {
			NX_CLKGEN_SetClockSource(NX_VIP_GetClockNumber(module),
					0, ClkSrc); /* external PCLK */
			NX_CLKGEN_SetClockDivisor(
				NX_VIP_GetClockNumber(module), 0, Divisor);
			NX_CLKGEN_SetClockDivisorEnable(
				NX_VIP_GetClockNumber(module), CTRUE);
		} else {
			NX_CLKGEN_SetClockSource(NX_VIP_GetClockNumber(module),
				0, 4 + param->port); /* external PCLK */
			NX_CLKGEN_SetClockDivisor(
				NX_VIP_GetClockNumber(module), 0, 1);
			NX_CLKGEN_SetClockDivisorEnable(NX_VIP_GetClockNumber(
				module), CTRUE);
		}

		NX_VIP_SetBaseAddress(module,
		(void *)IO_ADDRESS(NX_VIP_GetPhysicalAddress(module)));
	}
}
			...
			struct nxp_backward_camera_platform_data *param)
{
	if (param->is_mipi) {
		...
		NX_VIP_SetFieldMode(module, CFALSE, NX_VIP_FIELDSEL_BYPASS,
				    CFALSE, CFALSE);
		...
		NX_VIP_SetHVSync(module,
				 ...
				 param->h_active*2,
				 param->interlace ?
				 param->v_active >> 1 : param->v_active,
				 param->h_syncwidth,
				 param->h_frontporch,
				 param->h_backporch,
				 param->v_syncwidth,
			     ...
			     u32 lu_addr, u32 cb_addr, u32 cr_addr)
{
	...
	if (param->is_mipi) {
		NX_VIP_SetClipperAddr(module, NX_VIP_FORMAT_420,
			...
			param->interlace ?
			ALIGN(Width/2, 64) : param->cb_stride);
	} else {
		NX_VIP_SetClipperAddr(module, NX_VIP_FORMAT_420,
				      param->h_active, param->v_active,
				      lu_addr, cb_addr, cr_addr,
				      param->lu_stride,
				      param->cb_stride);
	}
...
static void _vip_run(int module)
{
	...
	if (!me->plat_data->use_deinterlacer && !me->plat_data->multi_buffer) {
		u32 lu_addr = me->plat_data->lu_addr;
		u32 cb_addr = me->plat_data->cb_addr;
		u32 cr_addr = me->plat_data->cr_addr;

		_vip_hw_set_addr(module, me->plat_data, lu_addr, cb_addr,
				 cr_addr);
	} else {
		struct queue_entry *ent = NULL;
		struct nxp_video_frame_buf *buf = NULL;
		...
		if (ent) {
			buf = (struct nxp_video_frame_buf *)(ent->data);
			_vip_hw_set_addr(vip_module_num, me->plat_data,
...
static void _vip_stop(int module)
{
...
#if 0
	{
		...
		while (CFALSE == NX_VIP_GetInterruptPending(module, intnum))
			pr_err("failt to interrrupt pending.\n");
...
static void _mlc_video_set_param(int module,
			 struct nxp_backward_camera_platform_data *param)
{
...
static void _mlc_rgb_overlay_set_param(int module,
			struct nxp_backward_camera_platform_data *param)
{
	...
	NX_MLC_SetRGBLayerInvalidPosition(
					module, layer, 0, 0, 0, 0, 0, CFALSE);
	NX_MLC_SetRGBLayerInvalidPosition(
					module, layer, 1, 0, 0, 0, 0, CFALSE);
	NX_MLC_SetPosition(
			module, layer, 0, 0, param->width-1, param->height-1);
...
static void _mlc_rgb_overlay_draw(int module,
		struct nxp_backward_camera_platform_data *me, void *mem)
{
...
static void _turn_on(struct nxp_backward_camera_context *me)
{
	...
	if (me->plat_data->pre_turn_on) {
		if (!me->plat_data->pre_turn_on(
					me->plat_data->vendor_context)) {
			pr_err("failed to platform pre_turn_on() function.\n");
			return;
		}
	}
...
static inline bool _is_backgear_on(
...
static inline bool _is_running(struct nxp_backward_camera_context *me)
{
	...
	NX_VIP_GetVIPEnable(me->plat_data->vip_module_num, &vipenb, &sepenb,
			    &clipenb, &decenb);
...
static void _decide(struct nxp_backward_camera_context *me)
{
	...
	if (me->plat_data->decide) {
		if (!me->plat_data->decide(me->plat_data->vendor_context)) {
			pr_err("failed to platform decide() function\n");
			return;
		}
	}
	...
	if (me->backgear_on && !me->running) {
		...
		schedule_delayed_work(&me->work,
			msecs_to_jiffies(me->plat_data->turn_on_delay_ms));
	}
...
static int wait_for_end_of_work(wait_queue_head_t *wq,
				atomic_t *status,
				int init_status,
				int end_work_status,
				int elapse)
{
	...
	if (atomic_read(status) == init_status) {
		timeout = wait_event_interruptible_timeout(*wq,
					atomic_read(status) == end_work_status,
					elapse);
		if (timeout <= 0)
		{
			pr_info("The timeout elapsed before the condition ");
			pr_info("Condition Status = %d, timeout : %ld\n",
...
static irqreturn_t vip_irq_handler(int irq, void *devdata)
{
	...
	if (!entry) {
		...
		buf = (struct nxp_video_frame_buf *)(entry->data);
		_vip_hw_set_addr(vip_module_num, me->plat_data, buf->lu_addr,
				 buf->cb_addr, buf->cr_addr);
...
static void _release_irq_deinter(void)
{
	...
	if (me->deinter_irq)
		free_irq(me->deinter_irq, &_context);
}
...
static void _init_hw_vip(struct nxp_backward_camera_context *me)
{
	...
	ret = request_irq(me->vip_irq, vip_irq_handler, IRQF_SHARED,
			  "backward-vip", me);
	if (ret)
		BUG();
}
...
#if defined(CONFIG_ARCH_S5P6818)
static void _set_deinter_interrupt(struct nxp_backward_camera_context *me,
				   bool on)
{
...
static void _init_hw_deinter(struct nxp_backward_camera_context *me)
{
	...
	NX_DEINTERLACE_SetBaseAddress(
		(void *)IO_ADDRESS(NX_DEINTERLACE_GetPhysicalAddress()));

	NX_CLKGEN_SetBaseAddress(NX_DEINTERLACE_GetClockNumber(),
			(void *)IO_ADDRESS(NX_CLKGEN_GetPhysicalAddress(
			NX_DEINTERLACE_GetClockNumber())));
	NX_CLKGEN_SetClockBClkMode(NX_DEINTERLACE_GetClockNumber(),
				    NX_BCLKMODE_ALWAYS);
	...
	if (request_irq(me->deinter_irq, deinter_irq_handler, IRQF_SHARED,
			"backward-deinter", me))
		BUG();
...
static void SetDeInterlace(U16 Height, U16 Width,
			    U32 Y_PrevAddr,  U32 Y_CurrAddr,   U32 Y_NextAddr,
			    U32 Y_SrcStride, U32 Y_DstAddr, U32 Y_DstStride,
			    U32 CB_CurrAddr, U32 CB_SrcStride, U32 CB_DstAddr,
			    U32 CB_DstStride, U32 CR_CurrAddr,
			    U32 CR_SrcStride, U32 CR_DstAddr, U32 CR_DstStride,
			    int IsODD)
{
	...
	if (IsODD) {
		...
		NX_DEINTERLACE_SetYCBCRField(NX_DEINTERLACE_FIELD_EVEN,
		...
		/*  Start */
		NX_DEINTERLACE_SetYCBCREnable(CTRUE, CTRUE, CTRUE);
		NX_DEINTERLACE_SetYCBCRField(NX_DEINTERLACE_FIELD_ODD,
						  NX_DEINTERLACE_FIELD_ODD,
						  NX_DEINTERLACE_FIELD_ODD);
	}
}

static int _make_deinter_context(struct nxp_video_frame_buf *src1,
				 struct nxp_video_frame_buf *src2,
				 struct nxp_deinter_run_ctx *ctx_buf)
{
	struct nxp_backward_camera_context *me = &_context;
	...
	debug_msg("%s - lu_addr : 0x%x, cb_addr : 0x%x, cr_addr : 0x%x\n",
		  __func__, deinter_buf->src[0].lu_addr,
		  deinter_buf->src[0].cb_addr, deinter_buf->src[0].cr_addr);
	...
	if (me->plat_data->devide_frame) {
		if (me->frame_set.cur_mode == FIELD_EVEN) {
			...
			/*  ODD */
			frame1->frame_num = 1;
			frame1->lu_addr = src1->lu_addr +
				(src1->lu_stride * src_height);
			frame1->cb_addr = src1->cb_addr +
				(src1->cb_stride * (src_height/2));
			frame1->cr_addr = src1->cr_addr +
				(src1->cr_stride * (src_height/2));
			...
			frame2->frame_num = 1;
			frame2->lu_addr = src2->lu_addr +
				(src2->lu_stride * src_height);
			frame2->cb_addr = src2->cb_addr +
				(src2->cb_stride * (src_height/2));
			frame2->cr_addr = src2->cr_addr +
				(src2->cr_stride * (src_height/2));
			frame2->lu_stride = YUV_YSTRIDE(src_width);
			frame2->cb_stride = YUV_STRIDE(src_width/2);
			frame2->cr_stride = YUV_STRIDE(src_width/2);
		}
...
static void _deinter_run(struct nxp_deinter_run_ctx *ctx)
{
	...
	unsigned long src_prev_y_data_phy, src_curr_y_data_phy,
		      src_next_y_data_phy, src_curr_cb_data_phy,
	...
	if (0 > wait_for_end_of_work(&me->wq_deinter_end, &me->status,
				     PROCESSING_START, PROCESSING_STOP, HZ/10))
		pr_info("%s: CRITICAL ERROR timeout(over 100ms)!!!\n",
		       __func__);
}
#endif
...
{
	struct nxp_backward_camera_context *me =
		(struct nxp_backward_camera_context *)data;
	struct queue_entry *entry = NULL;
	struct nxp_video_frame_buf *buf = NULL;
	...
	if (me->plat_data->use_deinterlacer) {
		...
		debug_msg("%s lu_addr:0x%x, cb_addr:0x%x, cr_addr:0x%x\n",
			  ...
			  buf->cb_addr, buf->cr_addr);
		debug_msg("%s lu_stride:%d, cb_stride:%d, cr_stride : %d\n",
			  __func__, buf->lu_stride,
			  buf->cb_stride, buf->cr_stride);
		debug_msg("%s - 1 : current display lu_addr : 0x%x\n",
			  __func__, buf->lu_addr);
		...
		for (i = 0; i < q_size-1; i++) {
			...
			buf = (struct nxp_video_frame_buf *)(entry->data);
			debug_msg("%s enqueue deinter empty lu_addr : 0x%x\n",
				  __func__, buf->lu_addr);
...
static void _set_vip_interrupt(struct nxp_backward_camera_context *me, bool on)
{
	...
	if (on)
		NX_VIP_SetInterruptEnable(me->plat_data->vip_module_num,
					  CAPTURE_CLIPPER_INT, CTRUE);
}
...
static void _deinter_worker(struct work_struct *work)
{
	...
	debug_msg("%s : vip done queue size : %d\n", __func__,
		  me->q_vip_done.size(&me->q_vip_done));
	...
	while (e_idx < 2) {
		...
		if (ent == NULL) {
			pr_err("%s : DEINTER UNDERRUN!! %d\n",
			       __func__, e_idx);
			pr_err("%s : Check Display Callback Cycle!!!, loop.\n",
			       __func__);
			mutex_unlock(&me->deinter_lock);
			return;
		}
	...
	buf = (struct nxp_video_frame_buf *)(entry->data);
	debug_msg("%s lu_addr : 0x%x, cb_addr : 0x%x, cr_addr : 0x%x\n",
		  __func__, buf->lu_addr, buf->cb_addr,
		  buf->cr_addr);
	q_vip_empty->enqueue(q_vip_empty, entry);
...
void nxp_mipi_csi_setting(struct nxp_backward_camera_platform_data *me,
			  int lanes)
{
	...
	if (NumberOfDataLanes >= 0)
		EnableDataLane0 = CTRUE;
	if (NumberOfDataLanes >= 1)
		EnableDataLane1 = CTRUE;
	if (NumberOfDataLanes >= 2)
		EnableDataLane2 = CTRUE;
	if (NumberOfDataLanes >= 3)
		EnableDataLane3 = CTRUE;
#endif
	...
	NX_MIPI_SetBaseAddress(me->vip_module_num,
			(void *)IO_ADDRESS(NX_MIPI_GetPhysicalAddress(
							 me->vip_module_num)));
	NX_CLKGEN_SetBaseAddress(NX_MIPI_GetClockNumber(me->vip_module_num),
			(void *)IO_ADDRESS(NX_CLKGEN_GetPhysicalAddress(
				NX_MIPI_GetClockNumber(me->vip_module_num))));
	NX_CLKGEN_SetClockDivisorEnable(
			NX_MIPI_GetClockNumber(me->vip_module_num), CTRUE);
...
#if defined(CONFIG_ARCH_S5P6818)
	{
		...
		pmipi = (volatile NX_MIPI_RegisterSet *)
			IO_ADDRESS(NX_MIPI_GetPhysicalAddress(
				me->vip_module_num));
...
#else
{
	...
	NX_CLKGEN_SetClockDivisorEnable(
			NX_MIPI_GetClockNumber(me->vip_module_num), CFALSE);
	/* TODO : use clk_get(), clk_get_rate() for dynamic clk binding */
#if defined(CONFIG_VIDEO_TW9992)
	NX_CLKGEN_SetClockSource(NX_MIPI_GetClockNumber(me->vip_module_num),
				 0, ClkSrc); /*  use pll2 -> current 295MHz */
	NX_CLKGEN_SetClockDivisor(NX_MIPI_GetClockNumber(me->vip_module_num),
				 0, Divisor);
#else
	NX_CLKGEN_SetClockSource(NX_MIPI_GetClockNumber(me->vip_module_num),
				 0, 2); /*  use pll2 -> current 295MHz */
	NX_CLKGEN_SetClockDivisor(NX_MIPI_GetClockNumber(me->vip_module_num),
				 0, 2);
#endif
	/* NX_CLKGEN_SetClockDivisor(NX_MIPI_GetClockNumber(
	   me->vip_module_num), 0, 6); */
	NX_CLKGEN_SetClockDivisorEnable(NX_MIPI_GetClockNumber(
				me->vip_module_num), CTRUE);

	NX_MIPI_CSI_SetParallelDataAlignment32(me->vip_module_num, 1, CFALSE);
	NX_MIPI_CSI_SetYUV422Layout(me->vip_module_num, 1,
				NX_MIPI_CSI_YUV422LAYOUT_FULL);
	NX_MIPI_CSI_SetFormat(me->vip_module_num, 1,
				NX_MIPI_CSI_FORMAT_YUV422_8);
	NX_MIPI_CSI_EnableDecompress(me->vip_module_num, CFALSE);
	NX_MIPI_CSI_SetInterleaveMode(me->vip_module_num,
				      NX_MIPI_CSI_INTERLEAVE_VC);
	...
	/*
	   ...
	   pdata->lanes, EnableDataLane0, EnableDataLane1, EnableDataLane2,
	   EnableDataLane3);
	 */
...
#if defined(CONFIG_VIDEO_TW9992)
			   NumberOfDataLanes,	/*  U32   NumberOfDataLanes
						    (0~3) */
...
#if defined(CONFIG_VIDEO_TW9992)
#if defined(CONFIG_ARCH_S5P4418)
			    0x43E8,      /* 19'h033E8: 1Ghz 19'h043E8:750Mhz */
					 /*	Use
					  *	LN28LPP_MipiDphyCore1p5Gbps
						_Supplement.
					  *	*/
			    0xC,        /*  4'hF: 1Ghz  4'hC: 750Mhz */
					/*  Use LN28LPP_MipiDphyCore1p5Gbps */
					/*  _Supplement. */
#elif defined(CONFIG_ARCH_S5P6818)
			    0x33E8,     /* 19'h033E8: 1Ghz 19'h043E8: 750Mhz */
					/*  Use LN28LPP_MipiDphyCore1p5Gbps */
					/*  _Supplement. */
			    0xF,        /*  4'hF: 1Ghz  4'hC: 750Mhz */
					/*  Use LN28LPP_MipiDphyCore1p5Gbps */
					/*  _Supplement. */
#endif
#else
...
#endif
			    0,           /*  U32 M_PLLCTL      , */
				/*  Refer to 10.2.2 M_PLLCTL of */
				/*  MIPI_D_PHY_USER_GUIDE.pdf */
				/*  Default value is all "0". */
				/*  If you want to change register values, */
				/*  it need to confirm from IP Design Team */
			    0            /*  U32 B_DPHYCTL */
				/*  Refer to 10.2.3 M_PLLCTL of */
				/*  MIPI_D_PHY_USER_GUIDE.pdf */
				/*  or NX_MIPI_PHY_B_DPHYCTL enum or */
				/*  LN28LPP_MipiDphyCore1p5Gbps_Supplement. */
				/*  default value is all "0". */
				/*  If you want to change register values, */
				/* it need to confirm from IP Design Team */
				   );

	NX_MIPI_SetInterruptEnable(me->vip_module_num,
				   NX_MIPI_INT_CSI_FrameEnd_CH1, CTRUE);
}
...
static void _set_display_irq_callback(struct nxp_backward_camera_context *me)
{
	...
	me->callback = (void *)nxp_soc_disp_register_irq_callback(
					dpc_module_num, _display_callback, me);
}
					...
					struct nxp_backward_camera_context *me)
{
	...
	if (me->callback) {
		nxp_soc_disp_unregister_irq_callback(dpc_module_num,
				(struct disp_irq_callback *)me->callback);
		me->callback = NULL;
	}
...
static int _video_allocation_memory(struct nxp_backward_camera_context *me,
				    struct nxp_video_frame_buf *buf)
{
	...
	buf->ion_handle = ion_alloc(me->ion_client, size, 0,
	...
	buf->cb_addr = buf->lu_addr + me->plat_data->lu_addr +
		me->plat_data->lu_stride * me->plat_data->v_active;
	buf->cr_addr = buf->cb_addr + me->plat_data->cb_addr +
		me->plat_data->cb_stride * (me->plat_data->v_active / 2);
...
static int _video_allocation_deinter_dst_memory(
	struct nxp_backward_camera_context *me,
	struct nxp_video_frame_buf *buf)
{
	struct ion_device *ion_dev = get_global_ion_device();
	...
	buf->lu_addr = ion_buffer->priv_phys;
	buf->cb_addr = buf->lu_addr +
		me->plat_data->lu_addr + lu_stride * me->plat_data->v_active;
	buf->cr_addr = buf->cb_addr +
		me->plat_data->cb_addr + cb_stride *
		(me->plat_data->v_active / 2);
...
static int _rgb_allocation_memory(struct nxp_backward_camera_context *me)
{
	...
	buf->ion_handle_rgb = ion_alloc(me->ion_client, size, 0,
					ION_HEAP_NXP_CONTIG_MASK, 0);
	...
	buf->dma_buf_rgb = ion_share_dma_buf(me->ion_client,
					     buf->ion_handle_rgb);
...
static int _init_buffer(struct nxp_backward_camera_context *me)
{
	...
	for (i = 0; i < MAX_BUFFER_COUNT; i++) {
		struct nxp_video_frame_buf *buf =
			&me->frame_set.deinter_bufs[i];
		struct queue_entry *entry =
			&me->frame_set.deinter_queue_entry[i];
...
static int _stride_cal(struct nxp_backward_camera_context *me,
		       enum FRAME_KIND type)
{
	int value = 0;
	int stride = 0;
...
static int _allocate_memory(struct nxp_backward_camera_context *me)
{
	...
	if (!me->plat_data->lu_addr) {
		...
		me->ion_handle_video = ion_alloc(me->ion_client, size, 0,
						 ION_HEAP_NXP_CONTIG_MASK, 0);
		if (IS_ERR(me->ion_handle_video)) {
			pr_err("%s: failed to ion_alloc(), size %d\n",
			       __func__, size);
			return -ENOMEM;
		}
		me->dma_buf_video = ion_share_dma_buf(me->ion_client,
						      me->ion_handle_video);
		...
		me->plat_data->cb_addr = me->plat_data->lu_addr +
			me->plat_data->lu_stride * me->plat_data->v_active;
		me->plat_data->cr_addr = me->plat_data->cb_addr +
			me->plat_data->cb_stride *
			(me->plat_data->v_active / 2);
	...
	if (!me->plat_data->rgb_addr) {
		u32 format = me->plat_data->rgb_format;
		u32 pixelbyte = _get_pixel_byte(format);
		int size = me->plat_data->width * me->plat_data->height *
			pixelbyte;
		struct ion_buffer *ion_buffer;
		size = PAGE_ALIGN(size);

		me->ion_handle_rgb = ion_alloc(me->ion_client, size, 0,
					       ION_HEAP_NXP_CONTIG_MASK, 0);
		...
		me->dma_buf_rgb = ion_share_dma_buf(me->ion_client,
						    me->ion_handle_rgb);
...
static void _free_buffer(struct nxp_backward_camera_context *me)
{
	if (me->plat_data->use_deinterlacer || me->plat_data->multi_buffer) {
		...
		for (i = 0; i < MAX_BUFFER_COUNT; i++) {
			struct nxp_video_frame_buf *buf =
				&me->frame_set.vip_bufs[i];

			if (buf) {
				if (buf->dma_buf != NULL) {
					dma_buf_put(buf->dma_buf);
					buf->dma_buf = NULL;
					ion_free(me->ion_client,
						 buf->ion_handle);
					buf->ion_handle = NULL;
				}
			}
		...
		if (me->frame_set.rgb_buf.dma_buf_rgb != NULL) {
			...
			ion_free(me->ion_client,
				 me->frame_set.rgb_buf.ion_handle_rgb);
...
static void _resume_work(struct work_struct *work)
{
	...
	if (!me->plat_data->use_deinterlacer && !me->plat_data->multi_buffer) {
		...
		_vip_hw_set_addr(vip_module_num, me->plat_data, lu_addr,
				 cb_addr, cr_addr);
...
static int nxp_backward_camera_resume(struct device *dev)
{
	...
	queue_delayed_work(system_nrt_wq, &me->resume_work,
			   msecs_to_jiffies(RESUME_CAMERA_ON_DELAY_MS));
...
static const struct dev_pm_ops nxp_backward_camera_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(nxp_backward_camera_suspend,
				nxp_backward_camera_resume)
};
				     ...
				     const char *buf, size_t n)
{
	...
	if (module == get_backward_module_num()) {
		while (is_backward_camera_on()) {
			pr_info("wait backward camera stopping...\n");
...
static int _create_sysfs(void)
{
	...
	if (ret) {
		pr_err("Fail, create sysfs group for backward camera\n");
		kobject_del(kobj);
		return -ret;
	}
...
static int nxp_backward_camera_probe(struct platform_device *pdev)
{
	...
	NX_MLC_SetBaseAddress(pdata->mlc_module_num,
			      (void *)IO_ADDRESS(NX_MLC_GetPhysicalAddress(
						pdata->mlc_module_num)));
	NX_VIP_SetBaseAddress(pdata->vip_module_num,
			      (void *)IO_ADDRESS(NX_VIP_GetPhysicalAddress(
						pdata->vip_module_num)));

	_init_context(me);

	if (pdata->use_deinterlacer || me->plat_data->multi_buffer) {
	...
	ret = request_irq(me->irq, _irq_handler,
	...
	if (me->plat_data->alloc_vendor_context) {
		if (!me->plat_data->alloc_vendor_context(
					 me->plat_data->vendor_context)) {
			pr_err("failed to platform alloc_vendor_context()");
...
static int nxp_backward_camera_remove(struct platform_device *pdev)
{
	...
	if (me->removed == false) {
		...
		/* TODO : To this routine : may not arise event
		   for irq restart term */
		me->removed = true;

		if (me->plat_data->free_vendor_context)
			me->plat_data->free_vendor_context(
					me->plat_data->vendor_context);
	}
	return 0;
}
