...
#define	I2C5_SCL	((PAD_GPIO_C + 25))		// AP_GPC25_ADV7391_HUBSCL
#define	I2C5_SDA	((PAD_GPIO_C + 27))		// AP_GPC27_ADV7391_HUBSDA

#define HMO_I2CBUS		6
#define	I2C6_SCL	((PAD_GPIO_D + 29) | PAD_FUNC_ALT0)	// AP_GPD29_HMO_SCL
#define	I2C6_SDA	((PAD_GPIO_D + 30) | PAD_FUNC_ALT0)	// AP_GPD30_HMO_SDA

#define DES_I2CBUS		7
#define	I2C7_SCL	((PAD_GPIO_E + 3) | PAD_FUNC_ALT0)	// AP_GSML_SCL
#define	I2C7_SDA	((PAD_GPIO_E + 2) | PAD_FUNC_ALT0)	// AP_GSML_SDA

#define PMIC_I2CBUS		1
#if 0
#define I2C8_SCL    ((PAD_GPIO_D + 4) | PAD_FUNC_ALT0)	// AP_GPD4_PMIC_SCL
#define I2C8_SDA    ((PAD_GPIO_D + 5) | PAD_FUNC_ALT0)	// AP_GPD5_PMIC_SDA
#endif
...
#if defined(CONFIG_TOUCHSCREEN_SEMISENS)
#include <linux/i2c.h>
#include <linux/input/semisens_ts.h>

static struct touch_pdata semisens_touch_pdata = {
	.irq_gpio		= GPIO_FOR_TS_IRQ,			// irq gpio define
	.reset_gpio		= GPIO_FOR_TS_RESETB,		// reset gpio define
	.ldo_en_gpio	= GPIO_FOR_TOUCH_LDO,		// ldo gpio for touch define

	.abs_max_x		= (1280 - 1), 
	.abs_max_y		= (800 - 1),
}; 

static struct i2c_board_info semisens_i2c_ts_info[] = {
	{
		I2C_BOARD_INFO(SEMISENS_TS_NAME, SEMISENS_TS_I2C_SLAVE_ADDR),
		.irq			= IRQ_NUM_FOR_TS_INTERRUPT,
		.platform_data	= &semisens_touch_pdata,		
	},
};
#endif /* end of #if defined(CONFIG_TOUCHSCREEN_SEMISENS) */
...
#define MAX9286_CAM_WIDTH		1280*4//5120//3840//2560//1280
#define MAX9286_CAM_HEIGHT		799//800 // 800*4

static int max9286_camera_power_enable(bool on)
{
#if 0// defined(CONFIG_VIDEO_MAX9286)
	unsigned int reset_io = (PAD_GPIO_B + 23);
...
static bool max9286_camera_power_state_changed(void)
{
    return is_front_camera_power_state_changed;
}

static void max9286_vin_setup_io(int module, bool force)
{
	/* do nothing */
	return;
}

static int max9286_camera_set_clock(ulong clk_rate)
{
#if 0
	PM_DBGOUT("%s: %d\n", __func__, (int)clk_rate);
	printk("%s: %d\n", __func__, (int)clk_rate);
    if (clk_rate > 0)
        nxp_soc_pwm_set_frequency(1, clk_rate, 5);
    else
        nxp_soc_pwm_set_frequency(1, 0, 0);
    msleep(1);
#endif
...
static int max9286_phy_enable(bool en)
{
	/* do nothing */
	return 0;
}

static struct i2c_board_info max9286_i2c_boardinfo[] = {
    {
        I2C_BOARD_INFO("max9286", 0x90>>1),
    },
};

struct nxp_mipi_csi_platformdata max9286_plat_data = {
	.module	=	0,
	.clk_rate	=	27000000, //27MHz
	.lanes		=	4,
	.alignment	=	0,
	.hs_settle	=	0,
	.width		=	MAX9286_CAM_WIDTH,
	.height		=	MAX9286_CAM_HEIGHT,
	.fixed_phy_vdd	=	false,
	.irq		=	0, /* not used */
	.base		=	0, /* not used */
	.phy_enable	=	max9286_phy_enable,
};
...
static struct nxp_v4l2_i2c_board_info sensor[] = {
	...
	{
        .board_info = &max9286_i2c_boardinfo[0],
        .i2c_adapter_id = DES_I2CBUS,
    },
};
...
static struct nxp_capture_platformdata capture_plat_data[] = {
...
#if defined(CONFIG_VIDEO_MAX9286)
    {
        ...
        .parallel = {
            .is_mipi        = true,
            .external_sync  = true,
            .h_active       = MAX9286_CAM_WIDTH,
            .h_frontporch   = 7,//8,
            .h_syncwidth    = 0,//7,
            .h_backporch    = 7,
            .v_active       = MAX9286_CAM_HEIGHT,
            .v_frontporch   = 0,//1,
            .v_syncwidth    = 0,//8,
            .v_backporch    = 1,
            .clock_invert   = false,
            ...
            .data_order     = NXP_VIN_CBY0CRY1,
            .interlace      = false,
            .clk_rate       = 30,//24000000,
            .late_power_down = false,
            .power_enable   = max9286_camera_power_enable,
            .power_state_changed = max9286_camera_power_state_changed,
            .set_clock      = max9286_camera_set_clock,
            .setup_io       = max9286_vin_setup_io,
        },
        .deci = {
            .start_delay_ms = 0,
            .stop_delay_ms  = 0,
        },
        .csi = &max9286_plat_data,
    },
#endif
...
#endif
#if defined(CONFIG_MAX9286_BACKWARD)
	static struct reg_val _sensor_init_data[] =
	{
		END_MARKER
    };
...
static int _sensor_power_enable(bool enable)
{
	...
	if (enable) {
		...
		/* Select IN */
		nxp_soc_gpio_set_io_func(sel_in,
				nxp_soc_gpio_get_altnum(sel_in));
		nxp_soc_gpio_set_io_dir(sel_in, 1);
		nxp_soc_gpio_set_out_value(sel_in, 1);
		mdelay(10);

		/* enable MUX */
		nxp_soc_gpio_set_io_func(en_mux,
				nxp_soc_gpio_get_altnum(en_mux));
		nxp_soc_gpio_set_io_dir(en_mux, 1);
		nxp_soc_gpio_set_out_value(en_mux, 0);
		mdelay(10);
...
static struct nxp_backward_camera_platform_data backward_camera_plat_data = {
	...
	.data_order             = 0,
	.interlace              = true,
	.multi_buffer		= true, /* if .user_deinterlace is true,
					   vip buffer uses multi_buffer
	...
	.devide_frame           = false, /* Data Sequence : TW9900 :
					(odd-even-odd ~ even) per
					frame = false, */
	.is_odd_first           = true,
...
static struct nxp_backward_camera_platform_data backward_camera_plat_data = {
	...
	.use_deinterlacer       = false,
	/*	.use_deinterlacer       = true,	*/
	.devide_frame           = false, /* Data Sequence :
					    TW9900 : (odd-even-odd ~ even) per
					    frame = false, */
	.is_odd_first           = true,
...
#if defined(CONFIG_MAX9286_BACKWARD)
#define BACKWARD_CAM_WIDTH  5120
#define BACKWARD_CAM_HEIGHT 799
...
#if defined(CONFIG_TOUCHSCREEN_SEMISENS)
	i2c_register_board_info(I2C_BUS_ID_FOR_TS, semisens_i2c_ts_info, 
			ARRAY_SIZE(semisens_i2c_ts_info));
	printk(" >>>>>>>>>>>>>>>>> irq num= %d:%d\n", PB_PIO_IRQ(GPIO_FOR_TS_IRQ), semisens_i2c_ts_info[0].irq);
#endif /* end of #if defined(CONFIG_TOUCHSCREEN_SEMISENS) */
