...
{
	unsigned int io				= CAMERA_POWER_EN;
	unsigned int reset_io	= CAMERA_RESET;
	unsigned int pd_io	= CAMERA_POWER_PD;

	if (enable) {
      nxp_soc_gpio_set_out_value(pd_io, 1);
			camera_power_control(0);
...
static void _sensor_setup_io(void)
{
	...
	const u_int port[][2] = {
		...
		{	PAD_GPIO_D	+ 28, NX_GPIO_PADFUNC_1 }, {	PAD_GPIO_D	+ 29, NX_GPIO_PADFUNC_1 },  
		{	PAD_GPIO_D	+ 30, NX_GPIO_PADFUNC_1 }, {	PAD_GPIO_D	+ 31, NX_GPIO_PADFUNC_1 },  
		{	PAD_GPIO_E	+  0, NX_GPIO_PADFUNC_1 }, {	PAD_GPIO_D	+  1, NX_GPIO_PADFUNC_1 },  
		{	PAD_GPIO_E	+  2, NX_GPIO_PADFUNC_1 }, {	PAD_GPIO_D	+  3, NX_GPIO_PADFUNC_1 },  
	};
...
#if defined(CONFIG_SLSIAP_BACKWARD_CAMERA)
		printk("plat: add device backward-camera platform device to boot\n");
