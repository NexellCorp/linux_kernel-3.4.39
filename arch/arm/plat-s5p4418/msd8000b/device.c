...
#if defined(CONFIG_GPIO_MSD)

static struct platform_device msd_gpio_device = {
	.name			= "msd-gpio",
	.id	= -1,
};
...
#include <linux/i2c.h>
#define	Goodix80X_I2C_BUS		(1)

static struct i2c_board_info __initdata Goodix80X_i2c_bdi = {
	.type	= "Goodix-MSD",
	.addr	= (0x14),
...
#if defined(CONFIG_GPIO_MSD)
    printk("plat: add device msd-gpio\n");
    platform_device_register(&msd_gpio_device);
#endif
