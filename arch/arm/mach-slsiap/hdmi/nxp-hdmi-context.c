...
static int _get_vsync_info(struct nxp_hdmi_context *me)
{
	...
	par->delay_mask = (DISP_SYNCGEN_DELAY_RGB_PVD	 |
...
static int _set_remote_sync(struct nxp_hdmi_context *me)
{
	...
	if (me->source_device != DISP_DEVICE_RESCONV) {
		/* nxp4330 don't support interlace */
		me->dpc_sync_info.interlace = false;

		ret = nxp_soc_disp_device_set_sync_param(me->source_device,
						(void*)&me->dpc_sync_param);
		if (ret) {
		...
		ret = nxp_soc_disp_device_connect_to(DISP_DEVICE_HDMI,
						     me->source_device,
						     &me->dpc_sync_info);
		if (ret) {
...
static inline void _set_hdmi_clkgen(struct nxp_hdmi_context *me)
{
	...
	/* use PLL2 */
	NX_DISPTOP_CLKGEN_SetClockSource(ToMIPI_CLKGEN, HDMI_SPDIF_CLKOUT, 2);
...
static inline void _calc_hdmi_sync_param(struct nxp_hdmi_context *me)
{
	...
	// this is test
	pr_debug("%s: _h_sync_offset %d\n", __func__, _h_sync_offset);
	param->v_sync_start = vsync->v_sync_width + vsync->v_back_porch
		+ vsync->v_active_len - 1;
	param->h_active_start = vsync->h_sync_width + vsync->h_back_porch
		+ _h_sync_offset;
	param->h_active_end = vsync->h_active_len + vsync->h_sync_width
		+ vsync->h_back_porch + _h_sync_offset;
	param->v_sync_hs_start_end0 = vsync->h_sync_width + vsync->h_back_porch
		+ 1 + _h_sync_offset;
...
static int _write_tx_level(int level)
{
	...
	regval = NX_HDMI_GetReg(0, HDMI_PHY_Reg5C);
	val = (level & 0x1f) << 3;
	regval &= ~0xF8;
	regval |= val;
	pr_debug("[HDMIPHY] write reg5c: 0x%x\n", regval);
	NX_HDMI_SetReg(0, HDMI_PHY_Reg5C, regval);
...
static void _config_hdmi(struct nxp_hdmi_context *me)
{
	 ...
	 */
	h_blank = vsync->h_front_porch + vsync->h_sync_width
		+ vsync->h_back_porch;
	v_blank = vsync->v_front_porch + vsync->v_sync_width
		+ vsync->v_back_porch;
	v_actline = vsync->v_active_len;
	/* total v */
	v2_blank = v_line = v_actline + v_blank;
	/* total h */
	h_line = vsync->h_active_len + vsync->h_front_porch
		+ vsync->h_sync_width + vsync->h_back_porch;
	...
	/* 480p, 576p must invert VSYNC, HSYNC */
...
static struct hdmi_preset_conf *_hdmi_preset2conf(u32 preset)
{
	int i;
...
static inline void _hdmi_enable(struct nxp_hdmi_context *me)
{
	...
	NX_DISPLAYTOP_HDMI_SetVSyncHSStartEnd(
			me->hdmi_sync_param.v_sync_hs_start_end0,
			me->hdmi_sync_param.v_sync_hs_start_end1);
}
...
#define HPD_WORK_DELAY	10
#else
#define HPD_WORK_DELAY	1000
#endif
static void clear_hdcp_int(void)
{
	u8 flag;
...
static irqreturn_t _hdmi_irq_handler(int irq, void *dev_data)
{
	...
	if (flag & HDMI_INTC_FLAG_HPD_UNPLUG) {
		hdmi_write_mask(HDMI_LINK_INTC_FLAG_0, ~0,
				HDMI_INTC_FLAG_HPD_UNPLUG);
		hpd_changed = true;
		pr_info("[HPD UNPLUG INT]\n");
	}
	if (flag & HDMI_INTC_FLAG_HPD_PLUG) {
		hdmi_write_mask(HDMI_LINK_INTC_FLAG_0, ~0,
				HDMI_INTC_FLAG_HPD_PLUG);
		hpd_changed = true;
		pr_info("[HPD PLUG INT]\n");
	}

	if (hpd_changed)
		queue_delayed_work(system_nrt_wq, &me->hpd_work,
				   msecs_to_jiffies(HPD_WORK_DELAY));

#if defined(CONFIG_NXP_HDMI_USE_HDCP) || \
    defined(CONFIG_NXP_DISPLAY_HDMI_USE_HDCP)
...
static void _hdmi_hpd_changed(struct nxp_hdmi_context *me, int state)
{
	...
	if (state) {
		...
		me->cur_conf = _hdmi_preset2conf(me->cur_preset);
	}
#if 0
	/* HACK: clear all HDCP Event? */
	else {
		pr_info("%s: clear HDCP event\n");
		me->hdcp.event = 0;
	}
#endif
...
{
	int state;
	struct nxp_hdmi_context *me = container_of(work,
		      ...
		      struct nxp_v4l2_i2c_board_info *hdcp)
{
	...
	ret = request_irq(me->irq, _hdmi_irq_handler, IRQF_DISABLED, "hdmi-int",
			  me);
	if (ret < 0) {
...
void hdmi_hook_set_mux(struct nxp_hdmi_context *me,
		       void (*hook)(struct nxp_hdmi_context *))
{
...
int hdmi_run(struct nxp_hdmi_context *me, bool set_remote_sync)
{
	...
	defined(CONFIG_NXP_DISPLAY_HDMI_AUDIO_I2S)
...
#endif
	/* HACK: Where is the origin of below code? */
#if 0
	_write_tx_level(15);              /* Yangshl add for HDMI EMI test 20160927 */
#endif
...
void hdmi_stop(struct nxp_hdmi_context *me)
{
	...
	if (DISP_DEVICE_RESCONV != me->source_device) {
		...
		nxp_soc_disp_device_disconnect(DISP_DEVICE_HDMI,
					       me->source_device);
	}
...
int hdmi_set_dv_preset(struct nxp_hdmi_context *me,
...
uint32_t hdmi_get_edid_preset(struct nxp_hdmi_context *me,
			      uint32_t preferred_preset)
{
	...
	pr_err("%s: preferred_preset %d is not supported, use edid preferred preset\n",
	       __func__, preferred_preset);
	return me->edid.preferred_preset(&me->edid);
}

struct hdmi_irq_callback *hdmi_register_irq_callback(int irq_num,
						     int (*callback)(void *),
						     void *data)
{
	if (!__me) {
		...
		struct hdmi_irq_callback *entry =
			kmalloc(sizeof(struct hdmi_irq_callback), GFP_KERNEL);
		if (!entry) {
...
void hdmi_register_notifier_hpd_changed(struct nxp_hdmi_context *me,
					void (*notify)(void *, int), void *data)
{
