...
static int _read_txclk_level(void)
{
	if (!_me->enabled)
		return -1;
	else {
		int level = 0;
		level = NX_HDMI_GetReg(0, HDMI_PHY_Reg5C);
		level = level >> 3;
		level &= 0x1F;
		return level;
	}
}
static ssize_t tx_level_show(struct device *pdev, struct device_attribute *attr,
			     char *buf)
{
	int tx_level = _read_tx_level();
	int clk_level = _read_txclk_level();
	return scnprintf(buf, PAGE_SIZE, "data level %d, clk level:%d\n",
			 tx_level, clk_level);
}

static ssize_t tx_level_store(struct device *pdev,
...
static struct device_attribute tx_level_attr =
__ATTR(tx_level, 0666,tx_level_show, tx_level_store);
...
static int _init_sysfs(void)
{
	...
	if (!kobj) {
		pr_err("%s: failed to kobject_create_and_add for hdmiphy\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (ret) {
		pr_err("%s: failed to sysfs_create_group for hdmiphy\n",
		       __func__);
		kobject_del(kobj);
		return -ret;
	}
...
static int nxp_hdmiphy_s_stream(struct nxp_hdmiphy *me, int enable)
{
	...
	if (enable) {
		ret = _hdmiphy_reg_set(me, me->preset,
				       NXP_HDMIPHY_PRESET_TABLE_SIZE);
		if (ret < 0) {
