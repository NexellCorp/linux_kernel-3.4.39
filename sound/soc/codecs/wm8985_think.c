...
#if !defined(FEATURE_NAVIGATION) && defined(FEATURE_IN_DASH)
#undef FEATURE_IN_DASH
#endif

#if !defined(FEATURE_NAVIGATION) && defined(FEATURE_ON_DASH)
#undef FEATURE_ON_DASH
#endif

#if defined(FEATURE_ON_DASH) && defined(FEATURE_IN_DASH)
...
#ifdef SUPPORT_REGULATOR_CONTROL
...
static const u16 wm8985_reg_defs[] = {
	...
	0x0180,     /* R2  - Power management 2 */
	0x0003,     /* R3  - Power management 3 */
	0x0012,     /* R4  - Audio Interface, Main Mic ADC and DAC data appears in right phase of LRC */
	...
	0x00F5,     /* R16 - Right ADC Digital Vol */
	0x0000,     /* R17 */
	0x002C,     /* R18 - EQ1 - low shelf */
	0x002C,     /* R19 - EQ2 - peak 1 */
	0x0162,     /* R20 - EQ3 - peak 2 */
	0x0156,     /* R21 - EQ4 - peak 3 */
	0x001F,     /* R22 - EQ5 - high shelf */
	0x0000,     /* R23 */
	...
	0x0025,     /* R45 - Left INP PGA gain ctrl */
	0x0025,     /* R46 - Right INP PGA gain ctrl */
...
#ifdef MONO_SPEAKER_CONTROL
	SOC_DAPM_SINGLE("Mono Switch", WM8985_OUT4_MONO_MIX_CTRL, 0, 1, 0),
#endif
#ifdef LINEOUT_CONTROL
...
#ifdef MONO_SPEAKER_CONTROL
	SND_SOC_DAPM_PGA("Mono Out3 Speaker Out", WM8985_POWER_MANAGEMENT_3,
		7, 0, NULL, 0),
	SND_SOC_DAPM_PGA("Mono Out4 Speaker Out", WM8985_POWER_MANAGEMENT_3,
		8, 0, NULL, 0),
#endif
...
#ifdef MONO_SPEAKER_CONTROL
	{ "Right Output Mixer", "Mono Switch", "Right DAC" },
#endif
#ifdef LINEOUT_CONTROL
...
#ifdef MONO_SPEAKER_CONTROL
	{ "Left Output Mixer", "Mono Switch", "Left DAC" },
#endif
#ifdef LINEOUT_CONTROL
...
#ifdef MONO_SPEAKER_CONTROL
	{ "Mono Out3 Speaker Out", NULL, "Right Output Mixer" },
	{ "SPKR", NULL, "Mono Out3 Speaker Out" },

	{ "Mono Out4 Speaker Out", NULL, "Left Output Mixer" },
	{ "SPKL", NULL, "Mono Out4 Speaker Out" },
#endif
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	amp_onoff = gpio_get_value(CFG_IO_AUDIO_SPK_AMP_POWER);
	if (amp_onoff)
		ucontrol->value.integer.value[0] = 1;
	else
		ucontrol->value.integer.value[0] = 0;
...
{
	if (ucontrol->value.integer.value[0] == 0) {
		gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 0);
		gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, 0);
	} else if (ucontrol->value.integer.value[0] == 1) {
		gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 1);
		gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, 1);
		msleep(100);
	} else {
		return -EINVAL;
	}
...
static int wm8985_dac_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) { /* SOFTMUTE Disabled, AMP Off, POP Suppressor Off */
		gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, !mute);
		//gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 1); //MCJINO
#ifdef FEATURE_NAVIGATION
		// for aux white noise issue.
		gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, 1);
#else
		gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, !mute);
#endif
	}
...
#endif
	gpio_free(CFG_IO_AUDIO_SPK_AMP_POWER);
	gpio_free(CFG_IO_POP_SUPPRESSOR_ONOFF);
	return 0;
}
...
static size_t on_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	...
	if (force_spk) {
		...
		unsigned int reg = snd_soc_read(codecwrite, WM8985_LOUT1_HP_VOLUME_CTRL);
		// headphone state
		if ((reg & WM8985_LOUT1MUTE_MASK) == 0) {
			snd_soc_update_bits(codecwrite, WM8985_LOUT2_SPK_VOLUME_CTRL,
		                           WM8985_LOUT2MUTE_MASK,
		                           !!1 << WM8985_LOUT2MUTE_SHIFT);
		        snd_soc_update_bits(codecwrite, WM8985_ROUT2_SPK_VOLUME_CTRL,
		                           WM8985_ROUT2MUTE_MASK,
		                           !!1 << WM8985_ROUT2MUTE_SHIFT);
		}
...
static ssize_t amp_on_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", is_spk_on);
}

static size_t amp_on_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
    int val, err;
    ...
    printk("amp_on_store %d\n", val);

	if (err < 0) {
		printk("[%s] kstrtoint err !!!\n", __func__);
		return -EINVAL;
	} else if (val != 0 && val != 1) {
		printk("[%s] invalid value err !!!\n", __func__);
		return -EINVAL;
    ...
    if(!val) {
        gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 0);
        gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, 0);
        printk("amp_swithc put amp off\n");
    }else {
        gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 1);
        gpio_set_value(CFG_IO_POP_SUPPRESSOR_ONOFF, 1);
        printk("amp_swithc put amp on\n");
...
static ssize_t spk_on_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", is_spk_on);
}

static size_t spk_on_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
#ifdef FEATURE_IN_DASH
	int val, err;
	...
	is_spk_on = val;
	snd_soc_update_bits(codecwrite, WM8985_OUT3_MIXER_CTRL,
                           WM8985_OUT3MUTE_MASK,
                           !val << WM8985_OUT3MUTE_SHIFT);
	snd_soc_update_bits(codecwrite, WM8985_OUT4_MONO_MIX_CTRL,
                           WM8985_OUT4MUTE_MASK,
                           !val << WM8985_OUT4MUTE_SHIFT);
...
static ssize_t hp_vol_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int vol = WM8985_LOUT1VOL_MASK & snd_soc_read(codecwrite, WM8985_LOUT1_HP_VOLUME_CTRL);
	return sprintf(buf, "%d\n", vol);
}

static size_t hp_vol_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
#ifdef FEATURE_IN_DASH
	// aux volume contorl
	...
	if (err < 0) {
		printk("[%s] kstrtoint err !!!\n", __func__);
		return -EINVAL;
	} else if (val > DEFAULT_HP_VOL) {
		printk("[%s] invalid value (exceed hp max) : %d !!!\n", __func__, val);
		return -EINVAL;
	}
	...
	if (val <= 0) {
		...
		snd_soc_update_bits(codecwrite, WM8985_LOUT1_HP_VOLUME_CTRL,
		                   WM8985_LOUT1MUTE_MASK,
		                   !!0 << WM8985_LOUT1MUTE_SHIFT);
		snd_soc_update_bits(codecwrite, WM8985_ROUT1_HP_VOLUME_CTRL,
		                   WM8985_ROUT1MUTE_MASK,
		                   !!0 << WM8985_ROUT1MUTE_SHIFT);
...
static ssize_t mic_gain_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int vol = WM8985_INPPGAVOLR_MASK & snd_soc_read(codecwrite, WM8985_RIGHT_INP_PGA_GAIN_CTRL);
	return sprintf(buf, "%d\n", vol);
}

static size_t mic_gain_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	int val, err;
	...
	if (err < 0) {
		printk("[%s] kstrtoint err !!!\n", __func__);
		return -EINVAL;
	} else if (val > WM8985_INPPGAVOLR_MASK) {
		printk("[%s] invalid value (exceed mic gain) : %d !!!\n", __func__, val);
		return -EINVAL;
	}

	if (val <= 0) {
		val = 0;
		snd_soc_update_bits(codecwrite, WM8985_RIGHT_INP_PGA_GAIN_CTRL,
		                   WM8985_INPPGAMUTER_MASK,
		                   !!1 << WM8985_INPPGAMUTER_SHIFT);
	} else {
		snd_soc_update_bits(codecwrite, WM8985_RIGHT_INP_PGA_GAIN_CTRL,
		                   WM8985_INPPGAMUTER_MASK,
		                   !!0 << WM8985_INPPGAMUTER_SHIFT);
	}

        snd_soc_update_bits(codecwrite, WM8985_RIGHT_INP_PGA_GAIN_CTRL,
				WM8985_INPPGAVOLR_MASK,
				val << WM8985_INPPGAVOLR_SHIFT);

	return size;
}

#ifdef FEATURE_IN_DASH
static ssize_t aux_on_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", is_aux_on);
}

static size_t aux_on_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	int val, err;
	...
	snd_soc_update_bits(codecwrite, WM8985_LOUT1_HP_VOLUME_CTRL,
			    WM8985_LOUT1MUTE_MASK,
			    !val << WM8985_LOUT1MUTE_SHIFT);
	snd_soc_update_bits(codecwrite, WM8985_ROUT1_HP_VOLUME_CTRL,
			    WM8985_ROUT1MUTE_MASK,
			    !val << WM8985_ROUT1MUTE_SHIFT);
...
#ifdef FEATURE_ON_DASH
static DEVICE_ATTR(on, S_IRUGO | S_IWUGO,
		on_show, on_store);
#endif
static DEVICE_ATTR(spk_on, S_IRUGO | S_IWUGO,
		spk_on_show, spk_on_store);
static DEVICE_ATTR(hp_vol, S_IRUGO | S_IWUGO,
		hp_vol_show, hp_vol_store);
static DEVICE_ATTR(mic_gain, S_IRUGO | S_IWUGO,
		mic_gain_show, mic_gain_store);

static DEVICE_ATTR(amp_on, S_IRUGO | S_IWUGO,
        amp_on_show, amp_on_store);

#ifdef FEATURE_IN_DASH
static DEVICE_ATTR(aux_on, S_IRUGO | S_IWUGO,
		aux_on_show, aux_on_store);
#endif


static struct attribute *spk_hp_sysfs_attrs[] = {
#ifdef FEATURE_ON_DASH
        &dev_attr_on.attr,
#endif
        &dev_attr_spk_on.attr,
        &dev_attr_hp_vol.attr,
        &dev_attr_mic_gain.attr,
        &dev_attr_amp_on.attr,
#ifdef FEATURE_IN_DASH
	&dev_attr_aux_on.attr,
#endif
        NULL
};

static struct attribute_group spk_hp_attribute_group = {
	.attrs = spk_hp_sysfs_attrs,
};

struct switch_dev spk_hp = {
	.name = "spk_hp",
};
...
static void wm8985_register_setting(struct snd_soc_codec *codec ) {
	...
	snd_soc_write(codec,WM8985_LEFT_INP_PGA_GAIN_CTRL,0x125);//R45(0x2D)
	snd_soc_write(codec,WM8985_RIGHT_INP_PGA_GAIN_CTRL,0x125);//R46(0x2E)
	snd_soc_write(codec,WM8985_LEFT_ADC_BOOST_CTRL,0x100);//R47
	snd_soc_write(codec,WM8985_RIGHT_ADC_BOOST_CTRL,0x100);//R48

	//ADC-EQ
	snd_soc_write(codec,WM8985_EQ1_LOW_SHELF,0x02C);//R18(0x12)
	snd_soc_write(codec,WM8985_EQ2_PEAK_1,0x02C);//R19(0x13)
	snd_soc_write(codec,WM8985_EQ3_PEAK_2,0x162);//R20(0x14)
	snd_soc_write(codec,WM8985_EQ4_PEAK_3,0x156);//R21(0x15)
	snd_soc_write(codec,WM8985_EQ5_HIGH_SHELF,0x01F);//R22(0x16)

	snd_soc_write(codec,WM8985_LEFT_ADC_BOOST_CTRL,0x100);//R47(0x2F)
	snd_soc_write(codec,WM8985_RIGHT_ADC_BOOST_CTRL,0x100);//R48(0x30)
}

#endif //FEATURE_NAVIGATION
...
static int wm8985_probe(struct snd_soc_codec *codec)
{
...
#ifdef FEATURE_NAVIGATION
	ret = gpio_request(CFG_IO_CODEC_POWER_EN, "CFG_IO_CODEC_POWER_EN");
	if (ret) {
		dev_err(codec->dev,"[%s] CFG_IO_CODEC_POWER_EN gpio request error : %d\n",__func__,ret);
		goto exit_codec_power_enable_gpio_request_failed;
	}
	gpio_direction_output(CFG_IO_CODEC_POWER_EN, 1);
	mdelay(50);
#endif
	...
	ret = gpio_request(CFG_IO_AUDIO_SPK_AMP_POWER, "SPEAKER_AMP_ONOFF");
	if (ret) {
		dev_err(codec->dev,"[%s] SPEAKER_AMP_ONOFF gpio request error : %d\n",__func__,ret);
		goto exit_spk_amp_onoff_gpio_request_failed;
	}
	//gpio_direction_output(CFG_IO_AUDIO_SPK_AMP_POWER, 0);
	gpio_direction_output(CFG_IO_AUDIO_SPK_AMP_POWER, 1); //MCJINO
	//gpio_set_value(CFG_IO_AUDIO_SPK_AMP_POWER, 1);

	ret = gpio_request(CFG_IO_POP_SUPPRESSOR_ONOFF, "POP_SUPPRESSOR_ONOFF");
	if (ret) {
...
#ifdef FEATURE_NAVIGATION
	// for aux white noise issue.
	gpio_direction_output(CFG_IO_POP_SUPPRESSOR_ONOFF, 1);
#else
	gpio_direction_output(CFG_IO_POP_SUPPRESSOR_ONOFF, 0);
#endif
...
#ifdef FEATURE_NAVIGATION
exit_codec_power_enable_gpio_request_failed:
	gpio_free(CFG_IO_CODEC_POWER_EN);
#endif
...
#ifdef FEATURE_NAVIGATION
	if (!ret) {
		ret = switch_dev_register(&spk_hp);
		if (ret < 0) {
			printk("%s : Failed to register spk_hp switch device\n", __func__);
		} else {
			ret = sysfs_create_group(&spk_hp.dev->kobj, &spk_hp_attribute_group);
			if (ret) {
				printk("[%s] create spk_hp sysfs err !!!!!!!!!!!!!\n", __func__);
			}
		}
	}
#endif
#ifdef CONFIG_SOUND_TUNE
	if (!ret) {
		ret = misc_register(&snd_tune_dev);
		if (ret) {
			DEBUG_MSG("[DIAG] misc_register failed (ret=%d)", ret);
		}
	}
#endif // CONFIG_SOUND_TUNE
#endif
...
#ifdef FEATURE_NAVIGATION
	switch_dev_unregister(&spk_hp);
	sysfs_remove_group(&spk_hp.dev->kobj, &spk_hp_attribute_group);
#endif
#ifdef CONFIG_SOUND_TUNE
