...
static inline void __sync_capture_start(void)
{
	...
	int  count[3] = { 0, };
	bool bLRCK = false;
	unsigned int run_val = __raw_readl(IO_BASE(PDM_IO_ISRUN)) | (1<<run_bit);
	unsigned int lrck_val = __raw_readl(IO_BASE(PDM_IO_LRCLK));
	...
	base = (void*)IO_ADDRESS(PHY_BASEADDR_I2S0);//0xf0055000;
#endif
#ifdef CFG_AUDIO_LOOPBACK_I2S1
	base = (void*)IO_ADDRESS(PHY_BASEADDR_I2S1);//0xf0056000;
#endif
#ifdef CFG_AUDIO_LOOPBACK_I2S2
	base = (void*)IO_ADDRESS(PHY_BASEADDR_I2S2);//0xf0057000;
#endif
