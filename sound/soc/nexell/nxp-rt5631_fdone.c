...
#include "nxp-i2s.h"
#include "../codecs/rt5631_fdone.h"

/*
#define	pr_debug	printk
*/

#if defined (CFG_IO_AUDIO_RT5631_AMP_EN)
#include <linux/gpio.h>
#define AUDIO_AMP_EN		CFG_IO_AUDIO_RT5631_AMP_EN
#define AUDIO_LINE_OUT_SEL1	CFG_IO_AUDIO_RT5631_LINE_OUT_SEL1
#define AUDIO_LINE_OUT_SEL2 CFG_IO_AUDIO_RT5631_LINE_OUT_SEL2
#endif
	...
	struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		gpio_set_value(AUDIO_LINE_OUT_SEL1, 1);
		gpio_set_value(AUDIO_LINE_OUT_SEL2, 1);
	} else {
		pr_debug("AMP_OFF\n");
		gpio_set_value(AUDIO_LINE_OUT_SEL1, 1);
		gpio_set_value(AUDIO_LINE_OUT_SEL2, 0);
...
static const struct snd_soc_dapm_route rt5631_audio_map[] = {
	/* headphone connected to AUXO1 */
	{"Headphone Jack", NULL, "AUXO1"},
...
#if defined (AUDIO_AMP_EN)
	gpio_request(AUDIO_AMP_EN, "rt5631_amp_en");
	gpio_direction_output(AUDIO_AMP_EN, 0);
	gpio_request(AUDIO_LINE_OUT_SEL1, "rt5631_line_out_sel1");
	gpio_direction_output(AUDIO_AMP_EN, 0);
	gpio_request(AUDIO_LINE_OUT_SEL2, "rt5631_line_out_sel2");
	gpio_direction_output(AUDIO_AMP_EN, 0);
#endif
	/*
	 * register card
...
static int rt5631_remove(struct platform_device *pdev)
{
	...
	gpio_free(AUDIO_LINE_OUT_SEL1);
	gpio_free(AUDIO_LINE_OUT_SEL2);
#endif
	return 0;
}
