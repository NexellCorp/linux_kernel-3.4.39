...
static void nxp_pcm_file_mem_allocate(char *filename,
			...
			struct snd_pcm_hw_params *params)
{
	...
	prtd->mem_len =
		(sample_rate * sample_ch * (sample_bits/8)) * DUMP_DMA_TIME;
	...
	pr_debug("file mem=0x%08x(%ld=rate:%d,ch:%d,bits:%d,time:%dsec)\n",
		...
		sample_bits, DUMP_DMA_TIME);
}

static void nxp_pcm_file_mem_free(char *filename,
			struct snd_pcm_substream *substream)
{
	struct nxp_pcm_runtime_data *prtd = substream_to_prtd(substream);
	...
	if (prtd->mem_area) {
		...
		if (filp) {
			...
			set_fs(KERNEL_DS);
			vfs_write(filp,
				(void *)prtd->mem_area, prtd->mem_len, &pos);
			set_fs(old_fs);
...
static int nxp_pcm_dma_prepare_and_submit(struct snd_pcm_substream *substream)
{
	...
	if (prtd->dma_param->real_clock != 0) /* i2s master mode */
		...
		period_time_us = 1000; /* i2s slave mode */
					...
					struct snd_pcm_hw_params *params)
{
	...
	dev_dbg(prtd->dev,
		"bytes: buffer=%6d, period=%6d, periods=%2d, time=%3lld us\n",
		prtd->buffer_bytes, prtd->period_bytes,
		prtd->periods, prtd->period_time_us);
...
static int nxp_pcm_new(struct snd_soc_pcm_runtime *runtime)
{
	...
	if (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream) {
		ret = nxp_pcm_preallocate_dma_buffer(pcm,
