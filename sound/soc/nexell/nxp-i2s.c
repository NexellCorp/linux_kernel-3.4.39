...
#define SND_NXP_DFS_PLLNO	5
#endif

#define	DEF_SAMPLE_RATE	48000
#define	DEF_FRAME_BIT	32

#define	I2S_BASEADDR	PHY_BASEADDR_I2S0
#define	I2S_CH_OFFSET	0x1000
#define	I2S_BUS_WIDTH	4
#define	I2S_PERI_BURST	4
...
#if defined(CONFIG_ARCH_S5P4418)
#define	PSR_PSRAEN_POS			15
#define	PSR_PSVALA_POS			8
#endif
...
static inline void i2s_reset(struct nxp_i2s_snd_param *par)
{
...
static int nxp_i2s_check_param(struct nxp_i2s_snd_param *par)
{
	...
	case 48:
		...
		break;
	default:
		pr_err("Fail, not support i2s frame bits %d (32, 48)\n",
			par->frame_bit);
		return -EINVAL;
	}
	...
	if (i >= ARRAY_SIZE(clk_ratio)) {
		pr_err("Fail, not support i2s sample rate %d\n",
			par->sample_rate);
		return -EINVAL;
	}
	...
	if (par->ext_is_en) {
		...
		if (BFS_48BIT == BFS) {
			request = par->sample_rate * 384;
			RFS = RATIO_384;
		} else {
			request = par->sample_rate * 256;
			RFS = RATIO_256;
		}
		rate_hz = request;
		if (par->pre_supply_mclk) {
...
#if defined(CONFIG_ARCH_S5P4418)
		en_pclk = set_sample_rate_clock(par->clk,
				request, &rate_hz, &divide);
#elif defined(CONFIG_ARCH_S5P6818)
		set_sample_rate_clock(par->clk, request, &rate_hz, &divide);
#endif
	...
	i2s->CSR = (BLC << CSR_BLC_POS) |
		...
		(BFS << CSR_BFS_POS);
#if defined(CONFIG_ARCH_S5P4418) && !defined(CONFIG_SND_NXP_DFS)
	i2s->PSR = ((PSRAEN & 0x1) << PSR_PSRAEN_POS) |
			((prescale & 0x3f) << PSR_PSVALA_POS);
#endif
	i2s_reset(par);
	...
	pr_info("i2s:ch%d,%s,%smode, %d(%ld)hz, %dFBITs, MCLK=%ldhz, RFS=%d\n",
	...
	pr_debug("i2s: BLC=%d, IMS=%d, LRP=%d, SDF=%d, RFS=%d, BFS=%d\n",
		BLC, IMS, LRP, SDF, RFS, BFS);
#if defined(CONFIG_ARCH_S5P4418) && !defined(CONFIG_SND_NXP_DFS)
	pr_debug("i2s: PSRAEN=%d, PSVALA=%d\n", PSRAEN, prescale);
#endif
#if !defined(CONFIG_SND_NXP_DFS)
	/* i2s support rates */
...
static int nxp_i2s_set_plat_param(struct nxp_i2s_snd_param *par, void *data)
{
	...
	par->sample_rate = plat->sample_rate ?
			plat->sample_rate : DEF_SAMPLE_RATE;
	...
	for (i = 0; 2 > i; i++, dma = &par->capt) {
		...
		dma->dma_ch_name = (char *)(i == 0 ? plat->dma_play_ch :
						plat->dma_capt_ch);
		dma->peri_addr =
			phy_base + (i == 0 ? I2S_TXD_OFFSET : I2S_RXD_OFFSET);
		dma->bus_width_byte = I2S_BUS_WIDTH;
		dma->max_burst_byte = I2S_PERI_BURST;

		pr_debug("snd i2s: %s dma (%s, peri 0x%x, bus %dbits)\n",
			STREAM_STR(i), dma->dma_ch_name, dma->peri_addr,
...
static int nxp_i2s_set_dfs_sysclk(struct nxp_i2s_snd_param *par)
{
	...
	dmap_play->real_clock = rate_hz / (RATIO_256 == RFS ? 256 : 384);
	dmap_capt->real_clock = rate_hz / (RATIO_256 == RFS ? 256 : 384);
	pr_debug("i2s: ch %d,%s,%s mode,%d(%ld)hz,%dFBITs,MCLK=%ldhz,RFS=%d\n",
		par->channel, par->master_mode ? "master" : "slave",
		par->trans_mode == 0 ? "iis" : (par->trans_mode == 1 ?
...
#if !defined(CONFIG_SND_NXP_DFS)
		if (RFS == RATIO_256 || BFS != BFS_48BIT) {
			pr_err("Fail, i2s RFS 256/BFS 32\n");
			pr_err("not support 24 sample bits\n");
			return -EINVAL;
		}
#endif
		if (BLC != BLC_24BIT) {
...
static void nxp_i2s_dai_init(struct snd_soc_dai_driver *dai)
{
	dai->probe = nxp_i2s_dai_probe;
	dai->remove = nxp_i2s_dai_remove;
	dai->suspend = nxp_i2s_dai_suspend;
	dai->resume = nxp_i2s_dai_resume;
	dai->ops = &nxp_i2s_ops;
...
static __devinit int nxp_i2s_probe(struct platform_device *pdev)
{
	...
	nxp_i2s_dai_init(&par->dai_drv);

	ret = nxp_i2s_set_plat_param(par, pdev);
	if (ret)
		goto err_out;

	ret = snd_soc_register_dai(&pdev->dev, &par->dai_drv);
	if (ret) {
		pr_err("fail, %s snd_soc_register_dai ...\n",
			pdev->name);
		goto err_out;
	}
